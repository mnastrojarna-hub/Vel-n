...
| `create_web_booking(p_moto_id, p_start_date, p_end_date, p_name, p_email, p_phone, ...)` | Vytvoří web rezervaci: najde/vytvoří auth usera + profil, zkontroluje dostupnost, vypočte cenu, vytvoří booking (source='web'). **Po vytvoření bookingu vloží jednotlivé extras z p_extras do booking_extras (name, unit_price, quantity).** p_extras formát: `[{name, unit_price}]`. Vrací {booking_id, amount, user_id}. SECURITY DEFINER |
