...
| `create_web_booking(..., p_booking_source)` | Vytvoří rezervaci: najde/vytvoří auth usera + profil, zkontroluje dostupnost, vypočte cenu (per-day pricing price_mon..price_sun, inclusive start+end), vytvoří booking. **p_booking_source** (text, default 'web') — 'app' nebo 'web'. p_extras formát: `[{name, unit_price}]`. p_password: nastaví heslo. Velikosti výbavy řidiče + spolujezdce (10 size params). Vrací {booking_id, amount, user_id}. SECURITY DEFINER |
