...
| `confirm_payment(booking_id, method)` | RPC: označí booking jako zaplacený, přepne pending→**reserved** (+ confirmed_at), SECURITY DEFINER |
...
| 2026-03-12 | **FIX confirm_payment:** Oprava přechodu stavu z `pending→active` na `pending→reserved` + nastavení `confirmed_at`. Booking po zaplacení přechází do reserved (čeká na vydání), ne active (motorka vydána). Stav active nastavuje admin ve Velínu při vydání motorky |
