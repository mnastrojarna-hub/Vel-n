...
export async function pushInvoice(supabase, flexiBase, flexiAuth, eventId) {
  ...
  const { responseBody, responseCode, error } = await flexiPost(`${flexiBase}/faktura-vydana.json`, flexiAuth, payload);
  ...
  if (error || !responseCode || responseCode >= 400) {
    ...
    return {
      ok: false,
      error: error || 'Flexi rejected request',
      responseCode
    };
  }
...
export async function pushExpense(supabase, flexiBase, flexiAuth, eventId) {
  const event = await loadEvent(supabase, eventId);
  const payload = {
    'winstrom': {
      'faktura-prijata': {
  ...
  const { responseBody, responseCode, error } = await flexiPost(`${flexiBase}/faktura-prijata.json`, flexiAuth, payload);
  const flexiId = responseBody?.winstrom?.results?.[0]?.id?.toString() || null;
...
export async function pushAsset(supabase, flexiBase, flexiAuth, eventId) {
  const event = await loadEvent(supabase, eventId);
  const payload = {
    'winstrom': {
      'majetek': {
  ...
  const { responseBody, responseCode, error } = await flexiPost(`${flexiBase}/majetek.json`, flexiAuth, payload);
  const flexiId = responseBody?.winstrom?.results?.[0]?.id?.toString() || null;
...
export async function pullVatReport(flexiBase, flexiAuth, period) {
  return {
    ok: true,
    vat_payer: false,
...
export async function pullAccountingState(flexiBase, flexiAuth, year) {
  const { responseBody, responseCode, error } = await flexiGet(`${flexiBase}/ucetni-zasady.json`, flexiAuth);
  if (error || !responseCode || responseCode >= 400) {
    return {
      ok: false,
      error: error || 'Flexi API error',
      responseCode
    };
  }
  return {
