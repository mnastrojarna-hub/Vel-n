...
/** Base64url encode for JWT */ function base64url(input) {
  return btoa(input).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
}
/** Create a signed JWT for Google OAuth2 using service account */ async function createGoogleJWT(serviceAccount) {
  ...
  const cryptoKey = await crypto.subtle.importKey('pkcs8', keyBinary, {
...
/** Get OAuth2 access token from Google */ async function getAccessToken(serviceAccount) {
...
/** Send FCM v1 push notification */ async function sendFCM(accessToken, projectId, deviceToken, title, body, data) {
  ...
  try {
    ...
    return {
      success: false,
      error: `FCM ${res.status}: ${errBody}`
    };
  } catch (e) {
    return {
      success: false,
      error: `FCM fetch error: ${e.message}`
    };
  }
}
serve(async (req)=>{
  ...
  try {
    ...
    if (!isServiceRole(token)) {
      return jsonResponse({
        success: false,
        error: 'Unauthorized — service_role only'
      }, 401);
    }
    ...
    const { data: profile } = await supabase.from('profiles').select('consent_push').eq('id', user_id).maybeSingle();
    if (profile && profile.consent_push === false) {
      return jsonResponse({
        success: true,
        sent: 0,
        reason: 'Push consent disabled'
      });
    }
    // Get user's active push tokens
    const { data: tokens, error: tokErr } = await supabase.from('push_tokens').select('token, platform').eq('user_id', user_id).eq('active', true);
