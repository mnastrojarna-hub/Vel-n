...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: newOrders } = await sb.from('purchase_orders').select('id, order_number, notes, supplier_id, suppliers(name, contact_email)').eq('status', 'draft').gte('created_at', fiveMinAgo);
    let emailsSent = 0;
    ...
    for (const order of newOrders || []){
      ...
      const { data: items } = await sb.from('purchase_order_items').select('quantity, unit_price, inventory(name, sku)').eq('order_id', order.id);
      ...
      const { error: emailErr } = await sb.functions.invoke('send-order-email', {
        body: {
          ...
          supplier_name: order.suppliers?.name || 'Dodavatel',
          items: emailItems,
          notes: order.notes || '',
          order_id: order.id,
