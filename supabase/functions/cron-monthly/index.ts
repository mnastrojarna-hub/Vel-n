...
/** 1. Měsíční výkonnostní report — motorky a pobočky za předchozí měsíc. */ async function generateMonthlyPerformance(admin) {
  try {
    const now = new Date();
    ...
    // Výkonnost motorek
    const { data: motos } = await admin.from('motorcycles').select('id, model, license_plate, branch_id');
    const motoPerformance = [];
    for (const moto of motos ?? []){
      const { data: bookings } = await admin.from('bookings').select('total_price, start_date, end_date, status').eq('moto_id', moto.id).gte('start_date', periodFrom).lte('start_date', periodTo);
    ...
    for (const branch of branches ?? []){
      const branchMotos = motoPerformance.filter((m)=>m.branch_id === branch.id);
      ...
      const avgUtilization = branchMotos.length > 0 ? Math.round(branchMotos.reduce((s, m)=>s + m.utilization_percent, 0) / branchMotos.length) : 0;
      // Výdaje pobočky
      const { data: expenses } = await admin.from('accounting_entries').select('amount').eq('type', 'expense').eq('branch_id', branch.id).gte('date', periodFrom).lte('date', periodTo);
...
/** 2. Aktualizace predikcí — spustí prediction-engine pro 3 měsíce. */ async function updatePredictions() {
  try {
