...
/** Sync all Stripe payment methods for a customer to the Supabase payment_methods table */ async function syncCardsToSupabase(supabase, userId, customerId) {
  try {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: profile } = await supabase.from('profiles').select('stripe_customer_id, full_name, email, phone').eq('id', user.id).single();
    ...
    if (action === 'delete') {
      ...
      const { data: ownedCard, error: ownerErr } = await supabase.from('payment_methods').select('id').eq('stripe_payment_method_id', payment_method_id).eq('user_id', user.id).maybeSingle();
      ...
      try {
        ...
        console.warn('Stripe detach failed (cleaning up locally):', e.message);
      }
      // Remove from Supabase table
      try {
        await supabase.from('payment_methods').delete().eq('stripe_payment_method_id', payment_method_id).eq('user_id', user.id);
      } catch (e) {
    ...
    if (action === 'set_default') {
      ...
      const { data: ownedPm } = await supabase.from('payment_methods').select('id').eq('stripe_payment_method_id', payment_method_id).eq('user_id', user.id).maybeSingle();
      if (!ownedPm) {
        return new Response(JSON.stringify({
          success: false,
          error: 'Karta nebyla nalezena nebo vám nepatří.'
        }), {
          status: 403,
          headers: {
            ...CORS,
      ...
      if (customerId) {
        try {
          ...
          console.warn('Stripe set_default failed (updating locally):', e.message);
        }
      }
    ...
    if (!customerId) {
      ...
      const { data: localCards } = await supabase.from('payment_methods').select('stripe_payment_method_id, brand, last4, exp_month, exp_year, holder_name, is_default').eq('user_id', user.id).order('is_default', {
        ascending: false
      });
    ...
    if (action === 'list' || !action) {
      ...
      try {
        ...
        console.warn('Stripe customer retrieve failed, returning local cards:', e.message);
