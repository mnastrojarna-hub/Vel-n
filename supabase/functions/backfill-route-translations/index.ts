...
async function callTranslateContent(table, id, fields, langs) {
 ...
 *  v některém jazyce u řádku, který popis má. */ const MISSING_OR = DEFAULT_LANGS.flatMap((l)=>[
    `translations->${l}->>name.is.null`,
    `and(description.not.is.null,translations->${l}->>description.is.null)`
  ]).join(',');
/** Vybere řádky, kterým chybí překlad v kterémkoli cílovém jazyce. */ async function fetchMissing(db, table, limit) {
  const { data, error } = await db.from(table).select('id, name, description').or(MISSING_OR).limit(limit);
  if (error) throw new Error(error.message);
  return data || [];
}
async function processTable(db, table, rows, langs, concurrency) {
 ...
 *  projekt může mít nový formát klíčů sb_secret_ / víc platných klíčů. */ // service_role se ověřuje sdíleným helperem (_shared/auth.ts): claim v payloadu
...
serve(async (req)=>{
  ...
  if (!authOk) return jsonResponse({
    error: 'service_role required'
  }, 401);
  const body = await req.json().catch(()=>({}));
  const scope = body.scope || 'all';
  const limit = Math.min(Math.max(Number(body.limit) || 12, 1), 40);
  const langs = Array.isArray(body.target_langs) && body.target_langs.length > 0 ? body.target_langs : DEFAULT_LANGS;
