...
async function callClaudeVision(apiKey, imageBase64, mediaType, isRetry = false) {
  ...
  try {
    ...
    if (!response.ok) {
      console.error('Claude Vision error:', response.status, await response.text());
      return null;
    }
...
Deno.serve(async (req)=>{
  ...
  if (!image) return jsonResponse({
    success: false,
    error: 'missing_image'
  }, 400);
  if (image.length > 12_000_000) return jsonResponse({
    success: false,
