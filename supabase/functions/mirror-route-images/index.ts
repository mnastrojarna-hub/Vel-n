...
const isWiki = (u)=>typeof u === 'string' && (u.includes('wikimedia.org') || u.includes('wikipedia.org'));
...
async function fetchWiki(url) {
  ...
  const candidates = [
    {
      u: `https://images.weserv.nl/?url=${encodeURIComponent(url)}&w=${THUMB_WIDTH}&output=jpg&q=80`,
      h: {}
    },
    {
      u: `${url}${sep}width=${THUMB_WIDTH}`,
      h: WIKI_HEADERS
    },
    {
      u: url,
...
async function runBatch(sb) {
  ...
  const outOfBudget = ()=>Date.now() - t0 > TIME_BUDGET_MS;
  {
    ...
    for (const r of routesData){
      const urls = [
        ...isWiki(r.cover_image) ? [
          r.cover_image
        ] : [],
        ...(r.images || []).filter(isWiki)
      ];
      if (urls.length) rows.push({
        table: 'routes',
        id: r.id,
        patch: {
    ...
    const addPoiRows = (data, table)=>{
      for (const p of data){
        const urls = [
          ...isWiki(p.image_url) ? [
            p.image_url
          ] : [],
          ...(p.images || []).filter(isWiki)
        ];
        if (urls.length) rows.push({
          table,
          id: p.id,
    ...
    const mirrored = new Map() // stará URL → nová URL
    ;
    const failedTransient = new Set() // 429/5xx/net — zkusí se příště
    ;
    const failedPermanent = new Set() // 404/415/422 — nemá smysl opakovat
    ;
    const failSamples = {} // důvod → počet
    ;
    let images = 0;
    let updated = 0;
    let processedRows = 0;
    const note = (why)=>{
      failSamples[why] = (failSamples[why] || 0) + 1;
    };
    ...
    const mirrorOne = async (url)=>{
      ...
      if (up.error) {
        failedTransient.add(url);
        note(`upload_${up.error.message}`.slice(0, 40));
        return;
      }
    ...
    for (const row of rows){
      const swap = (v)=>typeof v === 'string' && mirrored.has(v) ? mirrored.get(v) : v;
      const patch = {};
      if ('cover_image' in row.patch && isWiki(row.patch.cover_image) && mirrored.has(row.patch.cover_image)) {
        patch.cover_image = swap(row.patch.cover_image);
      }
      if ('image_url' in row.patch && isWiki(row.patch.image_url) && mirrored.has(row.patch.image_url)) {
        patch.image_url = swap(row.patch.image_url);
      }
      const imgs = row.patch.images;
      if (Array.isArray(imgs) && imgs.some((u)=>isWiki(u) && mirrored.has(u))) {
      ...
      const rowDone = [
        ...new Set(row.urls)
      ].every((u)=>mirrored.has(u) || failedPermanent.has(u));
