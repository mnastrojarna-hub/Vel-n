...
async function execTool(name, args) {
  switch(name){
    case 'motogo_search_motorcycles':
      {
        let q = sb.from('motorcycles').select('id, model, brand, category, engine_cc, power_kw, license_required, color, image_url, branch_id, price_mon, price_tue, price_wed, price_thu, price_fri, price_sat, price_sun, ideal_usage').eq('status', 'active').order('model');
    ...
    case 'motogo_get_motorcycle':
      {
        const { data, error } = await sb.from('motorcycles').select('*, branches(name, address, city, latitude, longitude)').eq('id', args.moto_id).maybeSingle();
        if (error) throw new Error(error.message);
    ...
    case 'motogo_get_faq':
      {
        ...
        const faqs = [
          ...
          {
            q: 'Jak platím?',
            a: 'Online platební kartou přes Stripe. Visa, Mastercard, Amex, Apple Pay, Google Pay.'
          },
          {
            q: 'Otevírací doba?',
