    ...
    inputSchema: {
      ...
      properties: {
        ...
        license_group: {
          ...
          description: 'Required driver license group. N = no license needed (kids motorcycles).'
        },
        kw_min: {
          type: 'number',
          description: 'Minimum engine power in kW'
        },
        kw_max: {
          type: 'number',
          description: 'Maximum engine power in kW'
        },
        price_max: {
          type: 'number',
          description: 'Maximum price per day in CZK'
        }
      }
    }
  },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        end_date: {
          type: 'string',
          format: 'date',
          description: 'Return date (YYYY-MM-DD), inclusive'
        },
        extras: {
          type: 'array',
          items: {
          ...
          description: 'Optional extras [{name, unit_price}]'
        },
        promo_code: {
          type: 'string',
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        delivery_address: {
          type: 'string',
          description: 'Optional — delivery anywhere in CZ'
        },
        return_address: {
          type: 'string',
          description: 'Optional — return at different location'
        },
        extras: {
          type: 'array',
          items: {
...
const RESOURCES = [
  ...
  {
    uri: 'motogo://motorcycles',
    name: 'Full motorcycle catalog',
    mimeType: 'application/json'
  },
  {
    uri: 'motogo://branches',
    name: 'Rental branches',
    mimeType: 'application/json'
  },
  {
    uri: 'motogo://faq',
    name: 'FAQ — rental policies',
    mimeType: 'text/markdown'
  },
  {
    uri: 'motogo://policies',
    name: 'Rental policies summary',
    mimeType: 'text/markdown'
  }
];
// ============================================================================
...
async function execTool(name, args) {
  switch(name){
    case 'motogo_search_motorcycles':
      {
        let q = sb.from('motorcycles').select('id, model, brand, category, engine_cc, power_kw, license_required, color, image_url, branch_id, price_mon, price_tue, price_wed, price_thu, price_fri, price_sat, price_sun, ideal_usage, branches(is_open)').eq('status', 'active').order('model');
        ...
        let result = (data || []).filter((m)=>m.branches?.is_open !== false);
        if (args.price_max) {
    ...
    case 'motogo_get_motorcycle':
      {
        ...
        const { data, error } = await sb.from('motorcycles').select('*, branches(name, address, city, gps_lat, gps_lng, is_open)').eq('id', args.moto_id).maybeSingle();
        if (error) throw new Error(error.message);
        // Motorka na zavřené pobočce se navenek nenabízí → jako neexistující.
        if (!data || data.branches?.is_open === false) {
    ...
    case 'motogo_get_availability':
      {
        const { data, error } = await sb.rpc('get_moto_booked_dates', {
          p_moto_id: args.moto_id
        });
        if (error) throw new Error(error.message);
        return {
          moto_id: args.moto_id,
          booked_ranges: data || []
        };
      }
    case 'motogo_quote':
      {
        const { moto_id, start_date, end_date, extras = [], promo_code } = args;
        const { data: moto, error: mErr } = await sb.from('motorcycles').select('id, model, price_mon, price_tue, price_wed, price_thu, price_fri, price_sat, price_sun').eq('id', moto_id).maybeSingle();
        ...
        while(d <= end){
          ...
          const p = Number(moto['price_' + dn] ?? 0);
          total += p;
          count++;
          breakdown.push({
        ...
        const extrasTotal = Array.isArray(extras) ? extras.reduce((s, e)=>s + Number(e.unit_price || 0), 0) : 0;
    ...
    case 'motogo_create_booking':
      {
        ...
        const ssBranch = mb?.branches?.type === 'samoobslužná';
        const { data, error } = await sb.rpc('create_web_booking', {
          p_moto_id: a.moto_id,
          p_start_date: a.start_date,
          p_end_date: a.end_date,
          p_name: a.customer_name,
          p_email: a.customer_email,
          p_phone: a.customer_phone,
          p_street: a.street ?? '',
          p_city: a.city ?? '',
          p_zip: a.zip ?? '',
    ...
    case 'motogo_get_branches':
      {
        ...
        const branches = (data || []).filter((b)=>b.active !== false && b.is_open !== false);
        return {
    ...
    case 'motogo_get_faq':
      {
        ...
        const faqs = [
          {
            q: 'Jak si rezervovat motorku?',
            a: 'Online přes formulář na motogo24.cz/rezervace, nebo přes MCP nástroj motogo_create_booking. Telefon +420 774 256 271.'
          },
          {
            q: 'Musím složit kauci?',
            a: 'Ne — MotoGo24 půjčuje bez kauce. Žádná blokace peněz na kartě.'
          },
          {
            q: 'Co je v ceně?',
            a: 'Pronájem motorky, motorkářská výbava (helma, bunda, kalhoty, rukavice), povinné ručení.'
          },
          {
            q: 'Jaký řidičský průkaz potřebuji?',
            a: 'Skupina A pro silnější stroje, A2 do 35 kW, A1 do 11 kW. Dětské motorky bez ŘP (ručí zákonný zástupce).'
          },
          {
            q: 'Můžu jet do zahraničí?',
            a: 'Ano, sjezd do zahraničí povolen. Zelená karta v ceně.'
          },
          {
            q: 'Kde si motorku vyzvednu?',
            a: 'Mezná 9, Pelhřimov, Vysočina. Volitelně přistavení kamkoliv v ČR za příplatek.'
          },
          {
            q: 'Můžu storno?',
            a: 'Bezplatné storno minimálně 7 dní před převzetím. Později individuálně.'
          },
          {
            q: 'Jak platím?',
            a: 'Předem online přes Stripe: karta (Visa, Mastercard, Amex), Apple Pay, Google Pay; na webu i QR platba / bankovní převod. V mobilní aplikaci karta + Apple Pay (iPhone) / Google Pay (Android). Hotovost nepřijímáme.'
          },
          {
            q: 'Máte mobilní aplikaci?',
            a: 'Ano — iPhone: https://apps.apple.com/cz/app/id6806045151, Android: https://play.google.com/store/apps/details?id=com.motogo24.app'
          },
          {
            q: 'Otevírací doba?',
            a: '24/7 nonstop, 365 dní v roce. Vyzvednutí přes přístupové kódy.'
          },
          {
            q: 'Minimální věk?',
            a: '18 let pro dospělé motorky. Dětské motorky bez věkového limitu (ručí rodič).'
          }
        ];
    ...
    case 'motogo_validate_voucher':
      {
        const { data, error } = await sb.rpc('validate_voucher_code', {
          p_code: args.code
        });
        if (error) throw new Error(error.message);
        return data || {
...
Web: https://www.motogo24.cz`
  ...
  if (uri === 'motogo://motorcycles') {
    ...
    const motos = (data || []).filter((m)=>m.branches?.is_open !== false).map(({ branches: _b, ...m })=>m);
    return {
      uri,
      mimeType: 'application/json',
      text: JSON.stringify(motos)
    };
  }
  ...
  if (uri === 'motogo://faq') {
    const faqs = await execTool('motogo_get_faq', {});
    let md = '# MotoGo24 FAQ\n\n';
...
async function logCall(toolName, args, statusCode, latencyMs, outcome, ip, ua, partnerId) {
  try {
...
serve(async (req)=>{
  ...
  try {
    ...
    const msg = e.message;
    void logCall(params?.name || method, params?.arguments || {}, 500, Date.now() - startedAt, 'error', ip, ua, partnerId);
