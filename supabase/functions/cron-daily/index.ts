...
/** Ověří SERVICE_ROLE_KEY. */ function verifyServiceRole(authHeader) {
  if (!authHeader) return false;
  const serviceRoleKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '';
  return authHeader === `Bearer ${serviceRoleKey}`;
}
/** 1. Servisní alerty — motorky s next_service_date < NOW() + 14 dní. */ async function checkServiceAlerts(admin) {
  try {
    const fourteenDaysFromNow = new Date();
    fourteenDaysFromNow.setDate(fourteenDaysFromNow.getDate() + 14);
    const cutoffDate = fourteenDaysFromNow.toISOString().split('T')[0];
    const { data: motos, error } = await admin.from('motorcycles').select('id, model, license_plate, next_service_date, status').eq('status', 'active').lte('next_service_date', cutoffDate).order('next_service_date');
    if (error) throw error;
    for (const moto of motos ?? []){
      // Zkontroluj jestli alert pro tuto motorku neexistuje
      const { data: existingAlert } = await admin.from('notification_log').select('id').eq('template', 'service_alert').eq('metadata->>moto_id', moto.id).eq('status', 'pending').maybeSingle();
...
/** 2. Inventory check — zavolá inventory-check endpoint interně. */ async function runInventoryCheck(admin) {
  try {
...
/** 3. Daily stats snapshot — zavolá SQL funkci snapshot_daily_stats(). */ async function runDailyStatsSnapshot(admin) {
  try {
...
/** 4. Booking reminders — rezervace kde start_date = ZÍTRA. */ async function sendBookingReminders(admin) {
  try {
    ...
    const { data: bookings, error } = await admin.from('bookings').select(`
      ...
      `).eq('start_date', tomorrowStr).in('status', [
      'pending',
      'active'
    ]);
    if (error) throw error;
...
/** 5. Overdue invoices — faktury po splatnosti. */ async function checkOverdueInvoices(admin) {
  try {
    const today = new Date().toISOString().split('T')[0];
    const { data: overdue, error } = await admin.from('invoices').select('id').eq('status', 'issued').lt('due_date', today);
    if (error) throw error;
