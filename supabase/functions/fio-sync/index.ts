...
const fmtAmount = (n)=>(Math.round((Number(n) || 0) * 100) / 100).toFixed(2);
...
const colNum = (tx, key)=>Number(tx[key]?.value ?? 0);
...
function txDate(tx) {
  ...
  if (typeof v === 'number') return new Date(v).toLocaleDateString('sv-SE', {
...
async function debugLog(sb, action, status, data, err) {
  try {
...
async function sendOpsMail(subject, html) {
  try {
    ...
    console.warn('[fio-sync] ops mail failed:', e.message);
  }
}
serve(async (req)=>{
  ...
  if (!svcMatch && !roleMatch && bearer.length > 0) {
    ...
    appMatch = !!row && bearer === String(row.value ?? '').replace(/^"|"$/g, '');
  }
  ...
  if (!svcMatch && !roleMatch && !appMatch && !adminMatch) return json({
    success: false,
    error: 'forbidden'
  }, 403);
  if (!FIO_TOKEN) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      await debugLog(sb, 'fio_fetch_failed', 'error', {
        http_status: resp.status
      }, body.slice(0, 500));
      return json({
        success: false,
        error: `fio_http_${resp.status}`
      });
    }
    ...
    if (txs.length > 0) {
      ...
      if (rows.length > 0) {
        const { error: insErr } = await sb.from('fio_transactions').upsert(rows, {
          onConflict: 'fio_id',
          ignoreDuplicates: true
        });
        if (insErr) {
          await debugLog(sb, 'fio_tx_insert_failed', 'error', {
            count: rows.length
          }, insErr.message);
          return json({
            success: false,
    ...
    const { data: pending } = await sb.from('fio_transactions').select('fio_id, tx_date, amount, currency, vs, counter_account, counter_bank, counter_name, message, tx_type').eq('status', 'new').order('fio_id', {
      ascending: true
    }).limit(50);
    const summary = {
      fetched: txs.length,
    ...
    for (const p of pending || []){
      const finish = async (status, bookingId, note)=>{
        await sb.from('fio_transactions').update({
          status,
      ...
      try {
        ...
        let booking = null;
        if (vsDigits) {
          const { data } = await sb.from('bookings').select('id, status, payment_status, total_price, booking_source, user_id, moto_id, start_date, end_date, docs_completed_at, motorcycles!moto_id(model, manual_url), profiles(full_name, email)').eq('payment_vs', vsDigits).maybeSingle();
          booking = data;
        }
        // ── Match 2: doplatek za úpravu (bookings.mod_surcharge_vs) ──
        if (!booking && vsDigits) {
          const { data: sur } = await sb.from('bookings').select('id, mod_surcharge_due, mod_surcharge_vs').eq('mod_surcharge_vs', vsDigits).maybeSingle();
          if (sur) {
            const due = Number(sur.mod_surcharge_due) || 0;
            const surId = String(sur.id);
            if (due <= 0) {
            ...
            const { data: cs, error: csErr } = await sb.rpc('confirm_booking_surcharge', {
              ...
              p_vs: vsDigits,
              p_paid_date: p.tx_date,
              p_transaction_ref: txRef
            });
            if (csErr || cs?.success === false) {
              const msg = csErr?.message || cs?.error || 'rpc_failed';
              await finish('error', surId, 'confirm_booking_surcharge: ' + msg);
              summary.errors++;
              await debugLog(sb, 'confirm_surcharge_failed', 'error', {
                fio_id: p.fio_id,
                booking_id: surId
              }, msg);
              continue;
            ...
            summary.surcharge++;
            await debugLog(sb, 'surcharge_confirmed', 'ok', {
              fio_id: p.fio_id,
              booking_id: surId,
              amount: p.amount,
              vs: vsDigits
        ...
        if (!booking && vsDigits) {
          const { data: inv } = await sb.from('invoices').select('booking_id').eq('variable_symbol', vsDigits).in('type', [
            'advance',
            'proforma'
          ]).neq('status', 'cancelled').not('booking_id', 'is', null).order('created_at', {
            ascending: false
          }).limit(1);
          const invBookingId = inv?.[0]?.booking_id;
          if (invBookingId) {
            const { data } = await sb.from('bookings').select('id, status, payment_status, total_price, booking_source, user_id, moto_id, start_date, end_date, docs_completed_at, motorcycles!moto_id(model, manual_url), profiles(full_name, email)').eq('id', invBookingId).maybeSingle();
            booking = data;
          }
        }
        if (!booking) {
        ...
        const { data: cp, error: cpErr } = await sb.rpc('confirm_payment', {
          p_booking_id: bId,
          p_method: 'bank_transfer'
        });
        if (cpErr || cp?.success === false) {
          const msg = cpErr?.message || cp?.error || 'rpc_failed';
          await finish('error', bId, 'confirm_payment: ' + msg);
          summary.errors++;
          await debugLog(sb, 'confirm_payment_failed', 'error', {
            fio_id: p.fio_id,
            booking_id: bId
          }, msg);
          continue;
        }
        if (cp?.was_already_paid) {
          await finish('already_paid', bId, 'souběh — už potvrzeno jinou cestou');
          continue;
        }
        await debugLog(sb, 'confirm_payment', 'ok', {
          fio_id: p.fio_id,
          booking_id: bId,
          amount: p.amount,
          vs: vsDigits
        ...
        const invoiceErrors = [];
        try {
          const { data: existingInv } = await sb.from('invoices').select('id, type').eq('booking_id', bId).in('type', [
            'advance',
            'proforma',
            'payment_receipt'
          ]).neq('status', 'cancelled');
          const genInvoice = (body)=>fetch(SUPABASE_URL + '/functions/v1/generate-invoice', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                Authorization: 'Bearer ' + SERVICE_KEY,
          ...
          if (!(existingInv || []).some((i)=>i.type === 'advance' || i.type === 'proforma')) {
            const zf = await genInvoice({
              type: 'advance',
              booking_id: bId,
              source: 'booking',
              variable_symbol: vsDigits || undefined,
              send_email: false
            });
            if (!zf?.success) invoiceErrors.push('ZF: ' + (zf?.error || 'failed'));
          }
          if (!(existingInv || []).some((i)=>i.type === 'payment_receipt')) {
            const dp = await genInvoice({
              type: 'payment_receipt',
              booking_id: bId,
              variable_symbol: vsDigits || undefined,
              invoice_date: p.tx_date,
              send_email: false
            });
            if (!dp?.success) invoiceErrors.push('DP: ' + (dp?.error || 'failed'));
          }
        } catch (e) {
          invoiceErrors.push(e.message);
        }
        if (invoiceErrors.length > 0) {
          await debugLog(sb, 'invoice_generation_failed', 'warning', {
            fio_id: p.fio_id,
            booking_id: bId
          }, invoiceErrors.join('; '));
        }
        ...
        if (profile?.email) {
          ...
          try {
            ...
            if (!mr.ok) await debugLog(sb, 'confirmation_mail_http_error', 'error', {
              fio_id: p.fio_id,
              booking_id: bId,
              http_status: mr.status
            });
          } catch (e) {
            await debugLog(sb, 'confirmation_mail_failed', 'error', {
              fio_id: p.fio_id,
              booking_id: bId
            }, e.message);
          }
        ...
        try {
          ...
          const fmtCs = (s)=>s ? new Date(String(s)).toLocaleDateString('cs-CZ') : '';
          const msg = newStatus === 'active' ? `Motorka ${moto?.model || ''} byla vydána. Přejeme příjemnou jízdu! V případě problému nás kontaktujte nebo použijte SOS tlačítko.` : `Vaše rezervace motorky ${moto?.model || ''} (${fmtCs(booking.start_date)} – ${fmtCs(booking.end_date)}) byla potvrzena. Smlouvu a fakturu najdete v sekci Dokumenty.`;
          ...
          if (userId) {
            ...
            if (!thread) {
              const { data: nt } = await sb.from('message_threads').insert({
                customer_id: userId,
                subject: 'Rezervace',
                channel: 'app'
        ...
        try {
          await sb.from('admin_audit_log').insert({
            admin_id: null,
            action: 'booking_payment_confirmed_fio',
            entity_type: 'booking',
            entity_id: bId,
            new_data: {
              fio_id: p.fio_id,
        ...
        summary.errors++;
        await debugLog(sb, 'tx_processing_failed', 'error', {
          fio_id: p.fio_id
        }, e.message);
        try {
          await sb.from('fio_transactions').update({
    ...
    await debugLog(sb, 'sync_failed', 'error', {}, err.message);
    return json({
      success: false,
      error: err.message
    }, 500);
...
async function notifyConfirmed(p, bookingId, expected, isSurcharge, overpay) {
  const bn = '#' + bookingId.slice(-8).toUpperCase();
  const what = isSurcharge ? 'DOPLATEK' : 'platba';
  await sendOpsMail(`✅ Fio: ${what} VS ${p.vs || '—'} — ${fmtAmount(p.amount)} Kč potvrzena automaticky (rez. ${bn})`, `<h2 style="margin:0 0 8px;font-size:18px;color:#0f1a14">✅ ${isSurcharge ? 'Doplatek potvrzen' : 'Platba potvrzena'} automaticky (Fio API)</h2>
...
async function notifyMismatch(p, bookingId, expected, what) {
  const bn = '#' + bookingId.slice(-8).toUpperCase();
  await sendOpsMail(`⚠️ Fio: platba VS ${p.vs || '—'} NESEDÍ částkou (${fmtAmount(p.amount)} / ${fmtAmount(expected)} Kč, rez. ${bn})`, `<h2 style="margin:0 0 8px;font-size:18px;color:#0f1a14">⚠️ Platba nesedí částkou — potvrďte ručně</h2>
...
async function notifyUnmatched(p, reason) {
  await sendOpsMail(`🔔 Fio: nespárovaná příchozí platba ${fmtAmount(p.amount)} ${p.currency || 'CZK'} (VS ${p.vs || '—'})`, `<h2 style="margin:0 0 8px;font-size:18px;color:#0f1a14">🔔 Nespárovaná příchozí platba na Fio účtu</h2>
