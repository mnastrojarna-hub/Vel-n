-- =============================================================================
-- MIGRACE: create_web_booking — ukládat datum narození (date_of_birth) z kroku 1
-- Datum: 2026-05-30
-- Branch: claude/customer-info-storage-bug-QGk6Y
--
-- Web rezervace nově sbírá „Datum narození" jako POVINNÉ pole v kroku 1
-- (kontaktní údaje), stejně jako appka. Krok 1 jako jediný spolehlivě ukládá do
-- profilu (přes tuto SECURITY DEFINER RPC, obchází RLS), proto DOB vedeme tudy —
-- uloží se i u nepřihlášeného nového web zákazníka. Věk se ve Velíně dopočítá.
--
-- Změna: 1 nový parametr `p_date_of_birth text DEFAULT NULL` na KONEC signatury
-- (37 → 38 arg) → nejdřív DROP staré 37-arg verze, pak recreate. Profile
-- INSERT/upsert (obě větve — existující i nový auth user) nově zapisuje
-- `date_of_birth = COALESCE(v_dob, profiles.date_of_birth)` (prázdné/neplatné
-- datum nepřepíše existující — důležité pro QR resume/„Zpět", kde se DOB
-- znovu neposílá).
-- =============================================================================
...
BEGIN
  ...
  -- datum narození → date (neplatný formát ignorujeme, nepřepíše existující)
