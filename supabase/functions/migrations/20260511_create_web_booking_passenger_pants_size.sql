-- =============================================================================
-- MIGRACE: create_web_booking — přidat p_passenger_pants_size (velikost kalhot spolujezdce)
-- Datum: 2026-05-11
-- Branch: claude/filter-motorcycle-sizes-Gth9s
--
-- Reportováno: na webu /rezervace v kroku 5 měla karta „Vybavení pro spolujezdce"
-- jen helmu / bundu / rukavice — chyběly kalhoty. Frontend (pages-rezervace.js +
-- pages-rezervace-steps.js) doplněn o size-row „pants" v passenger panelu a
-- _submitReservation nově posílá p_passenger_pants_size. Sloupec
-- bookings.passenger_pants_size už existoval (Velín BookingSummary.jsx ho i
-- zobrazoval), jen ho create_web_booking nezapisovalo.
--
-- Změna: nový parametr p_passenger_pants_size text DEFAULT NULL přidán na KONEC
-- signatury (za p_existing_booking_id) → žádný posun pozic, PostgREST volá podle
-- jmen. CREATE OR REPLACE by jinak vytvořil overload (32 vs 33 args), proto se
-- nejdřív DROPne stará 32-arg verze.
-- =============================================================================
