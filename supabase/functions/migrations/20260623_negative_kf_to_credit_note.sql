-- =============================================================================
-- MIGRACE 2026-06-23 (#3): Záporná KF → dobropis (přeplatek)
--
-- Kontext (bug report 2026-06-23):
--   KF-2026-0014 = −1 168 Kč. Konečná faktura nemá být záporná — vzniká, když
--   záloha (DP) převýší finální cenu (typicky sleva/test po složení zálohy) a
--   žádný refund (dobropis) neproběhl. Správně má jít o DOBROPIS, ne zápornou KF.
--
-- Řešení (staví na živé verzi z 20260618_late_pickup_discount_kf.sql —
--   loyalty i late-pickup řádky ZACHOVÁNY):
--   1) KF číslo nově přes next_document_number('KF') (atomické, viz #11).
--   2) Pokud po odečtu DP + dobropisů vyjde KF < 0, vystaví se credit_note (DB)
--      na přeplatek a KF se dorovná na 0 (místo záporné konečné faktury).
--      Pozn.: vystaví jen DOKLAD — žádný Stripe refund/mail (drží mantinely).
--   Týká se jen BUDOUCÍCH dokončení (trigger skipuje rezervace, které už KF mají).
-- =============================================================================
