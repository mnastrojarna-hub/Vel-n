-- =============================================================================
-- MIGRACE: create_web_booking — UPSERT profilu místo UPDATE (fix FK violation)
-- Datum: 2026-05-07
-- Reportováno: Při vytváření rezervace na webu hodilo
--   "insert or update on table \"bookings\" violates foreign key constraint
--    \"bookings_user_id_fkey\""
--
-- Root cause:
--   Když existoval řádek v `auth.users` (např. starý signup přes Flutter app
--   nebo edge funkci, která profil neuložila), ale chyběl odpovídající řádek
--   v `profiles`, RPC udělala jen `UPDATE profiles WHERE id=v_user_id`. UPDATE
--   match 0 řádků (no-op), funkce pokračovala dál a `INSERT INTO bookings`
--   selhal na FK `bookings.user_id → profiles.id`.
--
--   Diagnostika v DB potvrdila desítky osiřelých auth.users (převážně
--   `test.sim.*` testovací účty z AI tréninku + 3 reálné maily). Profily se
--   pro ně **nedoplňují** (pravidlo admin), pouze se opraví future flow.
--
-- Fix:
--   V existing-user větvi `INSERT INTO profiles ... ON CONFLICT (id) DO UPDATE`
--   místo pouhého UPDATE. Pokud profil chybí, dotvoří se ze submitnutých dat
--   rezervačního formuláře. Pokud existuje, aktualizuje se (jen prázdná pole,
--   stejně jako předtím).
--
-- Vedlejší úprava:
--   - `v_existing_auth record` → `v_existing_auth_id uuid` (přímý uuid scalar
--     je čistší než `record` s jednou kolonkou).
--   - `$$` → `$func$` delimiter (řešilo problém parsování v některých SQL
--     editorech, viz interní logika přepisu během 2026-05-07).
--
-- Bez změny signatury — 32 parametrů zůstává shodných s 20260428 verzí.
-- =============================================================================
...
BEGIN
  ...
  -- 3) DEDUPLIKACE ZÁKAZNÍKA + UPSERT profilu (FIX 2026-05-07)
  ...
  IF v_existing_auth_id IS NOT NULL THEN
    ...
    -- Pokud profil chybí (osiřelý auth.users), dotvoří se. Jinak update prázdných polí.
