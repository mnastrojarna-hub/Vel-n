-- =============================================================================
-- MIGRACE: Block anon booking se známým e-mailem + lookup RPC pro real-time check
-- Datum: 2026-05-09
-- Branch: claude/fix-booking-confirmation-validation-ZeAT2
--
-- Reportováno (česká reprodukce uživatelem): zákaznice (Petra) měla v profilu
-- ověřené doklady (id_number + license_number z předchozího Mindee scanu).
-- Někdo (Jaroslava S.) si na webu /rezervace zadal její e-mail
-- (semoradova.petra@seznam.cz), přeskočil sken dokladů a confirmation page
-- zobrazila „Doklady ✓ Ověřeny", přestože tato osoba nikdy doklady nenahrála
-- — RPC `get_web_booking_confirmation` totiž čte `docs_status` z profilu, nikoli
-- z konkrétní rezervace. Trigger `auto_generate_door_codes` by stejně tak
-- odeslal 6místné kódy bez dalšího ověření.
--
-- Root cause:
--   `create_web_booking` deduplikoval zákazníka **pouze podle lower(trim(email))**
--   proti `auth.users.email`. Když mail existoval, anon volání tiše napojilo
--   novou rezervaci na cizí profil.
--
-- Fix (kombinace UI + DB):
--   1) Nová RPC `check_web_booking_email(text)` → real-time check při blur na
--      poli e-mail v kroku 3 webu. Vrací jen `{exists: bool}` (bez profilových
--      dat). Frontend pak zobrazí inline hint „tento mail u nás už máme,
--      přihlaste se" se CTA, který otevře login panel (modul
--      `motogo-web-php/js/pages-rezervace-auth.js`).
--   2) `create_web_booking` v anon kontextu (`current_user='anon'`) odmítne
--      vytvoření, pokud `auth.users.email` existuje a NENÍ to resume
--      (`p_existing_booking_id IS NULL`). Vrací `{error:'email_exists',
--      message:'…'}`. Web (`pages-rezervace-steps.js#_submitReservation`) to
--      zachytí a aktivuje login panel s předvyplněným mailem.
--   3) `create_web_booking` má navíc bezpečnostní pojistku v authenticated
--      kontextu: pokud `auth.uid() <> nalezený user_id` (přihlášený zákazník
--      pod jiným účtem než zadal e-mail), vrátí `{error:'email_mismatch'}`.
--      Service_role (AI agent / edge fn) projde beze změny — `current_user`
--      ani `auth.uid()` nejsou `'anon'`.
--
-- Bez změny signatury `create_web_booking` (32 parametrů beze změn).
-- =============================================================================

-- ---------- 1) check_web_booking_email -------------------------------------
CREATE OR REPLACE FUNCTION public.check_web_booking_email(p_email text)
RETURNS jsonb
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.check_web_booking_email(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.check_web_booking_email(text) TO anon, authenticated;

-- ---------- 2) create_web_booking — anon gate + cross-account guard --------
...
BEGIN
  ...
  -- 3a) ★ NEW: anon + známý mail + ne-resume → blok
  -- Service_role (AI agent / edge fn) i authenticated session (přihlášený přes
  -- login panel) projdou. Anon (nepřihlášený zákazník) ne — má použít login
  -- panel v kroku 3 nebo obnovu hesla.
  ...
  -- 3b) ★ NEW: cross-account guard. Authenticated session pod jiným uživatelem,
  -- než kterému patří zadaný e-mail → odmítnout (zákazník je přihlášen na účet
  -- A, ale do formuláře zadá e-mail účtu B — bez tohoto by se rezervace zapsala
  -- pod účet B, ale auth.uid() bude účet A → potenciální cross-account zápis).
  IF v_existing_auth_id IS NOT NULL
