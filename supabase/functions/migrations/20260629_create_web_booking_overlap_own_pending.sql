-- =============================================================================
-- MIGRACE 2026-06-29: FIX — create_web_booking overlap ignoruje VLASTNÍ
--   rozpracovaný (pending+unpaid web) draft zákazníka.
--
-- Problém: web rezervace hlásila „Booking overlap — motorka není v termínu
--   dostupná" při kliknutí na „Pokračovat", přestože motorka je volná, a po
--   několika pokusech to prošlo. Příčina: blok „4) OVERLAP" počítal jako
--   kolizi i vlastní rozpracované pending+unpaid web drafty zákazníka
--   (z dvojkliku / abandonované předchozí session). Reuse přes
--   `p_existing_booking_id` vyloučil jen JEDEN konkrétní řádek, ostatní
--   vlastní drafty blokovaly. (Frontend `_submitReservation` zároveň neměl
--   ochranu proti dvojímu odeslání — řešeno samostatně na webu.)
--
-- Řešení: do EXISTS podmínky přidán `AND NOT (user_id = v_user_id AND
--   status='pending' AND payment_status='unpaid' AND booking_source='web')`.
--   Vlastní nedokončený web draft tedy zákazníka neblokuje (auto-cancel
--   `auto_cancel_expired_pending()` ho stejně uvolní); cizí zákazník (i jeho
--   pending) a potvrzené/aktivní rezervace blokují dál beze změny.
--
-- Tělo je 1:1 převzaté z `20260626_create_web_booking_license_groups.sql`,
--   změněn JEN blok „4) OVERLAP". Signatura, GRANT, ŘP kontrola, sleva,
--   upsert profilu i INSERT/UPDATE bookingu beze změny.
-- =============================================================================
...
BEGIN
  ...
  -- 4) OVERLAP  ← ZMĚNĚNO 2026-06-29: vlastní rozpracovaný (pending+unpaid web)
  --              draft zákazníka už neblokuje (auto-cancel ho uvolní); cizí
  --              zákazník i potvrzené/aktivní rezervace blokují dál.
  IF EXISTS (
