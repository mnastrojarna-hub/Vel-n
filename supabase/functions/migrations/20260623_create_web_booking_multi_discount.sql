-- =============================================================================
-- MIGRACE 2026-06-23 (D): FIX — create_web_booking: VÍCE slevových kódů/voucherů/%
--   se nyní odečte VŠECHNY (dřív jen jeden promo + jeden voucher do discount_amount).
--
-- Problém: v DB byly DVĚ create_web_booking — 26-arg (stará, ze 20260329) a
-- 39-arg (produkční, kterou web reálně volá: posílá 31 pojmenovaných parametrů
-- vč. p_consent_*, p_passenger_*, p_existing_booking_id, p_return_time,
-- p_trailer_moto_id, p_date_of_birth). Migrace 20260618 omylem přidala/přepsala
-- jen 26-arg verzi (mrtvý kód — web ji nikdy nevolá), takže sleva na 1. den se
-- u web rezervací NEAPLIKOVALA. Drop 26-arg/39-arg navíc rozbil web.
--
-- Řešení: KANONICKÁ je 39-arg verze (superset). Sem doplníme late slevu
-- (Model B — late PŘED procentem) a 26-arg mrtvou verzi zahodíme.
-- Tělo 39-arg verze je převzaté ze supabase-live-snapshot (schema_public.sql).
-- =============================================================================

-- Zahoď mrtvou 26-arg verzi (web ji nevolá; superset 39-arg pokryje vše)
DROP FUNCTION IF EXISTS public.create_web_booking(
  uuid, timestamp with time zone, timestamp with time zone, text, text, text,
  text, text, text, text, text, time without time zone, text, text, jsonb,
  numeric, text, text, uuid, text, text, text, text, text, text, text);

-- Kanonická 39-arg verze + late sleva (Model B)
