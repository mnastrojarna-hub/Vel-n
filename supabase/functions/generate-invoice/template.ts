...
function statusBadge(p) {
  if (p.isProforma) return p.isPaid ? {
    label: 'UHRAZENO',
    tone: 'paid'
  } : {
    label: 'K ÚHRADĚ',
...
<div class="inv-wrap" style="max-width:780px;margin:0 auto;background:#ffffff">
      ...
      <tr>
        ...
        <td style="vertical-align:top;width:50%">
          ...
          <table role="presentation" cellpadding="0" cellspacing="0" style="font-size:11.5px;line-height:1.5">
            ...
            ${p.customer.company_address ? `<tr><td style="color:#16a34a;padding-right:14px;vertical-align:top">Sídlo</td><td style="color:#0f1a14;font-weight:700">${p.customer.company_address}</td></tr>` : ''}
  ...
  ${dc.length > 0 ? `<div style="margin:0 32px 8px;padding:10px;background:#e0f2fe;border-radius:6px;border:1px solid #0284c7">
    ...
    ${dc.filter((c)=>!c.withheld_reason).map((d)=>`<div style="font-size:12px;font-weight:700;color:#0c4a6e;padding:2px 0">${d.code_type === 'motorcycle' ? 'Kód k motorce' : 'Kód šatny'}: <span style="font-size:16px;letter-spacing:3px;color:#0369a1;font-family:'Courier New',monospace">${d.door_code}</span></div>`).join('')}
    ${dc.some((c)=>c.withheld_reason) ? '<div style="font-size:11px;font-weight:600;color:#b45309;margin-top:4px">Kódy budou zaslány po ověření dokladů (OP/pas/ŘP).</div>' : `<div style="font-size:10px;color:#164e63;margin-top:4px">${dc.filter((c)=>!c.withheld_reason).length > 1 ? 'Kódy jsou platné' : 'Kód je platný'} pouze po dobu trvání pronájmu.</div>`}
  </div>` : ''}
