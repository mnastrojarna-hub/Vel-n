...
async function logDb(status, errorMessage, data, durationMs) {
  try {
...
function partNote(index, total, fromPage, toPage) {
...
async function claudeConvert(apiKey, pdfB64, note) {
  let lastErr = '';
  for(let attempt = 1; attempt <= 3; attempt++){
          ...
          tools: [
            {
              ...
              input_schema: {
                ...
                properties: {
                  html: {
                    type: 'string',
                    description: 'Document body as clean semantic HTML, Czech text preserved verbatim.'
                  }
                },
                required: [
                  'html'
    ...
    const toolUse = Array.isArray(data?.content) ? data.content.find((c)=>c?.type === 'tool_use' && c?.name === 'submit_html') : null;
...
Deno.serve(async (req)=>{
  ...
  if (!auth.ok) return jsonResponse({
    success: false,
    error: 'unauthorized',
    reason: auth.reason
  }, 401);
  ...
  if (!pdf) return jsonResponse({
    success: false,
    error: 'missing_pdf'
  }, 400);
  if (pdf.length > MAX_BASE64_CHARS) return jsonResponse({
    success: false,
    error: 'PDF je příliš velké (max 10 MB).'
  }, 400);
  ...
  try {
    ...
    let srcDoc = null;
    try {
      srcDoc = await PDFDocument.load(base64ToBytes(pdf), {
    ...
    if (pageCount !== null && pageCount > MAX_PAGES) {
      await logDb('error', `too_many_pages: ${pageCount}`, {
        pages: pageCount,
        b64_len: pdf.length
      }, performance.now() - t0);
      return jsonResponse({
        success: false,
        error: `PDF má ${pageCount} stran — maximum je ${MAX_PAGES}. Rozdělte dokument.`
      }, 400);
    }
    let html;
    let chunks = 1;
    if (srcDoc === null || pageCount === null || pageCount <= CHUNK_PAGES) {
      ...
      for(let start = 0; start < pageCount; start += CHUNK_PAGES){
        ranges.push([
          start,
          Math.min(start + CHUNK_PAGES, pageCount) - 1
        ]);
      }
      ...
      const parts = await mapLimit(ranges, CHUNK_CONCURRENCY, async ([from, to], i)=>{
        ...
        const pages = await part.copyPages(srcDoc, indices);
