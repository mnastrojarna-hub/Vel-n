...
async function applyExtensionChange(supabase, bookingId, chgStr, paidCzk = null) {
  if (!chgStr) return;
  let a;
  try {
    a = JSON.parse(chgStr);
  ...
  let cur = null;
  try {
    const { data } = await supabase.from('bookings').select('moto_id, start_date, end_date, pickup_time, total_price, late_pickup_discount_amount, original_start_date, modification_history').eq('id', bookingId).maybeSingle();
    cur = data || null;
  } catch  {
    cur = null;
  }
  const stale = !!(base && typeof base === 'object' && cur && (base.s && day(base.s) !== day(cur.start_date) || base.e && day(base.e) !== day(cur.end_date) || base.t !== undefined && hm(base.t) !== hm(cur.pickup_time) || base.p != null && Math.round(Number(base.p)) !== Math.round(Number(cur.total_price || 0))));
  if (stale && cur) {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      if (session.mode === 'setup' && metadata.action === 'add_card') {
        ...
        if (paymentType === 'extension') {
          try {
            await applyExtensionChange(supabase, resolvedBookingId, metadata.chg, !metadata.shop_order_id && session.amount_total != null ? session.amount_total / 100 : null);
          } catch (e) {
      ...
      if ((paymentType === 'booking' || paymentType === 'extension') && resolvedBookingId) {
        ...
        if (paymentType === 'extension') {
          try {
            await applyExtensionChange(supabase, resolvedBookingId, metadata.chg, Number.isFinite(paymentIntent.amount) ? paymentIntent.amount / 100 : null);
          } catch (e) {
