...
export async function execReadHR(name, input, sb) {
  switch(name){
    case 'get_employees':
      {
    ...
    case 'get_employee_detail':
      {
        const id = input.employee_id;
        if (!id) return {
          error: 'employee_id je povinný'
        };
        const [empR, attR, vacR, shiftR, docR, payR] = await Promise.all([
          sb.from('acc_employees').select('*').eq('id', id).single(),
          sb.from('emp_attendance').select('*').eq('employee_id', id).order('date', {
            ascending: false
          }).limit(30),
          sb.from('emp_vacations').select('*').eq('employee_id', id).order('start_date', {
            ascending: false
          }).limit(10),
          sb.from('emp_shifts').select('*').eq('employee_id', id).order('date', {
            ascending: false
          }).limit(30),
          sb.from('emp_documents').select('*').eq('employee_id', id).order('created_at', {
            ascending: false
          }),
          sb.from('acc_payrolls').select('*').eq('employee_id', id).order('period', {
            ascending: false
          }).limit(12)
        ]);
        if (!empR.data) return {
    ...
    case 'get_attendance_overview':
      {
        ...
        const { data } = await sb.from('emp_attendance').select('*, acc_employees(name, position)').gte('date', since).order('date', {
          ascending: false
        });
        const byStatus = {};
        for (const a of data || []){
    ...
    case 'get_payrolls':
      {
        ...
        const { data } = await sb.from('acc_payrolls').select('*, acc_employees(name)').order('period', {
          ascending: false
        }).limit(limit);
        return {
          payrolls: data || [],
