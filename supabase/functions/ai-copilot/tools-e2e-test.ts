...
export async function execE2ETest(name, input, sb) {
  const now = new Date();
  switch(name){
    // === CREATE TEST USER ===
    ...
    case 'cleanup_test_data':
      {
        ...
        if (testProfiles && testProfiles.length > 0) {
          for (const p of testProfiles){
            ...
            await sb.from('messages').delete().in('thread_id', ((await sb.from('message_threads').select('id').eq('customer_id', p.id)).data || []).map((t)=>t.id));
        ...
        return {
          status: 'cleaned',
          ...results,
          summary: `Smazáno: ${results.users} uživatelů, ${results.bookings} bookings, ${results.sos} SOS, ${results.promos} promo kódů`
        };
      }
    ...
    case 'verify_app_consistency':
      {
        ...
        if (noPriceMotos?.length) issues.push({
          area: 'pricing',
          severity: 'high',
          issue: 'Aktivní motorky bez ceny',
          count: noPriceMotos.length,
          details: noPriceMotos
        });
        ...
        if (noBranchMotos?.length) issues.push({
          area: 'fleet',
          severity: 'high',
          issue: 'Aktivní motorky bez pobočky',
          count: noBranchMotos.length,
          details: noBranchMotos
        });
        ...
        if (noImageMotos?.length) issues.push({
          area: 'content',
          severity: 'medium',
          issue: 'Motorky bez fotky',
          count: noImageMotos.length
        });
        // 4. Branches must have accessories
        const { data: branches } = await sb.from('branches').select('id, name');
        for (const br of branches || []){
          const { count } = await sb.from('branch_accessories').select('id', {
            count: 'exact',
            head: true
          }).eq('branch_id', br.id);
          if ((count || 0) === 0) issues.push({
            area: 'branch',
            severity: 'medium',
            issue: `Pobočka "${br.name}" nemá příslušenství`,
            branch_id: br.id
          });
        }
        ...
        for (const slug of required_templates){
          if (!slugs.has(slug)) issues.push({
            area: 'templates',
            severity: 'high',
            issue: `Chybí email šablona: ${slug}`
          });
        }
        // 6. Document templates
        const { data: docTemplates } = await sb.from('document_templates').select('type, active');
        const activeTypes = new Set((docTemplates || []).filter((t)=>t.active).map((t)=>t.type));
        for (const t of [
          'rental_contract',
          'vop',
          'handover_protocol'
        ]){
          if (!activeTypes.has(t)) issues.push({
            area: 'templates',
            severity: 'medium',
            issue: `Chybí aktivní šablona: ${t}`
          });
        }
        // 7. App settings
        ...
        for (const k of [
          ...
          if (!keys.has(k)) issues.push({
            area: 'settings',
            severity: 'high',
            issue: `Chybí app_setting: ${k}`
          });
        }
        ...
        if (noImgProducts?.length) issues.push({
          area: 'eshop',
          severity: 'low',
          issue: 'Aktivní produkty bez fotek',
          count: noImgProducts.length
        });
    ...
    case 'generate_e2e_report':
      {
        ...
        const readyMotos = (activeMotos || []).filter((m)=>m.price_weekday && m.image_url && m.branch_id);
        report.sections.push({
          ...
          issues: (activeMotos || []).filter((m)=>!m.price_weekday || !m.image_url || !m.branch_id).map((m)=>`${m.model}: ${!m.price_weekday ? 'bez ceny' : ''} ${!m.image_url ? 'bez fotky' : ''} ${!m.branch_id ? 'bez pobočky' : ''}`)
        });
        // Active promo codes
        const { data: promos } = await sb.from('promo_codes').select('code, type, value, active, valid_to').eq('active', true);
        const expired = (promos || []).filter((p)=>p.valid_to && p.valid_to < now.toISOString().slice(0, 10));
        report.sections.push({
          name: 'Promo kódy',
          status: expired.length === 0 ? 'ok' : 'warn',
          details: `${(promos || []).length} aktivních, ${expired.length} expirovaných (ale stále active)`,
          issues: expired.map((p)=>`${p.code} expiroval ${p.valid_to} ale je stále active=true`)
        });
        // Open branches
        const { data: openBranches } = await sb.from('branches').select('id, name, is_open');
        const closed = (openBranches || []).filter((b)=>!b.is_open);
        report.sections.push({
          name: 'Pobočky',
        ...
        report.sections.push({
          ...
          issues: outOfStock.map((p)=>`${p.name}: stock=0 ale stále aktivní`)
        });
        ...
        report.summary = {
          ...
          pass: report.sections.length - failCount - warnCount,
          fail: failCount,
          warn: warnCount
        };
