...
serve(async (req)=>{
  ...
  try {
    ...
    if (mode === 'execute' && Array.isArray(actions)) {
      ...
      try {
        await supabaseAdmin.from('ai_actions').insert(results.map((r)=>({
            admin_id: user.id,
            tool: r.tool,
            input: actions.find((a)=>a.tool === r.tool)?.input,
            result: r.result,
            success: r.success,
      ...
      return jsonResponse({
        ...
        response: `Provedeno ${results.filter((r)=>r.success).length}/${results.length} akcí.`
      });
    }
    // MODE: CHAT — normal AI conversation
    if (!message || typeof message !== 'string') return jsonResponse({
    ...
    if (conversation_history && Array.isArray(conversation_history)) {
      convMsgs = conversation_history.filter((m)=>m.role === 'user' || m.role === 'assistant').map((m)=>({
          role: m.role,
          content: m.content
        }));
      ...
      if (conv?.messages && Array.isArray(conv.messages)) {
        convMsgs = conv.messages.filter((m)=>m.role === 'user' || m.role === 'assistant').map((m)=>({
            role: m.role,
            content: m.content
          }));
      }
    ...
    // deno-lint-ignore no-explicit-any
    const rawMsgs = [];
    for (const m of convMsgs)rawMsgs.push({
      role: m.role,
      content: m.content
    });
    rawMsgs.push({
    ...
    const apiMessages = rawMsgs[0]?.role !== 'user' ? [
      {
        role: 'user',
        content: '(kontext)'
      },
    ...
    for(let i = 0; i < MAX_ITER; i++){
      if (Date.now() - t0 > TIMEOUT) return jsonResponse({
        response: 'Zpracování trvá příliš dlouho. Zkuste jednodušší dotaz.'
      });
      ...
      if (!res.ok) {
        ...
        if (res.status === 529 || res.status === 503) return jsonResponse({
          response: 'AI služba je přetížená.',
      ...
      if (ai.stop_reason === 'end_turn') {
        ...
        if (match) {
          try {
            ...
            parsedActions = (parsed.actions || []).map((a)=>({
                ...a,
                id: crypto.randomUUID(),
                risk: TOOL_RISK[a.tool] || 'medium'
              }));
          } catch  {}
        }
        ...
        return jsonResponse({
          response: cleanText || 'Odpověď nedostupná.',
          pending_actions: allPending.length > 0 ? allPending : undefined
        });
      }
      if (ai.stop_reason === 'tool_use') {
        ...
        const results = [];
        // deno-lint-ignore no-explicit-any
        for (const tc of ai.content.filter((b)=>b.type === 'tool_use')){
          ...
          if (isDry && data?.status === 'preview') {
            pendingActions.push({
              id: crypto.randomUUID(),
              tool: tc.name,
              input: tc.input || {},
              summary: data.summary || tc.name,
    ...
    return jsonResponse({
      response: 'Dotaz je příliš složitý. Zkuste ho zjednodušit.'
    });
  } catch (err) {
