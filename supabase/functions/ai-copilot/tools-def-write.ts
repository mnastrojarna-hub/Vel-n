...
export const WRITE_TOOLS_DEFINITION = [
  ...
  // === FLEET AGENT ===
  { name: 'update_motorcycle', description: 'Úprava motorky (stav, nájezd, pobočka, údaje). POZOR: přesun na SAMOOBSLUŽNOU pobočku (branch_id) zneplatní každou živou rezervaci té motorky, která veze vozík — samoobsluha vozík nevydává. Před přesunem to operátorovi řekni a nech si to potvrdit.', input_schema: { type: T, properties: { motorcycle_id: { ...S, description: 'UUID' }, status: S, mileage: N, branch_id: S, unavailable_reason: S, notes: S }, required: ['motorcycle_id'] } },
  ...
  { name: 'update_branch', description: 'Úprava pobočky (název, otevřeno/zavřeno, typ). POZOR: přepnutí type na \'samoobslužná\' zpětně zneplatní každou živou rezervaci na motorkách té pobočky, která veze vozík. Před přepnutím to operátorovi řekni a nech si to potvrdit.', input_schema: { type: T, properties: { branch_id: { ...S, description: 'UUID' }, name: S, is_open: B, type: S }, required: ['branch_id'] } },
