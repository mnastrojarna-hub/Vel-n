...
export async function execWriteOps(name, input, sb, dryRun) {
  switch(name){
    // === FINANCE ===
    ...
    case 'match_delivery_note':
      {
        ...
        const [r1, r2] = await Promise.all([
          sb.from('delivery_notes').update({
            ...
            match_method: 'ai',
            match_confidence: confidence || 1,
            matched_at: now
          }).eq('id', delivery_note_id),
          sb.from('invoices').update({
            matched_delivery_note_id: delivery_note_id
          }).eq('id', invoice_id)
        ]);
        if (r1.error) return {
    ...
    case 'create_service_order':
      {
        ...
        const { data, error } = await sb.from('service_orders').insert({
          moto_id,
          description,
          status: st || 'pending',
          created_at: new Date().toISOString()
    ...
    case 'update_service_order':
      {
        ...
        if (!order) return {
          error: 'Servisní objednávka nenalezena'
        };
    ...
    case 'create_attendance':
      {
        ...
        const { error } = await sb.from('emp_attendance').upsert({
          employee_id,
          date,
          check_in,
          check_out,
    ...
    case 'manage_vacation':
      {
        ...
        if (action === 'create') {
          ...
          const { error } = await sb.from('emp_vacations').insert({
            employee_id,
            start_date,
            end_date,
            days,
        ...
        if (action === 'approve' || action === 'reject') {
          const vacId = input.vacation_id;
          if (!vacId) return {
            error: 'vacation_id je povinný pro approve/reject'
          };
          const { data: vac } = await sb.from('emp_vacations').select('id, status, employee_id').eq('id', vacId).single();
          if (!vac) return {
            error: 'Dovolená nenalezena'
          };
          ...
          const { error } = await sb.from('emp_vacations').update({
    ...
    case 'manage_shifts':
      {
        ...
        const { error } = await sb.from('emp_shifts').upsert({
          employee_id,
          date,
          shift_type,
          branch_id
    ...
    case 'create_purchase_order':
      {
        ...
        const { data: supplier } = supplier_id ? await sb.from('suppliers').select('id, name').eq('id', supplier_id).single() : {
          data: null
        };
        ...
        if (items && Array.isArray(items)) {
          for (const item of items)await sb.from('purchase_order_items').insert({
            order_id: po.id,
            ...item
          });
        }
    ...
    case 'create_inventory_movement':
      {
        ...
        if (!item) return {
          error: 'Skladová položka nenalezena'
        };
    ...
    case 'update_feature_flag':
      {
        ...
        if (!flagId) {
          const { data: byName } = await sb.from('feature_flags').select('id').eq('name', flag_name).limit(1);
          flagId = byName?.[0]?.id;
        }
        if (!flagId) return {
          error: `Feature flag "${flag_name}" nenalezen`
        };
    ...
    case 'generate_test_report':
      {
        ...
        if (scope === 'all' || scope === 'bookings') {
          const { count: orphaned } = await sb.from('bookings').select('id', {
            count: 'exact',
            head: true
          }).is('user_id', null);
          const { count: noMoto } = await sb.from('bookings').select('id', {
            count: 'exact',
            head: true
          }).is('moto_id', null);
          checks.push({
            area: 'bookings',
            orphaned_bookings: orphaned || 0,
            no_moto_bookings: noMoto || 0
          });
        }
        ...
        if (scope === 'all' || scope === 'finance') {
          ...
          checks.push({
            area: 'finance',
            proforma_awaiting_payment: unpaid || 0
          });
        }
        return {
          status: 'executed',
    ...
    case 'check_data_integrity':
      {
        ...
        if (table === 'all' || table === 'bookings') {
          const { data: dups } = await sb.from('bookings').select('id, user_id, moto_id, start_date, end_date, status').in('status', [
            'active',
            'reserved'
          ]).order('moto_id');
          const seen = {};
          for (const b of dups || []){
            const k = `${b.moto_id}_${b.start_date}`;
            if (seen[k]) issues.push({
              type: 'overlap',
              table: 'bookings',
              ids: [
                seen[k],
        ...
        if (table === 'all' || table === 'profiles') {
          const { count: noName } = await sb.from('profiles').select('id', {
            count: 'exact',
            head: true
          }).is('full_name', null);
          if ((noName || 0) > 0) issues.push({
            type: 'missing_data',
            table: 'profiles',
            field: 'full_name',
            count: noName
        ...
        return {
          status: 'executed',
          summary: `Kontrola integrity: ${table}`,
          issues,
