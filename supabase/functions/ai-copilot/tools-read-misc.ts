  ...
  switch (name) {
    case 'get_documents': {
      try {
        ...
        if (docType === 'emails') { const { data } = await sb.from('sent_emails').select('id, template_slug, recipient_email, subject, status, error_message, booking_id, sent_at, created_at, attachments_meta').order('created_at', { ascending: false }).limit(limit); return { documents: data || [], total_count: (data || []).length } }
    ...
    case 'get_cms_settings': {
      try {
        ...
        if (section === 'flags') { const { data } = await sb.from('feature_flags').select('*').order('key', { ascending: true }); return { feature_flags: data || [] } }
        ...
        const [fR, vR, sR] = await Promise.all([
          sb.from('feature_flags').select('*').order('key', { ascending: true }),
