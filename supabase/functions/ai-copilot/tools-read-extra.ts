...
export async function execReadExtra(name, input, sb) {
  switch(name){
    case 'get_contracts':
      {
        const limit = input.limit || 20;
        let q = sb.from('contracts').select('*, acc_employees(name)').order('created_at', {
          ascending: false
        }).limit(limit);
    ...
    case 'get_purchase_orders':
      {
        const limit = input.limit || 20;
        let q = sb.from('purchase_orders').select('*, purchase_order_items(*)').order('created_at', {
          ascending: false
        }).limit(limit);
        if (input.status) q = q.eq('status', input.status);
        const { data } = await q;
    ...
    case 'get_booking_complaints':
      {
        const limit = input.limit || 20;
        let q = sb.from('booking_complaints').select('*, profiles(full_name, email)').order('created_at', {
          ascending: false
        }).limit(limit);
        if (input.status) q = q.eq('status', input.status);
        const { data } = await q;
    ...
    case 'get_booking_cancellations':
      {
        const limit = input.limit || 20;
        const { data } = await sb.from('booking_cancellations').select('*').order('created_at', {
          ascending: false
        }).limit(limit);
        const totalRefund = (data || []).reduce((s, c)=>s + (c.refund_amount || 0), 0);
        return {
          cancellations: data || [],
          count: (data || []).length,
    ...
    case 'get_moto_locations':
      {
        const { data } = await sb.from('moto_locations').select('*, motorcycles(model, spz)').order('updated_at', {
          ascending: false
        }).limit(50);
        return {
          locations: data || [],
    ...
    case 'get_auto_order_rules':
      {
        const { data } = await sb.from('auto_order_rules').select('*, inventory(name, sku)').order('created_at', {
          ascending: false
        });
        return {
          rules: data || [],
    ...
    case 'get_notification_log':
      {
        ...
        const { data } = await sb.from('notification_log').select('*').order('created_at', {
          ascending: false
        }).limit(limit);
        return {
          notifications: data || [],
    ...
    case 'get_message_templates':
      {
        ...
        return {
          sms_templates: smsR.data || [],
          email_templates: emailR.data || []
        };
      }
    ...
    case 'get_delivery_notes':
      {
        const limit = input.limit || 20;
        let q = sb.from('delivery_notes').select('*, invoices(number, total)').order('created_at', {
          ascending: false
        }).limit(limit);
    ...
    case 'get_performance_stats':
      {
        const [motoR, branchR] = await Promise.all([
          sb.from('moto_performance').select('*').order('updated_at', {
            ascending: false
          }).limit(30),
          sb.from('branch_performance').select('*').order('updated_at', {
            ascending: false
          }).limit(10)
        ]);
        return {
          moto_performance: motoR.data || [],
          branch_performance: branchR.data || []
        };
      }
    default:
      return null;
