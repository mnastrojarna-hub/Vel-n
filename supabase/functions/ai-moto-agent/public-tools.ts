...
export const PUBLIC_READ_TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        brand: {
          type: 'string',
          description: 'Značka, např. Kawasaki, BMW, Yamaha, Honda. Case-insensitive substring.'
        },
        model_query: {
          type: 'string',
          description: 'Volnotextový dotaz na model (např. "Z 900", "MT-09").'
        },
        category: {
          type: 'string',
          enum: [
          ...
          description: 'Kategorie dle DB. Sportovní = "sportovni", choppery = "chopper", nezařazené = "ostatni". Skútry = "scootery" (na dotaz „máte skútr / scooter 125" filtruj category=scootery, NE model_query).'
        },
        license_group: {
          type: 'string',
          enum: [
        ...
        available_on: {
          type: 'string',
          description: 'Datum YYYY-MM-DD — vrátí jen motorky volné v tento den.'
        },
        available_from: {
          type: 'string',
          description: 'Spolu s available_to — rozsah YYYY-MM-DD.'
        },
        available_to: {
          type: 'string'
        }
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        topic: {
          type: 'string',
          description: 'Volitelně téma — cancellation, deposit, included, addons, delivery_pricing, foreign_travel, fuel, license_groups, documents.'
        }
      }
    }
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        document: {
          type: 'string',
          description: 'Klíč dokumentu — `vop`, `rental_contract`, `handover_protocol`, `gdpr`, nebo slug webového dokumentu. Vynech pro seznam.'
        },
        query: {
          type: 'string',
          description: 'Volitelně klíčová slova — vrátí jen relevantní úseky.'
        }
      }
    }
  },
 ...
 */ export async function execPublicReadTool(name, args, sb, lang = 'cs') {
  switch(name){
