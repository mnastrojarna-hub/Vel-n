...
serve(async (req)=>{
  ...
  try {
    ...
    if (userErr || !user) {
      ...
      return new Response(JSON.stringify({
        error: 'Unauthorized: ' + (userErr?.message || 'invalid token')
      }), {
        status: 401,
        headers: {
          ...CORS,
    ...
    if (conversation_history && Array.isArray(conversation_history)) {
      ...
      let hist = conversation_history.filter((m)=>(m?.role === 'user' || m?.role === 'assistant') && typeof m?.content === 'string' && m.content.trim() !== '').map((m)=>({
          role: String(m.role),
          content: m.content.length > PER_MSG ? m.content.slice(0, PER_MSG) : m.content
        }));
      if (hist.length > MAX_MSGS) hist = hist.slice(-MAX_MSGS);
      let totalChars = hist.reduce((n, m)=>n + m.content.length, 0);
      while(hist.length > 1 && totalChars > TOTAL){
        totalChars -= hist[0].content.length;
        hist.shift();
      }
      while(hist.length && hist[0].role !== 'user')hist.shift();
      for (const m of hist)apiMessages.push({
        role: m.role,
        content: m.content
      });
    }
    ...
    try {
      ...
      if (booking_id) {
        ...
        const [pinnedRes, listRes] = await Promise.all([
          supabaseAdmin.from('bookings').select(bookingFields).eq('user_id', user.id).eq('id', booking_id).limit(1),
          supabaseAdmin.from('bookings').select(bookingFields).eq('user_id', user.id).in('status', [
            'active',
            'confirmed',
            'reserved'
          ]).order('start_date', {
        ...
        const { data: allBookings, error: allErr } = await supabaseAdmin.from('bookings').select(bookingFields).eq('user_id', user.id).in('status', [
          'active',
          'confirmed',
          'reserved'
        ]).order('start_date', {
...
Zákazník nemá aktivní rezervaci nebo se nepodařilo načíst data. Při dotazech na konkrétní motorku použij nástroj get_active_booking. NIKDY si nevymýšlej, jakou motorku má zákazník. Na dotaz, kde si motorku vyzvedne / kde je pobočka („kde jste", „kde je filiálka"), odpověz podle sekce POBOČKY níže (detaily přes get_branches) a pošli ho na pobočku — NIKDY netvrď, že informace o pobočkách není dostupná.`;
    ...
    try {
      ...
      const brRows = (brData || []).filter((b)=>b.active !== false && b.is_open !== false);
...
Tato verze appky přílohy NEUMÍ — o fotku NEŽÁDEJ, doptávej se slovně. Když fotka přesto přijde, vyhodnoť ji.`;
    ...
    // Přidává se v index.ts, takže platí pro config i FALLBACK prompt (text: SEASON_NOTE v booking-context.ts).
    ...
    for(let i = 0; i < MAX_ITERATIONS; i++){
      ...
      if (stopReason === 'end_turn') {
        const textBlocks = (aiResult.content || []).filter((c)=>c.type === 'text');
      ...
      if (stopReason === 'tool_use') {
        ...
        const toolResults = [];
        const toolCalls = (aiResult.content || []).filter((c)=>c.type === 'tool_use');
    ...
    reply = finalText.replace(/---JSON---[\s\S]*?---END---/g, '').replace(/---JSON---[\s\S]*$/, '').trim();
    let loggedConversationId = null;
    ...
    try {
      const MSG_CAP = 8000;
      const historyMsgs = (Array.isArray(conversation_history) ? conversation_history : []).filter((m)=>(m?.role === 'user' || m?.role === 'assistant') && typeof m?.content === 'string').map((m)=>({
          role: m.role,
      ...
      const fullMessages = [
        ...
        {
          role: 'user',
          content: String(userContent).slice(0, MSG_CAP)
        },
        {
          role: 'assistant',
          content: reply.slice(0, MSG_CAP)
        }
      ];
      let updatedId = null;
      if (convId) {
        // Appka adresuje konkrétní vlákno — žádná heuristika, žádné slévání souběžných konverzací.
        const { error: updErr } = await supabaseAdmin.from('ai_customer_conversations').update({
      ...
      if (!updatedId && historyMsgs.length > 0) {
        const { data: last } = await supabaseAdmin.from('ai_customer_conversations').select('id').eq('user_id', user.id).order('updated_at', {
          ascending: false
        }).limit(1).maybeSingle();
        if (last) {
          const { error: updErr } = await supabaseAdmin.from('ai_customer_conversations').update({
