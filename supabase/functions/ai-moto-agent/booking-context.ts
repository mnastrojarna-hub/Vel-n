...
export function formatBranchLines(m) {
  ...
  if (br.type === 'samoobslužná') {
    lines.push(m?.box_number ? `- KÓJE motorky: ${m.box_number} (na dveřích kóje je toto číslo; kód k motorce otevře právě ji, kód k výbavě otevře ŠATNU). Tohle zákazníkovi říct SMÍŠ — je to jeho rezervace.` : `- KÓJE motorky: v datech není vyplněná (motorcycles.box_number je prázdné) — číslo kóje NEHÁDEJ, řekni, že ho uvidí na displeji jednotky hned po zadání kódu.`);
  }
  return lines.join('\n');
}
