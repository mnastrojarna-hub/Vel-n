...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (booking_id) {
      const { data } = await supabase.from('bookings').select('user_id, stripe_payment_intent_id, stripe_session_id, total_price, payment_status, stripe_refund_id, card_brand, card_last4').eq('id', booking_id).single();
      if (!data) {
        return new Response(JSON.stringify({
          success: false,
