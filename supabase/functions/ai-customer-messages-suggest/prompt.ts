...
import { formatFleetSnapshot, formatBranchesSnapshot } from '../_shared/agent-knowledge/snapshots.ts';
...
import { dirLabel } from './context.ts';
export const CHANNEL_LIMITS = {
  sms: {
    max_chars: 320,
    hint: 'krátká SMS, bez podpisu, bez dlouhých URL'
  },
  whatsapp: {
    max_chars: 600,
    hint: 'krátké, lidské, emoji jen pokud je psal zákazník'
  },
  email: {
    max_chars: 1500,
    hint: 'celé věty, oslovení, krátký podpis „Tým MotoGo24"'
  },
  app_chat: {
    max_chars: 500,
    hint: 'stručné, lze odkázat na obrazovku v appce'
  }
};
const TONE = {
  friendly: 'přátelský a vstřícný',
  professional: 'profesionální a věcný',
  concise: 'maximálně stručný',
  detailed: 'podrobný, s vysvětlením souvislostí'
};
export async function loadKnowledgeInputs(sb) {
  const [pub, moto, ci, fleet, br, kb] = await Promise.all([
    ...
    sb.from('motorcycles').select('id, brand, model, category, license_required, status, power_kw, engine_cc, weight_kg, price_mon, price_tue, price_wed, price_thu, price_fri, price_sat, price_sun, branch_id, branches!branch_id(name, type, is_open)').in('status', [
      'active',
      'maintenance',
      'unavailable'
  ...
  return {
    pubCfg: pub.data?.value || {},
    motoCfg: moto.data?.value || {},
    company: ci.data?.value || {},
    fleet: (fleet.data || []).filter((m)=>m.branches?.is_open !== false),
    branches: (br.data || []).filter((b)=>b.active !== false && b.is_open !== false),
    kb
  };
}
function rules(title, c) {
  const out = [];
...
export function buildSystem(cfg, k, ctx) {
  const lim = CHANNEL_LIMITS[ctx.channel] || {
    max_chars: 1000,
    hint: 'standardní formát'
  };
...
5. Rozhodnutí o penězích (refund nad rámec pravidel, výjimka ze storna, sleva, uznání reklamace, škody) nevyslovuj za firmu — navrhni neutrální formulaci a do admin_note napiš, co musí rozhodnout člověk. Přístupové kódy (číslice) NIKDY nepiš.`,
    ...
    [
      ...
      rules('Agent zákaznických zpráv (tento)', cfg),
      rules('Servisní agent v appce', k.motoCfg),
      rules('Veřejný agent na webu', k.pubCfg)
    ].filter(Boolean).join('\n\n')
  ].filter(Boolean).join('\n\n');
...
- Zákazník: ${ctx.customerId ? `${c.full_name || '?'} | ${c.email || '?'} | ${c.phone || '?'} | jazyk ${c.language || 'cs'} | ŘP ${Array.isArray(c.license_group) ? c.license_group.join(', ') : c.license_group || '?'}${c.is_blocked ? ' | ZABLOKOVANÝ' : ''} [customer_id=${ctx.customerId}]` : 'nepřiřazen (dohledej přes find_booking)'}`,
    ctx.bookingContext + ctx.bookingsList,
    '## HISTORIE VLÁKNA (od nejstarší)\n' + (ctx.history.map((h)=>`[${dirLabel(h.direction)} ${String(h.created_at).slice(0, 16).replace('T', ' ')}] ${(h.content || '').slice(0, 1500)}`).join('\n') || '(prázdná)'),
    `## VÝSTUPNÍ FORMÁT
