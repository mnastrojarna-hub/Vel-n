...
const CHANNEL_LIMITS = {
  sms: {
    max_chars: 320,
    tone_hint: 'krátká SMS, bez podpisu, bez dlouhých URL'
  },
  whatsapp: {
    max_chars: 600,
    tone_hint: 'krátké, lidské, smí emoji jen pokud je psal zákazník'
  },
  email: {
    max_chars: 1500,
    tone_hint: 'celá věta, lze strukturované, krátký podpis'
  },
  app_chat: {
    max_chars: 400,
    tone_hint: 'stručné jako SMS, lze odkaz na obrazovku v appce'
  }
};
...
async function loadContext(messageId) {
  const { data: m } = await sb.from('messages').select('id, thread_id, direction, content, created_at, ai_suggestion_status').eq('id', messageId).maybeSingle();
  if (!m) return null;
  const { data: t } = await sb.from('message_threads').select('id, channel, status, customer_id, assigned_admin').eq('id', m.thread_id).maybeSingle();
  if (!t) return null;
  let customer = null;
  if (t.customer_id) {
    const { data: c } = await sb.from('profiles').select('id, full_name, email, phone, language, license_group, riding_experience, city, ico').eq('id', t.customer_id).maybeSingle();
    customer = c;
  }
  // Aktuální / nejbližší rezervace zákazníka
  let booking = null;
  if (t.customer_id) {
    const { data: b } = await sb.from('bookings').select('id, moto_id, start_date, end_date, status, payment_status, total_price, pickup_method, return_method, deposit, contract_url').eq('user_id', t.customer_id).in('status', [
      'pending',
      'reserved',
      'active',
      'completed'
  ...
  const { data: hist } = await sb.from('messages').select('direction, content, created_at').eq('thread_id', m.thread_id).order('created_at', {
    ascending: true
  }).limit(20);
...
function buildSystemPrompt(cfg, ctx) {
  ...
  if (ctx.customer) {
    ...
    parts.push(`\n— ZÁKAZNÍK —\n${c.full_name || '?'} | ${c.email || '?'} | jazyk: ${c.language || 'cs'}`);
  }
  if (ctx.booking) {
    const b = ctx.booking;
  ...
  if (ctx.history.length > 0) {
    ...
    for (const h of ctx.history){
      ...
      const label = d === 'customer' || d === 'inbound' ? 'Zákazník' : d === 'admin' || d === 'outbound' ? 'Admin' : d === 'system' ? 'Systém' : d;
  ...
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        {
          role: 'user',
          content: `Příchozí zákaznická zpráva:\n${userMessage}\n\nNavrhni odpověď ve formátu JSON.`
        }
      ]
    })
  });
...
async function processSuggestion(messageId) {
  ...
  try {
    ...
    if (ctx.message.ai_suggestion_status && ctx.message.ai_suggestion_status !== 'failed') {
      await logTraffic({
        ...
        latency_ms: Date.now() - start,
        details: {
          existing_status: ctx.message.ai_suggestion_status
        }
      });
      return;
    }
...
serve(async (req)=>{
  ...
  if (typeof EdgeRuntime !== 'undefined' && EdgeRuntime?.waitUntil) {
    ...
    processSuggestion(messageId).catch(()=>{});
  }
  return new Response(JSON.stringify({
    ok: true,
    message_id: messageId,
