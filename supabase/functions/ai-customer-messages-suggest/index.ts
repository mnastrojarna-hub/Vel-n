...
import { buildSystem, loadKnowledgeInputs } from './prompt.ts';
...
async function generate(cfg, threadId, focusMessageId) {
  const [ctx, k] = await Promise.all([
    loadThreadContext(sb, threadId, focusMessageId),
    loadKnowledgeInputs(sb)
  ]);
  if (!ctx) return null;
  const task = ctx.focus ? `Navrhni odpověď na tuto zprávu zákazníka (z ${String(ctx.focus.created_at).slice(0, 16).replace('T', ' ')}):\n"""${ctx.focus.content || ''}"""` : 'Ve vlákně není zpráva od zákazníka — navrhni vhodnou další zprávu týmu podle historie vlákna.';
...
async function processSuggestion(messageId) {
  ...
  let channel, threadId;
  const fail = (msg)=>sb.from('messages').update({
  ...
  try {
    ...
    if (m.ai_suggestion_status && m.ai_suggestion_status !== 'failed') {
      await logTraffic({
    ...
    if (cfg.channels && cfg.channels[channel] === false) {
      await fail(`Kanál ${channel} má AI návrhy ve Velínu vypnuté.`);
      await logTraffic({
        outcome: 'channel_off',
        message_id: messageId,
        thread_id: threadId,
...
serve(async (req)=>{
  ...
  if (body.mode === 'draft') {
    if (!body.thread_id || typeof body.thread_id !== 'string') return json({
  ...
  if (!messageId || typeof messageId !== 'string') return json({
  ...
  else processSuggestion(messageId).catch(()=>{});
  return json({
    ok: true,
    message_id: messageId,
    status: 'queued',
