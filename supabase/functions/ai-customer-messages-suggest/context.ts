...
export const isInbound = (d: string | null) => ['customer', 'inbound'].includes((d || '').toLowerCase())
const isStaff = (d: string | null) => ['admin', 'outbound'].includes((d || '').toLowerCase())
...
export function bookingLine(b: Record<string, unknown>): string {
...
async function loadBookings(sb: SupabaseClient, customerId: string): Promise<{ ctx: string; list: string }> {
  ...
  if (!all.length) return { ctx: '\n\n## KONTEXT REZERVACE:\nZákazník v DB nemá žádnou rezervaci (ani historickou).', list: '' }
  const today = new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Prague' }).format(new Date())
  const live = all.filter((b) => ['active', 'reserved', 'pending'].includes(String(b.status)))
...
async function loadStaffExamples(sb: SupabaseClient, excludeThread: string): Promise<string> {
  try {
    const { data } = await sb.from('messages').select('thread_id, direction, content, created_at')
      .neq('thread_id', excludeThread).order('created_at', { ascending: false }).limit(600)
    const byThread = new Map<string, Msg[]>()
    for (const m of ((data || []) as Array<Msg & { thread_id: string }>).reverse()) {
      if (!byThread.has(m.thread_id)) byThread.set(m.thread_id, [])
      byThread.get(m.thread_id)!.push(m)
    }
    const pairs: Array<{ at: string; q: string; a: string }> = []
    for (const msgs of byThread.values()) {
      for (let i = 1; i < msgs.length; i++) {
        ...
        pairs.push({ at: a.created_at, q: qa.slice(0, 300), a: aa.slice(0, 450) })
      }
    }
...
export async function loadThreadContext(sb: SupabaseClient, threadId: string, focusMessageId?: string): Promise<ThreadContext | null> {
  ...
  if (focusMessageId) {
    ...
    if (idx >= 0) { focus = history[idx]; history = history.slice(0, idx + 1) }
  }
  if (!focus) focus = [...history].reverse().find((m) => isInbound(m.direction)) || null
  history = history.slice(-40)

  return {
    thread, customerId,
    channel: String(thread.channel || 'unknown').toLowerCase(),
    customer: ((custRes as { data: Record<string, unknown> | null }).data) || null,
    bookingContext: bookings.ctx, bookingsList: bookings.list,
    history, focus, staffExamples,
  }
}
