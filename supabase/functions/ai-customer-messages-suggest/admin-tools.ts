...
const ADMIN_TOOLS = [
  {
    ...
    input_schema: {
      ...
      properties: {
        customer_id: {
          type: 'string',
          description: 'UUID zákazníka (volitelné — výchozí je zákazník vlákna)'
        }
      },
      required: []
    }
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        query: {
          type: 'string',
          description: 'Číslo rezervace, e-mail, telefon nebo jméno'
        }
      },
      required: [
        'query'
...
async function customerOverview(sb, cid) {
  const [prof, bk, sos, compl, orders] = await Promise.all([
    ...
    sb.from('bookings').select(BOOKING_SELECT).eq('user_id', cid).order('start_date', {
      ascending: false
    }).limit(30),
    sb.from('sos_incidents').select('id, booking_id, type, title, description, severity, status, moto_rideable, resolution, created_at').eq('user_id', cid).order('created_at', {
      ascending: false
    }).limit(10),
    sb.from('booking_complaints').select('booking_id, subject, description, status, resolution, created_at').eq('customer_id', cid).order('created_at', {
      ascending: false
    }).limit(10),
    sb.from('shop_orders').select('order_number, status, payment_status, total, created_at, shipped_at, tracking_number').eq('customer_id', cid).order('created_at', {
      ascending: false
    }).limit(10)
  ]);
  const email = prof.data?.email;
  const mails = email ? await sb.from('sent_emails').select('template_slug, subject, status, sent_at, booking_id').eq('recipient_email', email).order('created_at', {
    ascending: false
  }).limit(15) : {
    data: []
  };
  const bookings = (bk.data || []).map((b)=>({
      ...
      mileage_start: b.mileage_start,
      mileage_end: b.mileage_end,
      handover_protocol_filled_at: b.handover_protocol_filled_at,
      docs_completed_at: b.docs_completed_at,
      extras_price: b.extras_price,
      delivery_fee: b.delivery_fee,
      payment_method: b.payment_method,
      pay_channel: b.pay_channel,
      damage_flag: b.damage_flag,
      damage_note: b.damage_note,
      complaint_status: b.complaint_status,
      modification_history: b.modification_history,
      notes: b.notes,
      moto_box_number: b.motorcycles?.box_number,
      branch: b.motorcycles?.branches?.name
    }));
  return {
    profile: prof.data,
    bookings,
    sos_incidents: sos.data || [],
    complaints: compl.data || [],
    shop_orders: orders.data || [],
    sent_emails: mails.data || []
  };
}
async function findBooking(sb, query) {
  ...
  if (/^[0-9a-f]{8}$/i.test(q)) {
    const { data } = await sb.from('bookings').select(BOOKING_SELECT).order('created_at', {
      ascending: false
    }).limit(1500);
    const hit = (data || []).filter((b)=>String(b.id).toLowerCase().endsWith(q.toLowerCase()));
    return {
      bookings: hit.map((b)=>({
          user_id: b.user_id,
          line: bookingLine(b)
  ...
  const { data: profs } = await sb.from('profiles').select('id, full_name, email, phone').or(`email.ilike.%${safe}%,phone.ilike.%${safe.replace(/\s/g, '')}%,full_name.ilike.%${safe}%`).limit(5);
  const out = [];
  for (const p of profs || []){
    const { data } = await sb.from('bookings').select(BOOKING_SELECT).eq('user_id', p.id).order('start_date', {
      ascending: false
    }).limit(10);
    out.push({
      customer: p,
      bookings: (data || []).map(bookingLine)
    });
  }
  return out.length ? {
    matches: out
...
export async function executeAgentTool(sb, name, input, customerId, lang) {
  try {
    if (name === 'get_customer_overview') {
      const cid = typeof input.customer_id === 'string' && input.customer_id || customerId;
      return cid ? await customerOverview(sb, cid) : {
        error: 'Vlákno nemá přiřazeného zákazníka — použij find_booking.'
      };
    }
    if (name === 'find_booking') return await findBooking(sb, String(input.query || ''));
    if (NEEDS_CUSTOMER.has(name) && !customerId) return {
      error: 'Vlákno nemá přiřazeného zákazníka — nejdřív find_booking.'
    };
