...
function parseResult(text) {
      ...
      if (reply) {
        return {
          ...
          confidence: [
            ...
            'medium',
            'high'
          ].includes(p.confidence) ? p.confidence : 'medium',
  ...
  return {
    reply,
    confidence: 'low',
    admin_note: 'Výstup nebyl ve formátu JSON — zkontroluj text návrhu.'
  };
}
export async function runAgent(sb, system, task, customerId, maxTokensCfg) {
  ...
  for(let i = 0; i < MAX_ITERS; i++){
    ...
    if (data.stop_reason === 'max_tokens' && maxTokens < 16000) {
      maxTokens = 16000;
      continue;
    }
    if (data.stop_reason === 'tool_use') {
      ...
      for (const tb of data.content.filter((b)=>b.type === 'tool_use')){
        ...
        const r = await executeAgentTool(sb, String(tb.name), tb.input || {}, customerId, 'cs');
        results.push({
          type: 'tool_result',
          tool_use_id: tb.id,
          content: JSON.stringify(r).slice(0, 60_000)
        });
      }
      messages.push({
        role: 'user',
