...
export function buildHtml(v, form, signature, signer) {
  ...
  const where = signer.by === 'kiosk' ? `na displeji pobočky (zařízení ${esc(signer.deviceId || '?')})` : 'v aplikaci MotoGo24';
