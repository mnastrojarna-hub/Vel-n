...
/** Dekódovaná velikost obrázku v data-URL (bajty), bez odečtu paddingu — jako kiosk. */ export function signatureBytes(sig) {
...
/** Kalendářní den v Praze (YYYY-MM-DD) — porovnání „je už den převzetí?“. */ export function pragueDay(d) {
...
/** signed_at z kiosku (podpis mohl čekat ve frontě); mimo rozumné okno → teď. */ export function parseSignedAt(raw, now) {
...
/** PDF přes render-pdf do bucketu `documents`; když se nepovede, uloží HTML. Vrací cestu. */ export async function storeDocument(admin, bookingId, docId, html) {
...
/** Úklid souboru, ke kterému nevznikl řádek generated_documents (sirotek); best-effort. */ export async function removeDocument(admin, path) {
  if (!path) return;
  try {
