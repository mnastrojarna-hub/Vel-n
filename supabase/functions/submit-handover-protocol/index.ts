...
const HANDOVER_CHECKS = [
  {
    key: 'clean',
    label: 'Motocykl předán čistý a v provozuschopném stavu'
  },
  {
    key: 'docs',
    label: 'Doklady k vozidlu (OTP, zelená karta) předány'
  },
  {
    key: 'keys',
    label: 'Klíče a zabezpečení předány'
  },
  {
    key: 'instructed',
    label: 'Nájemce poučen o obsluze a provozu'
  },
  {
    key: 'gear',
    label: 'Ochranná výbava předána a vyzkoušena'
  }
];
const EXTRA_GEAR_CHECKS = [
  {
    key: 'phone_holder',
    label: 'Držák na telefon'
  },
  {
    key: 'usb_adapter',
    label: 'USB 12V přechodka'
  },
  {
    key: 'disc_lock',
    label: 'Kotoučový zámek'
  },
  {
    key: 'rain_suit',
    label: 'Set nepromokavé bundy a kalhot'
  },
  {
    key: 'rain_boots',
    label: 'Nepromoky na nohy'
  },
  {
    key: 'rain_gloves',
    label: 'Nepromoky na ruce'
  },
  {
    key: 'tie_net',
    label: 'Upínací síťka'
  ...
  {
    key: 'chain_spray',
    label: 'Sprej na řetěz'
  }
];
function buildHtml(v, form, signature, autofilled) {
  ...
  const accTable = accRows ? `<table style="width:100%;border-collapse:collapse;font-size:11px;margin:6px 0;border:1px solid #ddd"><tr><th style="padding:6px 8px;border:1px solid #ddd;background:#f0f7ff;text-align:left;font-size:10px;text-transform:uppercase">Položka</th><th style="padding:6px 8px;border:1px solid #ddd;background:#f0f7ff;text-align:left;font-size:10px;text-transform:uppercase">Velikost</th><th style="padding:6px 8px;border:1px solid #ddd;background:#f0f7ff;text-align:center;font-size:10px;text-transform:uppercase">Předáno</th></tr>${accRows}</table>` : '<p style="font-size:12px">Žádné zapůjčené příslušenství.</p>';
  const damageBlock = `<div style="font-size:12px;margin:5px 0">${damage.checked ? '☑' : '☐'} Poškození při předání</div>` + (damage.checked && damage.desc ? `<p style="font-size:12px;margin:4px 0 0;padding:8px 10px;background:#fef2f2;border:1px solid #fca5a5;border-radius:6px">${escMulti(damage.desc)}</p>` : '');
  const TD = 'padding:6px 8px;border:1px solid #ddd';
  const TDL = `${TD};background:#f8faf9;font-weight:600;width:220px`;
  const row = (l, val)=>`<tr><td style="${TDL}">${esc(l)}</td><td style="${TD}">${val ? escMulti(val) : '&nbsp;'}</td></tr>`;
  ...
  const sigCaption = autofilled ? 'Protokol vyplněn automaticky systémem — zákazník motocykl převzal dle rezervace a do 60 minut od zpřístupnění nenahlásil žádné závady. MotoGo24 automaticky souhlasí.' : '';
  const sigBlock = autofilled ? `<div style="margin-top:32px;padding:12px;border:1px dashed #999;border-radius:8px;font-size:11px;color:#444">${esc(sigCaption)}</div>` : `<div style="margin-top:40px;display:flex;justify-content:center"><div style="text-align:center;width:60%"><div style="height:96px;border:1px solid #ccc;border-radius:8px;display:flex;align-items:center;justify-content:center;background:#fff;overflow:hidden">${signature ? `<img src="${signature}" alt="podpis" style="max-width:100%;max-height:92px"/>` : ''}</div><div style="border-top:1px solid #999;margin-top:6px;padding-top:6px;font-size:11px">Podpis nájemce — ${esc(v.customer_name)}</div></div></div>`;
  ...
  return `<!DOCTYPE html><html lang="cs"><head><meta charset="utf-8"><title>Předávací protokol</title></head><body style="margin:0;padding:0;font-family:'Segoe UI',sans-serif;color:#1a1a1a"><div style="max-width:780px;margin:0 auto;padding:32px"><h1 style="text-align:center;font-size:19px;border-bottom:2px solid #2563eb;padding-bottom:12px">PŘEDÁVACÍ PROTOKOL</h1><p style="text-align:center;font-size:12px;color:#666">k rezervaci č. ${esc(v.booking_number)} ze dne ${esc(v.today)}</p>` + parties + `<h3 style="font-size:13px;margin-top:14px">Stav při předání</h3><table style="width:100%;border-collapse:collapse;font-size:12px;border:1px solid #ddd">${row('Stav km při předání', mileage ? `${mileage} km` : '')}</table>` + `<h3 style="font-size:13px;margin-top:14px">Kontrola předání</h3>${checkList}` + `<h3 style="font-size:13px;margin-top:14px">Příslušenství a výbava</h3>${accTable}` + `<h3 style="font-size:13px;margin-top:14px">Doplňkové vybavení</h3>${extraList}` + `<h3 style="font-size:13px;margin-top:14px">Poškození</h3>${damageBlock}` + (notes ? `<h3 style="font-size:13px;margin-top:14px">Poznámky</h3><p style="font-size:12px">${escMulti(notes)}</p>` : '') + sigBlock + foot;
}
function fmtDate(d) {
  if (!d) return '';
  try {
...
serve(async (req)=>{
  ...
  try {
    ...
    const mode = body.mode === 'auto' ? 'auto' : 'customer';
    if (!bookingId) return json({
      success: false,
      error: 'missing_booking_id'
    }, 400);
    // Auth: auto = jen service_role; customer = přihlášený majitel rezervace.
    const who = await authClassify(req);
    if (mode === 'auto' && who.kind !== 'service') return json({
      success: false,
      error: 'forbidden'
    }, 403);
    if (mode === 'customer' && who.kind === 'none') return json({
      success: false,
      error: 'unauthenticated'
    }, 401);
    // Načtení rezervace + vozidlo + pobočka (service role, obejde RLS).
    const { data: booking, error: bErr } = await admin.from('bookings').select('*, motorcycles!moto_id(model, spz, vin, branch_id, branches(type))').eq('id', bookingId).maybeSingle();
    if (bErr || !booking) return json({
      success: false,
      error: 'not_found'
    }, 404);
    ...
    if (booking.handover_protocol_filled_at) {
      return json({
        success: true,
        already_filled: true,
        autofilled: booking.handover_protocol_autofilled
      });
    }
    const moto = booking.motorcycles || {};
    const branchType = (moto.branches || {}).type;
    if (branchType !== 'samoobslužná') return json({
      success: false,
    ...
    try {
      ...
      if (rp.ok && rp.headers.get('content-type')?.includes('application/pdf')) {
        ...
        const up = await admin.storage.from('documents').upload(pdfPath, bytes, {
          upsert: true,
    ...
    if (!stored) {
      ...
      const up = await admin.storage.from('documents').upload(pdfPath, new Blob([
        html
      ], {
        type: 'text/html'
      }), {
    ...
    try {
      const rawMileage = form.mileage ?? '';
      const km = parseInt(String(rawMileage).replace(/[^\d]/g, ''), 10);
      if (!autofilled && Number.isFinite(km) && km > 0 && (!booking.mileage_start || km > Number(booking.mileage_start))) {
        await admin.from('bookings').update({
          mileage_start: km
        }).eq('id', bookingId);
      }
