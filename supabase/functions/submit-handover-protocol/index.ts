...
import { CORS, fail, fmtDate, json, parseSignedAt, pragueDay, removeDocument, SIG_MAX_APP, SIG_MAX_KIOSK, SIG_RE, signatureBytes, storeDocument, UUID_RE } from './util.ts';
...
serve(async (req)=>{
  ...
  try {
    ...
    if (modeRaw === 'auto') return fail('forbidden', 403, {
      reason: 'auto_mode_removed'
    });
    const mode = modeRaw === 'kiosk' ? 'kiosk' : 'customer';
    const now = new Date();
    // ── Auth ───────────────────────────────────────────────────────────────
    ...
    if (mode === 'kiosk') {
      ...
      const { data: br, error: aErr } = await admin.rpc('kiosk_device_branch', {
        p_device_id: deviceId,
        p_device_token: token,
        p_touch: false
      });
      if (aErr) return fail('auth_error', 500, {
        detail: aErr.message
      });
    ...
    const { data: booking, error: bErr } = await admin.from('bookings').select('*, motorcycles!moto_id(model, spz, vin, mileage, branch_id, branches(type))').eq('id', bookingId).maybeSingle();
    if (bErr) return fail('db_error', 500, {
      detail: bErr.message
    });
    ...
    if (booking.handover_protocol_filled_at) return json({
      success: true,
      already_filled: true
    });
    if (mode === 'kiosk') {
      ...
      const { data: code, error: cErr } = await admin.from('branch_door_codes').select('id').eq('booking_id', bookingId).eq('code_type', 'motorcycle').eq('branch_id', branchId).eq('sent_to_customer', true).limit(1).maybeSingle();
      if (cErr) return fail('auth_error', 500, {
        detail: cErr.message
      });
      if (!code) return fail('forbidden', 403);
    }
    const okStatus = mode === 'kiosk' ? OK_STATUS_KIOSK : OK_STATUS_APP;
    if (!okStatus.includes(String(booking.status))) return fail('wrong_status', 400, {
    ...
    const signer = mode === 'kiosk' ? {
      by: 'kiosk',
      deviceId,
      signedAt: parseSignedAt(body.signed_at, now)
    } : {
      by: 'app',
      signedAt: now
    };
    ...
    if (mode === 'kiosk') {
      ...
      form.checks = {
        ...
        ...form.checks && typeof form.checks === 'object' ? form.checks : {}
      };
      if (!form.damage || typeof form.damage !== 'object') form.damage = {
        checked: false,
        desc: ''
      };
    ...
    const vars = {
      booking_number: bn,
      today: now.toLocaleDateString('cs-CZ', {
    ...
    let pdfPath = '';
    try {
      pdfPath = await storeDocument(admin, bookingId, docId, html);
    } catch (e) {
    ...
    const { data: claimed, error: clErr } = await admin.from('bookings').update({
      handover_protocol_filled_at: claimedAt,
      handover_protocol_autofilled: false
    }).eq('id', bookingId).is('handover_protocol_filled_at', null).select('id');
    if (clErr) {
      await removeDocument(admin, pdfPath);
      return fail('claim_failed', 500, {
    ...
    if (gErr) {
      ...
      if (gErr.code === '23505') return json({
        success: true,
        already_filled: true
      });
      ...
      try {
        await admin.from('bookings').update({
          handover_protocol_filled_at: null
