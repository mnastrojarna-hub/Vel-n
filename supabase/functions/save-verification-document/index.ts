...
serve(async (req)=>{
  ...
  const docSide = docSideRaw === 'front' || docSideRaw === 'back' ? docSideRaw : null;
  ...
  if (!meta) return jsonRes({
    success: false,
    error: 'Invalid doc_type (id|dl|passport)'
  }, 400);
  if (!imageB64Raw) return jsonRes({
    success: false,
  ...
  if (bytes.byteLength === 0) return jsonRes({
    success: false,
    error: 'Empty image'
  }, 400);
  if (bytes.byteLength > MAX_BYTES) return jsonRes({
    success: false,
    error: 'Image too large (>8MB)'
  }, 413);
  ...
  if (bookingId) {
    try {
      ...
      if (!bk || bk.user_id !== userId) {
        return jsonRes({
          success: false,
          error: 'Booking does not match user'
        }, 403);
      }
    } catch  {
      return jsonRes({
  ...
  if (ins.error && /metadata/i.test(ins.error.message || '')) {
    ...
    if (ins2.error) {
      ...
      return jsonRes({
        ...
        document_id: null,
        file_path: filePath,
        warn: 'documents row insert failed'
      });
    }
    docId = ins2.data?.id || null;
  } else if (ins.error) {
    ...
    return jsonRes({
      ...
      document_id: null,
      file_path: filePath,
      warn: 'documents row insert failed: ' + ins.error.message
    });
  } else {
    docId = ins.data?.id || null;
  }
  ...
  if (ocrFields) {
    ...
    const upd = {};
    if (meta.dbType === 'id_card' && idNum) {
      upd.id_number = idNum;
      upd.id_verified_at = nowIso;
    }
    if (meta.dbType === 'passport' && idNum) {
      upd.id_number = idNum;
      upd.passport_verified_at = nowIso;
    }
    if (meta.dbType === 'drivers_license') {
      if (licNum) {
        upd.license_number = licNum;
        upd.license_verified_at = nowIso;
      }
      if (dateRe.test(licExp)) {
        ...
        try {
          ...
          curExp = String(prof?.license_expiry ?? '').trim();
        } catch  {
          curExp = 'x';
        }
