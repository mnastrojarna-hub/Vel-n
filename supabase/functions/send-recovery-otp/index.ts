...
function siteForLang(lang: string): string { return lang === 'cs' ? 'https://www.motogo24.cz' : 'https://motogo24.com' }
function webLabelForLang(lang: string): string { return lang === 'cs' ? 'motogo24.cz' : 'motogo24.com' }

/** Kurátorské překlady OTP mailu (auth-kritický → bez API latence). */
const OTP_L: Record<string, { subject: string; title: string; intro: string; hour: string; ignore: string; team: string; auto: string }> = {
  cs: { subject: 'Obnovení hesla — ověřovací kód', title: 'Obnovení hesla', intro: 'Pro obnovení hesla zadejte na webu MotoGo24 následující ověřovací kód. Pak rovnou nastavíte nové heslo. Kód platí', hour: '1 hodinu', ignore: 'Pokud jste o obnovení hesla nežádali, e-mail ignorujte. Vaše heslo zůstane beze změny.', team: 'Tým MotoGo24', auto: 'Tato zpráva byla vygenerována automaticky.' },
  en: { subject: 'Password reset — verification code', title: 'Password reset', intro: 'To reset your password, enter the following verification code on the MotoGo24 website. Then set a new password right away. The code is valid for', hour: '1 hour', ignore: "If you didn't request a password reset, please ignore this email. Your password will remain unchanged.", team: 'The MotoGo24 Team', auto: 'This message was generated automatically.' },
  de: { subject: 'Passwort zurücksetzen — Bestätigungscode', title: 'Passwort zurücksetzen', intro: 'Um Ihr Passwort zurückzusetzen, geben Sie den folgenden Bestätigungscode auf der MotoGo24-Website ein. Anschließend legen Sie direkt ein neues Passwort fest. Der Code ist', hour: '1 Stunde', ignore: 'Wenn Sie kein Zurücksetzen des Passworts angefordert haben, ignorieren Sie diese E-Mail bitte. Ihr Passwort bleibt unverändert.', team: 'Ihr MotoGo24-Team', auto: 'Diese Nachricht wurde automatisch generiert.' },
  nl: { subject: 'Wachtwoord opnieuw instellen — verificatiecode', title: 'Wachtwoord opnieuw instellen', intro: 'Voer de volgende verificatiecode in op de website van MotoGo24 om je wachtwoord opnieuw in te stellen. Daarna stel je meteen een nieuw wachtwoord in. De code is', hour: '1 uur', ignore: 'Als je geen wachtwoordherstel hebt aangevraagd, kun je deze e-mail negeren. Je wachtwoord blijft ongewijzigd.', team: 'Het MotoGo24-team', auto: 'Dit bericht is automatisch gegenereerd.' },
...
/** Pretty HTML mail s 8-znakovým OTP kódem v jazyce zákazníka (cs default). */
function renderRecoveryOtpHtml(otp: string, lang: string): string {
...
async function sendViaResend(to: string, html: string, subject: string): Promise<{ ok: boolean; id?: string; error?: string }> {
