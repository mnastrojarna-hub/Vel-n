...
function classifyAnthropicError(status, errText) {
  ...
  if (status === 429) return {
    code: 'rate_limit',
    msg: 'Anthropic API: rate limit, zkus za chvíli.'
  };
  return {
    code: 'anthropic_error',
    msg: `Anthropic API ${status}: ${errText.slice(0, 300)}`
  };
}
function systemPrompt(langName, langCode, hasPlaceholders) {
  return [
    ...
    hasPlaceholders ? '- IMPORTANT: keep ALL template placeholders like {customer_name}, {today}, {{var}}, %s exactly as they are — do NOT translate, do NOT remove, do NOT change the braces.' : '- Keep any {placeholder} / {{var}} tokens unchanged if present.',
    '- Keep emoji unchanged.'
  ].join('\n');
}
async function buildSource(supabaseAdmin, table, row) {
...
async function translateToLang(src, langCode) {
  ...
  if (src.pdf) {
    userContent.push({
      ...
      source: {
        type: 'base64',
        media_type: src.pdf.mediaType,
        data: src.pdf.dataB64
      }
    });
  }
  userContent.push({
    type: 'text',
    text: src.pdf ? `Translate the attached PDF document into ${langName}. Source metadata (Czech): ${JSON.stringify(src.textFields)}. Call the submit_translation tool with the translated values.` : `Translate this Czech document into ${langName}. Czech source: ${JSON.stringify(src.textFields)}. Call the submit_translation tool with the translated values.`
  });
  ...
  for (const k of Object.keys(src.textFields)){
    schemaProperties[k] = {
      type: 'string',
      description: `Translated ${k} (${langName}). Keep HTML structure intact for content_html.`
    };
  }
  ...
  if (src.pdf && !('content_html' in schemaProperties)) {
    schemaProperties.content_html = {
      type: 'string',
      description: `Full translated document body as clean semantic HTML (${langName}).`
    };
  }
  const tool = {
  ...
  const out = {};
  for (const k of Object.keys(src.textFields)){
    out[k] = typeof parsed[k] === 'string' ? parsed[k] : src.textFields[k];
  }
  if (!('content_html' in out) && typeof parsed.content_html === 'string') {
    out.content_html = parsed.content_html;
  }
  return out;
}
serve(async (req)=>{
  ...
  try {
    ...
    const targetLangs = Array.isArray(body.target_langs) && body.target_langs.length > 0 ? body.target_langs.filter((l)=>typeof l === 'string' && LANG_NAMES[l]) : DEFAULT_LANGS;
    if (table !== 'custom_documents' && table !== 'document_templates') return jsonResponse({
    ...
    if (readErr) return jsonResponse({
    ...
    let src;
    try {
      src = await buildSource(supabaseAdmin, table, row);
    } catch (e) {
      return jsonResponse({
    ...
    if (!hasContent) return jsonResponse({
      success: true,
      translations: {},
      skipped: 'no_content'
    });
    ...
    if (Object.keys(translations).length === 0) return jsonResponse({
    ...
    const mergeJsonb = (col, valueFor)=>{
      const existing = row[col];
      const m = existing && typeof existing === 'object' ? {
        ...existing
      } : {};
