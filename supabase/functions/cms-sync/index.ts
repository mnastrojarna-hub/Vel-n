...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (!isServiceRole) {
      ...
      const { data: adminUser, error: adminError } = await adminCheck.from('admin_users').select('id').eq('user_id', user.id).eq('is_active', true).maybeSingle();
    ...
    // Načti CMS proměnné
    let query = admin.from('cms_variables').select('key, value, type, category, description, updated_at').eq('is_active', true).order('category').order('key');
      ...
      switch(varType){
        ...
        case 'array':
          try {
            ...
            value = v.value.split(',').map((s)=>s.trim());
          }
          break;
        default:
          value = v.value;
    ...
    const { data: featureFlags } = await admin.from('feature_flags').select('key, enabled, description, metadata').eq('is_active', true);
    const flags = {};
