...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    // ── 1. Load report ──
    const { data: report, error: loadErr } = await supabase.from('flexi_reports').select('*').eq('id', report_id).single();
    if (loadErr || !report) {
      return jsonResponse({
    ...
    if (report.status !== 'approved') {
      return jsonResponse({
        error: `Report must be approved before submission (current: ${report.status})`
      }, 400);
    }
    ...
    if (!recipientConfig) {
      return jsonResponse({
        error: `No recipient configured for report type: ${report.report_type}`
      }, 400);
    }
    ...
    const { error: uploadErr } = await supabase.storage.from('datova-schranka-outbox').upload(storagePath, xmlBytes, {
    ...
    if (uploadErr) {
      ...
      return jsonResponse({
        error: 'Failed to store XML: ' + uploadErr.message
      }, 500);
    }
    ...
    if (dsApiUrl && dsLogin && dsPassword && recipientDsId) {
      // Send via ISDS API
      try {
        const isdsResult = await sendViaIsds(dsApiUrl, dsLogin, dsPassword, recipientDsId, reportLabel, report, xmlContent);
    ...
    return jsonResponse({
      ...
      instructions: method === 'manual' ? 'Soubor je připraven v úložišti. Nahrajte ho ručně do datové schránky na mojedatovaschranka.cz' : null
    });
  } catch (err) {
...
// ─── XML Generator ─────────────────────────────────────────────
function generateIsdsXml(report, recipientDsId, reportLabel) {
  const periodStr = report.quarter ? `${report.year} Q${report.quarter}` : `${report.year}`;
...
// ─── ISDS API Call ─────────────────────────────────────────────
async function sendViaIsds(apiUrl, login, password, recipientDsId, subject, report, xmlContent) {
