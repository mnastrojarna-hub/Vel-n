...
async function runBatch(sb, opts) {
  ...
  const { data, error } = await sb.from('points_of_interest').select('id, name, lat, lng, country, translations').is('image_url', null).is('photo_checked_at', null).eq('is_active', true).order('sort_order', {
    ascending: true,
    nullsFirst: false
  }).order('id', {
    ascending: true
