...
function nameScore(poiName, candidate) {
...
const filePathUrl = (file)=>`https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(file.replace(/^File:/i, '').replace(/ /g, '_'))}`;
...
async function viaWikidataId(p) {
  ...
  if (img) return {
    url: img.startsWith('http://') ? 'https://' + img.slice(7) : img,
    source: 'wikidata-id-p18',
    score: 1
  };
  ...
  const listUrl = 'https://commons.wikimedia.org/w/api.php?origin=*&format=json&action=query' + '&list=categorymembers&cmtype=file&cmlimit=5&cmtitle=' + encodeURIComponent('Category:' + category);
  const cm = await getJson(listUrl);
  const members = cm?.query?.categorymembers || [];
  for (const m of members){
    const title = m.title || '';
    if (!/\.(jpe?g|png|tiff?|webp)$/i.test(title)) continue;
    return {
      url: filePathUrl(title),
      source: 'wikidata-id-commons-cat',
      score: 0.9
    };
  }
  return null;
...
async function viaWikipedia(p) {
  const langs = [
    ...WIKI_LANGS[(p.country || '').toUpperCase()] || [],
    'en'
  ];
  const seen = new Set();
  for (const lang of langs){
    ...
    const url = `https://${lang}.wikipedia.org/w/api.php?action=query&format=json&origin=*` + `&generator=search&gsrsearch=${q}&gsrlimit=3&gsrnamespace=0` + `&prop=pageimages|coordinates&piprop=name&colimit=3`;
    const data = await getJson(url);
    ...
    for (const pg of Object.values(pages)){
      ...
      if (hasCo) {
        ...
        const d = haversine(p.lat, p.lng, co.lat, co.lon);
      ...
      if (ok) {
        return {
          url: filePathUrl(file),
          source: `wikipedia:${lang}`,
          score: Math.min(1, score + (hasCo ? 0.2 : 0))
        };
      }
    }
  }
...
async function viaCommonsGeo(p) {
  const url = `https://commons.wikimedia.org/w/api.php?action=query&format=json&origin=*` + `&generator=geosearch&ggscoord=${p.lat}|${p.lng}&ggsradius=${GEO_RADIUS_M}` + `&ggslimit=20&ggsnamespace=6`;
  const data = await getJson(url);
  const pages = data?.query?.pages;
...
async function findPhoto(p) {
  ...
  return await viaWikidataId(p) || await viaWikidata(p) || await viaWikipedia(p) || await viaCommonsGeo(p);
}
serve(async (req)=>{
  ...
  const budgetMs = Math.min(TIME_BUDGET_MS, Math.max(3_000, parseInt(params.get('budget_ms') || '', 10) || TIME_BUDGET_MS));
  const task = runBatch(sb, {
    dryRun,
    limit,
    budgetMs
  }).catch(async (e)=>{
    await sb.from('debug_log').insert({
      source: 'backfill-poi-photos',
      action: 'batch',
...
async function runBatch(sb, opts) {
  const t0 = Date.now();
  // Kolik bodů bez fotky ještě zbývá (informativně).
  const { count: remaining } = await sb.from('points_of_interest').select('id', {
    count: 'exact',
    head: true
  }).is('image_url', null).eq('is_active', true);
  // Kolik bodů se ještě NEZKOUŠELO (skutečná práce pro cron; když 0 → job končí).
  const { count: remainingUnchecked } = await sb.from('points_of_interest').select('id', {
    count: 'exact',
    head: true
  ...
  const { data, error } = await sb.from('points_of_interest').select('id, name, lat, lng, country, translations, wikidata_id').is('image_url', null).is('photo_checked_at', null).eq('is_active', true).order('sort_order', {
    ascending: true,
    nullsFirst: false
  }).order('id', {
    ascending: true
  ...
  await Promise.all(Array.from({
    length: Math.min(CONCURRENCY, pois.length)
  }, worker));
  const result = {
    dry_run: opts.dryRun,
    scanned: pois.length,
