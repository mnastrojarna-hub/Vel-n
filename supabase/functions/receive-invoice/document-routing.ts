...
export async function routeDocument(parsed, financialEventId, supabase) {
  try {
      ...
      case 'contract_purchase':
        {
          ...
          if (parsed.purchase?.serial_number) {
            const { data: moto } = await supabase.from('motorcycles').select('id').or(`vin.ilike.%${parsed.purchase.serial_number}%`).maybeSingle();
            if (moto) {
              await supabase.from('financial_events').update({
                linked_entity_type: 'motorcycle',
                linked_entity_id: moto.id
