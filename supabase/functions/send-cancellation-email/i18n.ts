...
<p>Ваш відгук для нас дуже важливий — залиште його на <a href="${v.google_review_url || '#'}" style="color:#2563eb">Google</a> або <a href="${v.facebook_review_url || '#'}" style="color:#2563eb">Facebook</a>.</p>
...
function renderModifiedBody(lang, v) {
  ...
  const priceMsgs = {
    cs: {
      ...
      minus: `K úpravě se vztahuje <strong>vrácení ${v.price_difference}</strong> formou dobropisu, který najdete v příloze. Refund jde zpět na původní platební kartu.`
    },
    en: {
      plus: `An additional payment of <strong>${v.price_difference}</strong> applies. The payment receipt will follow after payment.`,
      minus: `A refund of <strong>${v.price_difference}</strong> applies as a credit note (attached). The amount returns to your original card.`
    },
    de: {
      plus: `Es fällt eine Nachzahlung von <strong>${v.price_difference}</strong> an. Der Beleg folgt nach Zahlungseingang.`,
      minus: `Eine Rückerstattung von <strong>${v.price_difference}</strong> wird als Gutschrift (Anhang) erstattet — Betrag geht zurück auf Ihre ursprüngliche Karte.`
    },
    nl: {
      plus: `Er volgt een bijbetaling van <strong>${v.price_difference}</strong>. Het betalingsbewijs komt na betaling.`,
      minus: `Een terugbetaling van <strong>${v.price_difference}</strong> wordt verwerkt als creditnota (bijlage) — bedrag terug op je oorspronkelijke kaart.`
    },
    es: {
      plus: `Se aplica un pago adicional de <strong>${v.price_difference}</strong>. El comprobante llegará tras el pago.`,
      minus: `Se aplica un reembolso de <strong>${v.price_difference}</strong> mediante nota de crédito (adjunta) — el importe vuelve a tu tarjeta original.`
    },
    fr: {
      plus: `Un supplément de <strong>${v.price_difference}</strong> s'applique. Le justificatif suivra après paiement.`,
      minus: `Un remboursement de <strong>${v.price_difference}</strong> est émis sous forme d'avoir (joint) — la somme retourne sur votre carte d'origine.`
    },
    pl: {
      plus: `Wymagana jest dopłata <strong>${v.price_difference}</strong>. Potwierdzenie płatności nadejdzie po płatności.`,
      minus: `Zwrot <strong>${v.price_difference}</strong> w formie noty kredytowej (załącznik) — środki wracają na pierwotną kartę.`
    },
    uk: {
      plus: `Потрібна доплата <strong>${v.price_difference}</strong>. Підтвердження оплати надійде після платежу.`,
      minus: `Повернення <strong>${v.price_difference}</strong> у формі кредит-ноти (у вкладенні) — кошти повертаються на початкову картку.`
    }
  };
  const attachInfo = {
