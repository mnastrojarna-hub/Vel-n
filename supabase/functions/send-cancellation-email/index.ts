...
import { renderEmail, normalizeLang, helpCardLabels } from './i18n.ts';
...
function webLabelForLang(lang) {
  return lang === 'cs' ? 'www.motogo24.cz' : 'motogo24.com';
}
/** URL odkazy motogo24.cz → zákazníkova doména (jen non-cs; e-mail se nemění). */ function localizeBodyLinks(html, lang) {
...
async function translateEmailTemplate(subjectCz, bodyCz, lang) {
...
async function resolveTemplateForLang(supabase, tpl, lang) {
...
serve(async (req)=>{
  ...
  try {
    ...
    let { booking_id, customer_email, customer_name, motorcycle, start_date, end_date, cancellation_reason, cancelled_by_source, refund_amount, refund_percent, source = 'app', language } = reqBody;
    // If no email provided but booking_id exists, look up from profile
    if (!customer_email && booking_id) {
      try {
        const { data: bk } = await supabase.from('bookings').select('user_id, profiles(full_name, email), motorcycles!moto_id(model)').eq('id', booking_id).single();
        if (bk?.profiles) {
          customer_email = bk.profiles.email;
          if (!customer_name) customer_name = bk.profiles.full_name;
          if (!motorcycle) motorcycle = bk.motorcycles?.model;
        }
      } catch  {}
    }
    if (!customer_email) {
    ...
    if (booking_id && !force) {
      try {
        const { data: existing } = await supabase.from('sent_emails').select('id, sent_at, status').eq('booking_id', booking_id).eq('template_slug', 'booking_cancelled').eq('status', 'sent').gte('sent_at', new Date(Date.now() - 30 * 60 * 1000).toISOString()).limit(1);
        if (existing?.length) {
          await supabase.from('debug_log').insert({
            source: 'send-cancellation-email',
            ...
            request_data: {
              booking_id,
              existing_email_id: existing[0].id,
              existing_sent_at: existing[0].sent_at
            }
          }).then(()=>{}, ()=>{});
          return new Response(JSON.stringify({
            success: true,
    ...
    const skipRefund = reqBody.skip_refund === true;
    if (booking_id) {
      ...
      var manualRefund = reqBody.manual_refund === true;
      try {
        // Load booking data for refund calculation
        const { data: booking } = await supabase.from('bookings').select('start_date, end_date, total_price, payment_status, stripe_payment_intent_id, booking_source, motorcycles!moto_id(model)').eq('id', booking_id).single();
        if (booking) {
          if (!start_date) start_date = booking.start_date;
          if (!end_date) end_date = booking.end_date;
          if (!motorcycle) motorcycle = booking.motorcycles?.model || '';
          ...
          if (refund_amount > 0) {
            ...
            for(let attempt = 0; attempt < 3 && !attached; attempt++){
              try {
                const { data: cnRows } = await supabase.from('invoices').select('id, number, pdf_path').eq('booking_id', booking_id).or('type.eq.credit_note,and(type.eq.payment_receipt,source.eq.cancellation)').neq('status', 'cancelled').order('created_at', {
                  ascending: false
                }).limit(1);
                const cn = cnRows?.[0];
                if (cn?.pdf_path) {
                  const path = cn.pdf_path;
                  const b64 = await downloadAsBase64(supabase, path);
                  if (b64) {
                    const ext = path.toLowerCase().endsWith('.pdf') ? 'pdf' : 'html';
                    attachments.push({
                      content: b64,
                      filename: `Dobropis-${cn.number || 'DB'}.${ext}`,
                      storage_path: path
                    });
                    attached = true;
                    await supabase.from('debug_log').insert({
                ...
                console.warn('Credit note lookup attempt', attempt, 'failed:', e.message);
              }
            }
            if (!attached) {
              await supabase.from('debug_log').insert({
        ...
        console.warn('Auto-refund lookup failed:', e);
      }
    }
    const vars = {
    ...
    for (const trySlug of slugsToTry){
      const { data: tpl } = await supabase.from('email_templates').select('slug, name, subject, body_html, active, attachments, subject_translations, body_translations').eq('slug', trySlug).eq('active', true).maybeSingle();
      if (tpl?.body_html) {
    ...
    if (dbAttachmentsList.includes('Dobropis') && booking_id && !attachments.some((a)=>/dobropis|credit/i.test(a.filename))) {
      try {
        const { data: cn } = await supabase.from('invoices').select('id, number, pdf_path, type').eq('booking_id', booking_id).or('type.eq.credit_note,and(type.eq.payment_receipt,source.eq.cancellation)').neq('status', 'cancelled').order('created_at', {
          ascending: false
        }).limit(1);
        if (cn?.length && cn[0].pdf_path) {
          ...
          if (b64) {
            ...
            attachments.push({
              content: b64,
              filename: `Dobropis-${cn[0].number || 'DB'}.${ext}`,
    ...
    if (custLang !== 'cs') {
      ...
      if (csI18n) {
        csSubject = csI18n.subject;
        csTemplateHtml = csI18n.body;
      }
      if (csTemplateHtml) {
    ...
    try {
      await supabase.from('sent_emails').insert({
        ...
        attachments_meta: attachments.length ? attachments.map((a)=>({
            filename: a.filename,
            storage_path: a.storage_path || null
          })) : null
      });
