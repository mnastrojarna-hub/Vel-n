...
async function handleListMotorcycles(req) {
  ...
  let q = sb.from('motorcycles').select('id, model, brand, category, engine_cc, power_kw, torque_nm, weight_kg, fuel_tank_l, seat_height_mm, has_abs, license_required, color, year, ideal_usage, image_url, images, branch_id, price_mon, price_tue, price_wed, price_thu, price_fri, price_sat, price_sun, min_rental_days, max_rental_days, status, branches(name, address, city)').eq('status', 'active').order('model');
...
async function handleMotorcycleDetail(id) {
  const { data, error } = await sb.from('motorcycles').select('*, branches(id, name, address, city, latitude, longitude)').eq('id', id).maybeSingle();
  if (error) return jsonResponse({
    error: error.message
  }, 500);
