...
/** Načte data z DB dle typu a filtrů. */ async function loadExportData(admin, dataType, filters) {
  ...
  switch(dataType){
    case 'bookings':
      {
        let query = admin.from('bookings').select('id, status, payment_status, payment_method, start_date, end_date, total_price, created_at, profiles!inner(full_name, email, phone), motorcycles!inner(model, license_plate)').order('created_at', {
          ascending: false
        });
    ...
    case 'motorcycles':
      {
        let query = admin.from('motorcycles').select('id, model, license_plate, vin, year, category, status, price_per_day, next_service_date, branches!inner(name)').order('model');
    ...
    case 'customers':
      {
        const { data } = await admin.from('profiles').select('id, full_name, email, phone, address, license_group, created_at').order('created_at', {
          ascending: false
        }).limit(5000);
        return {
          columns: [
    ...
    case 'invoices':
      {
        let query = admin.from('invoices').select('id, number, type, customer_name, customer_dic, total_amount, vat_amount, status, issue_date, due_date').order('issue_date', {
          ascending: false
        });
    ...
    case 'accounting':
      {
        let query = admin.from('accounting_entries').select('id, type, amount, category, description, date, payment_method, reference_type, branches!inner(name)').order('date', {
          ascending: false
        });
    ...
    case 'inventory':
      {
        let query = admin.from('inventory').select('id, name, sku, stock, min_stock, unit, unit_price, supplier, branch_id').order('name');
        if (branchId) query = query.eq('branch_id', branchId);
        const { data } = await query.limit(5000);
        return {
          columns: [
    ...
    case 'sos':
      {
        let query = admin.from('sos_incidents').select('id, type, status, latitude, longitude, description, is_fault, created_at, profiles!inner(full_name, phone), bookings!inner(motorcycles!inner(model, license_plate))').order('created_at', {
          ascending: false
        });
...
/** Konvertuje data do CSV formátu. */ function toCsv(columns, rows) {
...
/** Konvertuje data do XML formátu. */ function toXml(dataType, rows) {
  const items = rows.map((row)=>{
    const fields = Object.entries(row).map(([k, v])=>`    <${k}>${escapeXml(String(v ?? ''))}</${k}>`).join('\n');
    return `  <item>\n${fields}\n  </item>`;
  }).join('\n');
      ...
      case 'xlsx':
        {
          ...
          const content = body.format === 'pdf' ? toCsv(columns, rows) // Zjednodušeno — v produkci by se použilo PDF API
          ...
          const { error: uploadError } = await admin.storage.from('documents').upload(fileName, fileBytes, {
