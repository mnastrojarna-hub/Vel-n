...
function decodeEntities(s: string): string {
  return s
    ...
    .replace(/&nbsp;/gi, ' ').replace(/&[a-z]+;/gi, ' ').trim()
}

async function analyzePage(path: string) {
  ...
  try {
    const res = await fetch(url, {
      signal: ctrl.signal,
      redirect: 'follow',
      headers: { 'User-Agent': 'MotoGo24-SEO-Check/1.0' },
    })
    clearTimeout(tm)
    ...
    if (!res.ok) {
      return { path, statusCode, redirected, finalUrl, error: `HTTP ${statusCode}` }
    }
...
serve(async (req) => {
  ...
  if (paths.length === 0) {
    return new Response(JSON.stringify({ error: 'No valid paths' }), { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
  }
  ...
  for (let i = 0; i < paths.length; i += concurrency) {
    ...
    settled.forEach(r => { results[r.path] = r })
  }
  return new Response(JSON.stringify({ results, baseUrl: BASE, checkedAt: new Date().toISOString() }), {
