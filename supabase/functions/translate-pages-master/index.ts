...
async function fetchMaster(masterUrls, sb, page) {
  const errors = [];
  ...
  try {
    ...
    const snapPages = snap && typeof snap === 'object' && !Array.isArray(snap) ? snap.pages ?? snap : null;
    if (snapPages && typeof snapPages === 'object') {
      const pagesMap = snapPages;
      if (page) {
        if (pagesMap[page]) return {
          pages: {
            [page]: pagesMap[page]
          },
          source: 'app_settings.pages_master_cs'
        };
      } else {
        return {
          pages: pagesMap,
    ...
    errors.push(`app_settings.pages_master_cs: ${e.message}`);
  }
  ...
  const bundledPages = MASTER_CS_BUNDLED.pages || {};
  if (page) {
    if (bundledPages[page]) return {
      pages: {
        [page]: bundledPages[page]
      },
      source: 'bundled'
    };
...
async function translateTree(tree, lang) {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (body.master_url) {
      ...
      if (settingMasterUrl) masterUrls = [
        String(settingMasterUrl)
      ];
    }
    const targetLangs = Array.isArray(body.target_langs) && body.target_langs.length ? body.target_langs.filter((l)=>LANG_NAMES[l]) : DEFAULT_LANGS;
    if (action === 'list') {
      const { pages, source } = await fetchMaster(masterUrls, sb);
      const wantPages = Array.isArray(body.pages) && body.pages.length ? body.pages : null;
      const pageKeys = wantPages ? wantPages.filter((p)=>pages[p]) : Object.keys(pages);
      const queue = [];
      for (const p of pageKeys)for (const l of targetLangs)queue.push({
        page: p,
        lang: l
      });
      return jsonResponse({
    ...
    if (e instanceof AnthropicError) {
      ...
      const httpStatus = e.code === 'insufficient_credits' ? 402 : e.code === 'invalid_api_key' ? 401 : e.code === 'rate_limit' ? 429 : 502;
