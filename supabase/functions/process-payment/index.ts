...
import { stripe, SITE_URL, PRODUCT_NAMES, CORS, getOrCreateStripeCustomer } from './stripe-customer.ts';
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (body.action === 'verify_setup_session') {
      ...
      try {
        ...
        const bookingId = md.booking_id || session.client_reference_id || null;
        if (!bookingId) {
          return new Response(JSON.stringify({
            success: false,
            error: 'No booking_id in session'
          }), {
            status: 400,
            headers: {
              ...CORS,
        ...
        const { data: bk } = await supabaseAdmin.from('bookings').select('payment_status').eq('id', bookingId).single();
        ...
        if (rpcErr) {
          try {
            await supabaseAdmin.from('debug_log').insert({
              ...
              action: 'verify_setup_session_failed',
              component: 'free_booking',
              status: 'error',
        ...
        const wasAlreadyPaid = !!confirmData?.was_already_paid;
        try {
          await supabaseAdmin.from('debug_log').insert({
            source: 'process-payment',
            action: 'verify_setup_session_confirmed',
            component: 'free_booking',
            status: 'ok',
            request_data: {
              session_id: sessionId,
        ...
        if (!wasAlreadyPaid) {
          try {
            const { data: booking } = await supabaseAdmin.from('bookings').select('booking_source, start_date, end_date, total_price, motorcycles!moto_id(model, manual_url), profiles(full_name, email)').eq('id', bookingId).single();
            const profile = booking?.profiles ?? null;
            if (profile?.email) {
              const moto = booking?.motorcycles ?? null;
            ...
            try {
              await supabaseAdmin.from('debug_log').insert({
                source: 'process-payment',
                action: 'verify_setup_session_mail_failed',
                component: 'free_booking',
                status: 'error',
    ...
    if (body.action === 'verify_shop_session') {
      const sessionId = body.session_id;
      ...
      try {
        ...
        if (!sid && orderId) {
          const { data: ord } = await supabaseAdmin.from('shop_orders').select('stripe_session_id').eq('id', orderId).maybeSingle();
          sid = ord?.stripe_session_id || null;
        }
        if (!sid) {
          return new Response(JSON.stringify({
            success: false,
            error: 'No session to verify'
          }), {
            headers: {
              ...CORS,
              'Content-Type': 'application/json'
        ...
        const sessionOrderId = md.order_id || session.client_reference_id || null;
        if (sessionOrderId) {
        ...
        const freeSession = session.payment_status === 'no_payment_required' && (session.amount_total ?? 0) === 0 // web objednávka celá pokrytá slevou
        ;
        ...
        try {
          await supabaseAdmin.from('debug_log').insert({
            source: 'process-payment',
            action: 'verify_shop_session_confirmed',
        ...
        try {
          ...
          const { data: order } = await supabaseAdmin.from('shop_orders').select('customer_name, customer_email, order_number, status, total').eq('id', orderId).single();
          const { data: items } = await supabaseAdmin.from('shop_order_items').select('product_name').eq('order_id', orderId);
          const hasVoucherItem = (items || []).some((it)=>/voucher|poukaz/i.test(String(it.product_name || '')));
          let { data: vouchers } = await supabaseAdmin.from('vouchers').select('code, amount, valid_until').eq('order_id', orderId);
          if (hasVoucherItem && (!vouchers || vouchers.length === 0)) {
            try {
              await supabaseAdmin.rpc('regen_voucher_for_order', {
                p_order_id: orderId
              });
            } catch  {}
            const re = await supabaseAdmin.from('vouchers').select('code, amount, valid_until').eq('order_id', orderId);
            vouchers = re.data;
          }
          ...
          try {
            await supabaseAdmin.from('debug_log').insert({
              source: 'process-payment',
              action: 'verify_shop_session_postprocess_failed',
              component: 'voucher_checkout',
              status: 'error',
              error_message: e.message,
              request_data: {
    ...
    if (body.action === 'create_sos_payment_link') {
      const supabaseAdmin = createClient(Deno.env.get('SUPABASE_URL'), Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'));
    ...
    if (body.source === 'web' && body.customer_email && body.type === 'shop') {
    ...
    const paymentType = type || 'booking';
    const paymentMode = mode || 'intent';
    const explicitSuccessUrl = body.success_url;
    const explicitCancelUrl = body.cancel_url;
    // Validate required fields
    if ((paymentType === 'booking' || paymentType === 'extension' || paymentType === 'sos') && !booking_id) {
      return new Response(JSON.stringify({
        success: false,
        error: `Missing booking_id for ${paymentType} payment`
      }), {
        status: 400,
        headers: {
          ...CORS,
    ...
    if (paymentType === 'booking' && booking_id) {
      ...
      const { data: bk } = await tmpSupabase.from('bookings').select('payment_status, status').eq('id', booking_id).single();
      if (bk?.payment_status === 'paid') {
        return new Response(JSON.stringify({
          success: false,
          error: 'Tato rezervace je již zaplacena.'
        }), {
          status: 409,
          headers: {
            ...CORS,
    ...
    if (paymentType === 'shop' && !order_id) {
      return new Response(JSON.stringify({
        success: false,
        error: 'Missing order_id for shop payment'
      }), {
        status: 400,
        headers: {
          ...CORS,
    ...
    if (paymentType === 'shop') {
      const json = (status, payload)=>new Response(JSON.stringify({
          success: false,
          ...payload
        }), {
          status,
      ...
      if (who.kind === 'none') {
        return json(401, {
          error: 'Přihlášení chybí nebo vypršelo — přihlaste se prosím znovu.',
          code: 'auth_required'
        });
      }
      const sbShop = createClient(Deno.env.get('SUPABASE_URL') ?? '', Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '');
      const { data: ord } = await sbShop.from('shop_orders').select('id, customer_id, payment_status, total').eq('id', order_id).maybeSingle();
      if (!ord) return json(404, {
        error: 'Objednávka nenalezena.',
        code: 'order_not_found'
      });
      const owner = who.kind === 'service' || who.kind === 'admin' || who.kind === 'user' && !!who.userId && who.userId === ord.customer_id;
      if (!owner) return json(403, {
        error: 'Tuto objednávku nelze zaplatit z tohoto účtu.',
        code: 'forbidden'
      });
      if (ord.payment_status === 'paid') return json(409, {
        error: 'Objednávka je už zaplacená.',
        code: 'already_paid'
      });
      if (ord.payment_status !== 'pending') return json(409, {
        error: 'Tuto objednávku už nelze zaplatit.',
        code: 'wrong_status'
      });
      const serverCzk = Math.round(Number(ord.total) || 0);
      if (serverCzk <= 0) return json(409, {
        error: 'Objednávka nevyžaduje platbu.',
        code: 'no_payment_needed'
      });
      if (serverCzk < 15) return json(400, {
        error: 'Minimální částka platby kartou je 15 Kč.',
        code: 'below_minimum'
      });
    ...
    if (paymentType === 'extension' && booking_id && change && typeof change === 'object') {
      ...
      let cur = null;
      try {
        const { data, error } = await supabase.from('bookings').select('user_id, status, total_price, trailer_moto_id, moto_id, start_date, end_date, pickup_time, motorcycles!moto_id(is_trailer)').eq('id', booking_id).maybeSingle();
        if (error) dryErr = 'validation_unavailable';
        else if (!data) dryErr = 'booking_not_found';
        else cur = data;
      } catch (_e) {
        dryErr = 'validation_unavailable';
      }
      ...
      if (!dryErr && cur) {
        ...
        if (who.kind === 'none') {
          ...
          return new Response(JSON.stringify({
            success: false,
            error: 'Přihlášení chybí nebo vypršelo — přihlaste se prosím znovu a úpravu zopakujte.',
            code: 'auth_required'
          }), {
            status: 401,
            headers: {
        ...
        const owner = who.kind === 'service' || who.kind === 'admin' || who.kind === 'user' && !!who.userId && who.userId === cur.user_id;
        if (!owner) {
          return new Response(JSON.stringify({
            success: false,
            error: 'Tuto rezervaci nelze upravit z tohoto účtu. Přihlaste se prosím účtem, kterým byla vytvořena.',
            code: 'forbidden'
          }), {
            status: 403,
            headers: {
      ...
      if (!dryErr && cur && [
        'cancelled',
        'completed'
      ].includes(String(cur.status || ''))) dryErr = 'wrong_status';
      ...
      if (!dryErr && cur && base && typeof base === 'object') {
        if (base.s && day(base.s) !== day(cur.start_date) || base.e && day(base.e) !== day(cur.end_date) || base.t !== undefined && hm(base.t) !== hm(cur.pickup_time) || base.p != null && Math.round(Number(base.p)) !== Math.round(Number(cur.total_price || 0))) {
          return new Response(JSON.stringify({
            success: false,
            error: 'Rezervace se mezitím změnila (jiné zařízení nebo web). Načtěte ji prosím znovu a úpravu zopakujte.',
            code: 'stale_booking'
          }), {
            status: 409,
            headers: {
        ...
        try {
          ...
          if (sw && typeof sw === 'object' && sw.m && sw.d) {
            ...
            if (typeof c.moto_id === 'string' && c.moto_id && c.moto_id !== cur.moto_id && cur.trailer_moto_id) {
              const { data: selfSvc, error: selfErr } = await supabase.rpc('moto_is_self_service', {
                p_moto_id: c.moto_id
              });
          ...
          dryErr = dryErr || 'validation_unavailable';
        }
      }
      ...
      if (!dryErr && cur) {
        try {
          const ns = c.p_new_start ?? c.start_date;
          const ne = c.p_new_end ?? c.end_date;
          if ((ns || ne) && !c._swap) {
            const mt = Array.isArray(cur.motorcycles) ? cur.motorcycles[0] : cur.motorcycles;
            const units = [
              cur.trailer_moto_id,
              mt?.is_trailer === true ? cur.moto_id : null
            ].filter((u)=>typeof u === 'string' && u.length > 0);
            for (const unit of units){
              const { data: busy, error: busyErr } = await supabase.rpc('trailer_unit_busy', {
                p_unit: unit,
                p_start: day(ns ?? cur.start_date),
                p_end: day(ne ?? cur.end_date),
                p_exclude: booking_id
              });
              if (busyErr || typeof busy !== 'boolean') {
                dryErr = 'trailer_check_unavailable';
                break;
              }
              if (busy) {
                dryErr = 'trailer_unavailable';
                break;
              }
            }
          }
      ...
      if (expected != null && Math.abs(Number(amount) - expected) > 1) {
        return new Response(JSON.stringify({
          success: false,
          error: `Částka doplatku neodpovídá výpočtu serveru (${expected} Kč). Obnovte stránku a zkuste znovu.`,
          code: 'amount_mismatch',
          expected
        }), {
          status: 400,
    ...
    if (amount <= 0 && booking_id) {
      const { data: dbBooking } = await supabase.from('bookings').select('total_price, payment_status, promo_code_id, voucher_id, discount_amount').eq('id', booking_id).single();
      ...
      if (dbBooking && dbBooking.total_price > 0) {
        return new Response(JSON.stringify({
          success: false,
          error: 'Částka neodpovídá ceně rezervace (' + dbBooking.total_price + ' Kč). Obnovte stránku a zkuste znovu.'
        }), {
          status: 400,
          headers: {
            ...CORS,
      ...
      if (dbBooking?.promo_code_id) {
        const { data: promo } = await supabase.from('promo_codes').select('type, value').eq('id', dbBooking.promo_code_id).single();
      ...
      if (!isTrue100 && dbBooking?.voucher_id) {
        const { data: voucher } = await supabase.from('vouchers').select('amount').eq('id', dbBooking.voucher_id).single();
      ...
      if (!isTrue100) {
        console.error('Free booking rejected — discount is not truly 100%:', {
          ...
          promo_code_id: dbBooking?.promo_code_id
        });
        return new Response(JSON.stringify({
          success: false,
      ...
      if (rpcError) {
        ...
        return new Response(JSON.stringify({
          success: false,
          error: 'Potvrzení rezervace selhalo. Zkuste to znovu.'
        }), {
          status: 500,
          headers: {
            ...CORS,
    ...
    if (paymentMode === 'intent') {
      ...
      if (payment_method_id && customerId) {
        ...
        try {
          ...
          try {
            if (booking_id) await supabase.from('bookings').update({
              stripe_payment_intent_id: intent.id
            }).eq('id', booking_id);
            if (order_id) await supabase.from('shop_orders').update({
              stripe_payment_intent_id: intent.id
            }).eq('id', order_id);
          } catch  {}
          try {
            await supabase.from('debug_log').insert({
              source: 'process-payment',
              action: 'stripe_off_session_attempt',
              component: paymentType,
              status: 'ok',
              request_data: {
                booking_id,
          ...
          const stripeErr = e;
          const piFromError = stripeErr.raw?.payment_intent;
          try {
            await supabase.from('debug_log').insert({
              source: 'process-payment',
              action: 'stripe_off_session_failed',
              component: paymentType,
              status: 'error',
              request_data: {
                booking_id,
          ...
          if (stripeErr.code === 'authentication_required' && piFromError?.client_secret) {
            return new Response(JSON.stringify({
              ...
              payment_intent_id: piFromError.id,
              amount,
              currency: currency || 'czk'
            }), {
              status: 200,
