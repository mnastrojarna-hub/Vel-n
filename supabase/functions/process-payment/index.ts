  ...
  try {
    ...
    if (paymentType === 'extension' && booking_id && change && typeof change === 'object') {
      ...
      if (base && typeof base === 'object') {
        try {
          const { data: cur } = await supabase.from('bookings').select('start_date, end_date, pickup_time, total_price').eq('id', booking_id).maybeSingle();
          const day = (v)=>String(v || '').slice(0, 10);
          const hm = (v)=>v == null ? '' : String(v).slice(0, 5);
          if (cur && (base.s && day(base.s) !== day(cur.start_date) || base.e && day(base.e) !== day(cur.end_date) || base.t !== undefined && hm(base.t) !== hm(cur.pickup_time) || base.p != null && Math.round(Number(base.p)) !== Math.round(Number(cur.total_price || 0)))) {
            return new Response(JSON.stringify({
              success: false,
      ...
      try {
        ...
        if (sw && typeof sw === 'object' && sw.m && sw.d) {
          ...
          const { data: curB } = await supabase.from('bookings').select('total_price').eq('id', booking_id).maybeSingle();
      ...
      if (dryErr) {
        return new Response(JSON.stringify({
          success: false,
          error: `Změnu nelze aplikovat (${dryErr}) — platba doplatku zrušena. Obnovte stránku a zkuste znovu.`,
          code: dryErr
        }), {
          status: 409,
          headers: {
