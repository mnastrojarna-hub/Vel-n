...
import { parseV2Result, extractLicenseCategory, getModelId, debugLog, pollForResult, MINDEE_ENQUEUE_URL } from './mindee-parser.ts';
...
serve(async (req)=>{
  ...
  try {
    ...
    console.log('[scan-document] Invoked: document_type=' + (document_type || 'id') + ' user_id=' + (user_id || 'anon') + ' image_len=' + (image_base64 ? image_base64.length : 0) + ' MINDEE_KEY_SET=' + !!MINDEE_API_KEY);
    ...
    if (!modelId) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: `Model ID not configured for type '${docType}' — set MINDEE_MODEL_ID_ID, MINDEE_MODEL_ID_DL, MINDEE_MODEL_ID_PASSPORT in Supabase Secrets`
      }), {
        status: 500,
        headers: {
          ...CORS,
      ...
      try {
        ...
        if (resp.ok) {
          ...
          await debugLog('mindee_enqueue', docType, 'success', {
            ...
            enqueue_response: enqueueResult,
            response_keys: Object.keys(enqueueResult)
          });
          break;
        } else {
      ...
      if (maybeResult && typeof maybeResult === 'object' && Object.keys(maybeResult).length > 0) {
        ...
        if (prediction && typeof prediction === 'object' && Object.keys(prediction).length > 0) {
          ...
          await debugLog('mindee_ocr', docType, 'success', {
            ...
            fields_found: Object.keys(data).length,
    ...
    if (docType === 'dl') {
      ...
      console.log('[scan-document] DL fields: licenseNumber=' + (data.licenseNumber || 'null') + ', licenseCategory=' + (data.licenseCategory || 'null') + ', idNumber=' + (data.idNumber || 'null'));
    }
    const durationMs = Date.now() - startTime;
    await debugLog('mindee_ocr', docType, 'success', {
      document_type: docType,
