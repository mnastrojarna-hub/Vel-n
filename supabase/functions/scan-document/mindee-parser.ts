...
export function parseV2Result(result, docType) {
  const data = {};
  ...
  if (surnames) {
    if (Array.isArray(surnames)) {
      data.lastName = surnames.map((s)=>typeof s === 'string' ? s : extractField(s)).join(' ');
    } else {
  ...
  if (givenNames) {
    if (Array.isArray(givenNames)) {
      data.firstName = givenNames.map((g)=>typeof g === 'string' ? g : extractField(g)).join(' ');
    } else {
  ...
  const expiry = getDateVal('expiry_date') || getDateVal('expiration_date') || getDateVal('date_of_expiry') || getDateVal('valid_until') || getDateVal('validity_end') || getDateVal('end_of_validity');
  if (expiry) data.expiryDate = expiry;
  const issued = getDateVal('issue_date') || getDateVal('issuance_date') || getDateVal('date_of_issue') || getDateVal('valid_from');
  ...
  if (docType === 'dl') {
    ...
    const dlNum = getVal('license_number') || getVal('permit_number') || getVal('card_number') || getVal('dl_number') || getVal('driving_license_number') || getVal('licence_number') || data.idNumber || '';
    if (dlNum) {
...
export function parseCzechAddress(raw) {
  const out = {};
  ...
  if (zipMatch) {
    ...
    withoutZip = trimmed.slice(0, zipMatch.index) + '|' + trimmed.slice(zipMatch.index + zipMatch[0].length);
  }
...
export function extractLicenseCategory(data, rawText) {
  const categories = [
    'AM',
    'A1',
    'A2',
...
export function getModelId(docType, modelIdId, modelIdDl, modelIdPassport) {
  switch(docType){
    case 'dl':
      return modelIdDl;
    case 'passport':
      return modelIdPassport;
    case 'id':
    default:
      return modelIdId;
  }
...
export async function debugLog(action, component, status, requestData, responseData, errorMessage) {
  try {
...
// Mindee v2 polling
export async function pollForResult(jobId, mindeeApiKey, pollingUrl, maxAttempts = 25) {
