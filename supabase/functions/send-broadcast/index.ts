...
;
/** Replace {{var}} placeholders in template */ function renderTemplate(template, vars) {
  if (!template) return '';
  let result = template;
...
/** Send email via Resend with retry */ async function sendEmail(to, subject, html) {
...
/** Send SMS/WA via send-message edge function (internal call) */ async function sendSmsOrWa(channel, to, body, customerId, campaignId, templateSlug, templateVars) {
  try {
...
serve(async (req)=>{
  ...
  try {
    ...
    // 1. Load campaign
    const { data: campaign, error: campErr } = await supabase.from('broadcast_campaigns').select('*, message_templates(name, slug, body_template, subject_template, wa_template_id)').eq('id', campaign_id).in('status', [
      'draft',
      'scheduled'
    ]).single();
    if (campErr || !campaign) {
      return new Response(JSON.stringify({
        error: 'Campaign not found or not in draft/scheduled status'
      }), {
        status: 404,
        headers: {
          ...CORS,
    ...
    const templateVars = campaign.template_vars || {};
    ...
    const channelConsentCol = channel === 'email' ? 'consent_email' : channel === 'whatsapp' ? 'consent_whatsapp' : 'consent_sms';
    let query = supabase.from('profiles').select('id, full_name, email, phone').eq('marketing_consent', true).eq(channelConsentCol, true).not(contactField, 'is', null);
    if (segment === 'vip') {
      query = query.or('reliability_score->>score.gt.80');
    } else if (segment === 'past_customers') {
      const { data: bookingUsers } = await supabase.from('bookings').select('user_id').eq('status', 'completed');
      const ids = [
        ...new Set((bookingUsers || []).map((b)=>b.user_id))
      ].filter(Boolean);
      if (ids.length === 0) {
        await supabase.from('broadcast_campaigns').update({
          status: 'completed',
      ...
      const { data: allBookingUsers } = await supabase.from('bookings').select('user_id');
      const ids = [
        ...new Set((allBookingUsers || []).map((b)=>b.user_id))
      ].filter(Boolean);
      if (ids.length > 0) {
    ...
    for (const recipient of allRecipients){
      ...
      if ((sentCount + failedCount) % 50 === 0 && sentCount + failedCount > 0) {
        const { data: check } = await supabase.from('broadcast_campaigns').select('status').eq('id', campaign_id).single();
      ...
      if (channel === 'email') {
        ...
        // SMS or WhatsApp
        success = await sendSmsOrWa(channel, recipient.phone, renderedBody, recipient.id, campaign_id, templateSlug, perRecipientVars);
      }
      if (success) {
        sentCount++;
      } else {
...
/** Build branded marketing HTML email — unified design (1:1 with invoice + screen) */ function buildMarketingHtml(body, subject) {
