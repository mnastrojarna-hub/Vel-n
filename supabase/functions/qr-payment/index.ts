...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: bk } = await sb.from('bookings').select('id, booking_source, payment_status, total_price, profiles(full_name, email)').eq('id', booking_id).maybeSingle();
    if (!bk) return json({
      success: false,
      error: 'not_found'
    }, 404);
    if (!surcharge) {
      if (bk.booking_source !== 'web') return json({
        success: false,
        error: 'not_web_booking'
      }, 400);
      if (bk.payment_status === 'paid') return json({
        success: false,
        error: 'already_paid'
      }, 400);
    }
    ...
    const { data: rpc, error: rpcErr } = await sb.rpc(surcharge ? 'set_booking_mod_surcharge' : 'set_booking_qr_payment', {
      p_booking_id: booking_id
    });
    if (rpcErr || !rpc || rpc.success === false) {
      return json({
        success: false,
        error: rpc?.error || rpcErr?.message || 'rpc_failed'
      }, 400);
    }
    ...
    if (!surcharge) try {
      ...
      const { data: existing } = await sb.from('invoices').select('number, pdf_path').eq('booking_id', booking_id).in('type', [
        'advance',
        'proforma'
      ...
      if (existing?.[0]?.pdf_path) {
        ...
        const gi = await fetch(SUPABASE_URL + '/functions/v1/generate-invoice', {
          ...
          body: JSON.stringify({
            ...
            language: locale || 'cs',
            variable_symbol: vs,
            due_note: 'do 4 hodin od vystavení'
          })
        });
        const gj = await gi.json().catch(()=>({}));
        if (gj?.success) {
          invoiceNumber = gj.number;
          invoicePath = gj.pdf_path;
        } else console.warn('[qr-payment] generate-invoice:', JSON.stringify(gj));
      }
    } catch (e) {
      console.warn('[qr-payment] ZF gen failed:', e.message);
    }
    ...
    const custEmail = bk.profiles?.email || null;
    const custName = bk.profiles?.full_name || '';
    if (custEmail) {
      const attachment_paths = invoicePath ? [
        {
          filename: (invoiceNumber || 'zalohova-faktura') + '.pdf',
          path: invoicePath
        }
      ] : [];
      const subject = surcharge ? 'Doplatek k rezervaci #' + bookingNumber + ' — QR / bankovní převod' : 'Platební údaje k rezervaci #' + bookingNumber + ' — QR / bankovní převod';
      ...
      const postEmail = (payload)=>fetch(SUPABASE_URL + '/functions/v1/send-email', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: 'Bearer ' + SERVICE_KEY,
      ...
      try {
        ...
        console.warn('[qr-payment] template mail error:', e.message);
      }
      if (!sentOk) {
        try {
          await postEmail({
            to: custEmail,
            subject,
            raw_html: surcharge ? surchargeEmailHtml({
              custName,
              amount,
              account,
              iban,
          ...
          console.warn('[qr-payment] fallback mail failed:', e.message);
        }
      }
    }
    ...
    try {
      ...
      console.warn('[qr-payment] ops mail failed:', e.message);
    }
    return json({
      success: true,
      vs,
...
function opsEmailHtml(p) {
  ...
  <p style="margin:0 0 12px;color:#374151;font-size:14px">${p.surcharge ? 'Admin upravil zaplacenou rezervaci s doplatkem. Zkontrolujte prosím připsání na účtu a po přijetí doplatek potvrďte ve Velíně (detail rezervace → Potvrdit doplatek) — teprve pak zákazníkovi odejde potvrzení úpravy.' : 'Zákazník zvolil platbu QR / převodem. Zkontrolujte prosím připsání na účtu a po přijetí platbu potvrďte ve Velíně (rezervace → Potvrdit platbu → Bankovní převod).'}</p>
  ...
  ${p.surcharge ? '<p style="margin:0;color:#374151;font-weight:700;font-size:13px">Rezervace je jinak zaplacená — nic se automaticky neruší.</p>' : '<p style="margin:0;color:#b91c1c;font-weight:700;font-size:13px">Splatnost 4 h. Nezaplacená rezervace se pak automaticky zruší. Případnou pozdní/duplicitní platbu vraťte převodem zpět.</p>'}
  `;
}
