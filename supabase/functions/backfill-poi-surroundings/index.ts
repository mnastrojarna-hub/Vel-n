...
const CAT_LABEL = {
  food: 'restaurace / občerstvení',
  castle: 'hrad / zámek',
  lookout: 'rozhledna / vyhlídka',
  water: 'vodní plocha (přehrada/jezero/rybník)',
  spring: 'studánka / pramen / minerální vývěr',
  sights: 'památka / zajímavost',
  nature: 'přírodní lokalita',
  military: 'vojenská památka / opevnění',
  aviation: 'letecká památka / muzeum',
  tech: 'technická / průmyslová památka',
  moto: 'motoristická zajímavost (okruh, muzeum)',
  other: 'zajímavé místo'
};
...
function buildPrompt(row, catalog) {
  ...
  const parts = [
    ...
    needDesc ? `- "description": 3–4 věty (cca 55–90 slov) — poutavý, konkrétní popis samotného místa: co to je, čím je výjimečné, co tam návštěvník uvidí/zažije.` : '',
    '',
...
async function generate(row, catalog) {
  let res = null;
    ...
    res = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: buildPrompt(row, catalog)
          }
        ]
      })
    });
...
async function translate(table, id, fields) {
...
async function fetchMissing(db, table, limit) {
  const cols = table === 'points_of_interest' ? 'id, name, description, category, country, region, lat, lng' : 'id, name, description, lat, lng';
  ...
  const { data, error } = await db.from(table).select(cols).is('surroundings', null).order('image_url', {
    nullsFirst: false
  }).limit(limit);
  if (error) throw new Error(error.message);
  return data || [];
}
async function processTable(db, table, rows, concurrency) {
  const catalog = table === 'points_of_interest';
  let processed = 0, failed = 0;
  const reasons = {} // důvod selhání → počet (diagnostika)
  ;
  const note = (why)=>{
  ...
  for(let i = 0; i < rows.length; i += concurrency){
    ...
    await Promise.all(chunk.map(async (r)=>{
      try {
        ...
        failed++;
        note(String(e instanceof Error ? e.message : e).slice(0, 60));
      }
    }));
  }
  return {
 ...
 *  projekt může mít víc platných service klíčů / nový formát sb_secret_. */ // service_role se ověřuje sdíleným helperem (_shared/auth.ts): claim v payloadu
...
serve(async (req)=>{
  ...
  let ok = bearer.length > 0 && (bearer === SUPABASE_SERVICE_KEY || await isServiceRole(bearer));
  ...
  if (!ok) return jsonResponse({
    error: 'service_role required'
  }, 401);
  ...
  const budgetMs = Math.min(Math.max(Number(body.budget_ms) || 220_000, 20_000), 280_000);
  const runBatch = async ()=>{
    const t0 = Date.now();
    let table = 'points_of_interest';
    let totalP = 0, totalF = 0;
    const allReasons = {};
    const logResult = async (extra)=>{
      ...
      if (totalF > 0 || Object.keys(allReasons).length > 0) {
        await db.from('debug_log').insert({
          source: 'backfill-poi-surroundings',
          action: 'batch',
  ...
  if (wantWait === '1' || wantWait === 'true') return jsonResponse({
    success: true,
    ...await task
  });
