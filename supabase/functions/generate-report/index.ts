...
/** Generuje měsíční / roční report. */ async function generatePeriodReport(admin, periodFrom, periodTo, branchId) {
  // Příjmy
  let incomeQuery = admin.from('accounting_entries').select('amount, category, date').eq('type', 'income').gte('date', periodFrom).lte('date', periodTo);
  if (branchId) incomeQuery = incomeQuery.eq('branch_id', branchId);
  const { data: income } = await incomeQuery;
  // Výdaje
  let expenseQuery = admin.from('accounting_entries').select('amount, category, date').eq('type', 'expense').gte('date', periodFrom).lte('date', periodTo);
  if (branchId) expenseQuery = expenseQuery.eq('branch_id', branchId);
  const { data: expenses } = await expenseQuery;
  // Rezervace
  let bookingQuery = admin.from('bookings').select('id, status, total_price, start_date').gte('start_date', periodFrom).lte('start_date', periodTo);
  if (branchId) {
  ...
  return {
    ...
    avg_booking_value: bookings && bookings.length > 0 ? Math.round(bookings.reduce((s, b)=>s + Number(b.total_price ?? 0), 0) / bookings.length) : 0
  };
}
/** Generuje P&L (Výsledovka). */ async function generatePL(admin, periodFrom, periodTo, branchId) {
  const base = await generatePeriodReport(admin, periodFrom, periodTo, branchId);
  // Servisní náklady
  let maintenanceQuery = admin.from('maintenance_log').select('cost').gte('date', periodFrom).lte('date', periodTo);
...
/** Generuje ROI analýzu motorek. */ async function generateMotoROI(admin, periodFrom, periodTo, branchId) {
  let motoQuery = admin.from('motorcycles').select('id, model, license_plate, purchase_price, status, branch_id');
  if (branchId) motoQuery = motoQuery.eq('branch_id', branchId);
  const { data: motos } = await motoQuery;
  const results = [];
  for (const moto of motos ?? []){
    // Tržby z motorky
    const { data: bookings } = await admin.from('bookings').select('total_price').eq('moto_id', moto.id).eq('status', 'completed').gte('start_date', periodFrom).lte('start_date', periodTo);
    const revenue = bookings?.reduce((s, b)=>s + Number(b.total_price ?? 0), 0) ?? 0;
    // Náklady servisu
    const { data: maintenanceLogs } = await admin.from('maintenance_log').select('cost').eq('moto_id', moto.id).gte('date', periodFrom).lte('date', periodTo);
    ...
    results.push({
      ...
      roi_percent: purchasePrice > 0 ? Math.round((revenue - maintenanceCost) / purchasePrice * 100) : 0,
      ...
      case 'moto_roi':
        {
          ...
          const totalCost = details.reduce((s, d)=>s + d.maintenance_cost, 0);
          reportData = {
            type: body.type,
            period: {
              from: body.period_from,
