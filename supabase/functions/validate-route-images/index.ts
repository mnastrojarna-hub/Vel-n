...
const isWiki = (u)=>/wikimedia\.org|wikipedia\.org/i.test(u);
/** HEAD check — u Wikimedia s width=320 (ověří existenci souboru levně). */ async function urlAlive(url) {
  try {
    let u = url;
 ...
 *  Vrací Special:FilePath URL (konzistentní formát — appka si přidá ?width=). */ async function wikiPageImage(title) {
  for (const lang of [
    'cs',
    'en'
  ]){
    try {
      const api = `https://${lang}.wikipedia.org/w/api.php?action=query&format=json` + `&redirects=1&prop=pageimages&piprop=name&titles=${encodeURIComponent(title)}`;
...
serve(async (req)=>{
  ...
  /** Vrátí opravenou verzi seznamu fotek (mrtvé nahradí/vyřadí). */ async function repair(name, urls) {
    const out = [];
    let changed = false;
    for (const u of urls){
      if (!u) {
  ...
  if (scope === 'all' || scope === 'routes') {
    const { data: routes, error } = await db.from('routes').select('id, name, cover_image, images').order('created_at').range(offset, offset + limit - 1);
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
    ...
    for (const r of routes || []){
      ...
      details.push({
        ...
        name: r.name,
        action: 'updated',
        url: patch.cover_image ?? undefined
      });
      if (!dryRun) {
  ...
  if (scope === 'all' || scope === 'pois') {
    const { data: pois, error } = await db.from('route_pois').select('id, name, image_url, images').order('created_at').range(offset, offset + limit - 1);
    if (error) {
      return new Response(JSON.stringify({
        error: error.message
      }), {
