...
async function getSignKey() {
  ...
  cachedKey = await crypto.subtle.importKey('pkcs8', pemToPkcs8(SIGN_KEY_PEM), {
...
Deno.serve(async (req)=>{
  ...
  if (!SIGN_KEY_PEM) return json({
    error: 'sign_key_not_configured'
  }, 503);
  let body;
  try {
    body = await req.json();
  ...
  if (!expected) return json({
    error: 'token_not_configured'
  }, 503);
  if (!timingSafeEqual(expected, token)) return json({
    error: 'invalid_token'
  }, 403);
