 ...
 */ export function refundIsPositive(v) {
...
// =============================================================================
// Door codes / docs-required block (rendered into vars.door_codes_block)
// =============================================================================
// Vykresluje se v indexu (send-booking-email/index.ts). Když má rezervace
// uvolněné přístupové kódy → renderDoorCodesReleasedBlock. Když má ale kódy
// zadržené (chybí doklady) nebo žádné kódy ještě neexistují, ale rezervace je
// reserved/active a check_booking_docs_status hlásí chybějící doklady →
// renderDocsRequiredBlock vrátí výzvu k nahrání + zmínku o osobním ověření.
...
const DOOR_CODES_BLOCK_LABELS = {
  cs: {
    title: 'Přístupové kódy k pobočce',
    moto: 'Kód k motorce',
    gear: 'Kód šatny',
    valid: 'Kódy jsou platné po dobu trvání pronájmu.',
    validOne: 'Kód je platný po dobu trvání pronájmu.'
  },
  en: {
    title: 'Branch access codes',
    moto: 'Motorcycle code',
    gear: 'Locker room code',
    valid: 'Codes are valid for the rental period.',
    validOne: 'The code is valid for the rental period.'
  },
  de: {
    title: 'Filial-Zugangscodes',
    moto: 'Code zum Motorrad',
    gear: 'Code zur Umkleide',
    valid: 'Die Codes gelten für die Dauer der Buchung.',
    validOne: 'Der Code gilt für die Dauer der Buchung.'
  },
  nl: {
    title: 'Toegangscodes filiaal',
    moto: 'Motorcode',
    gear: 'Kleedkamercode',
    valid: 'De codes zijn geldig tijdens de huurperiode.',
    validOne: 'De code is geldig tijdens de huurperiode.'
  },
  es: {
    title: 'Códigos de acceso de sucursal',
    moto: 'Código de la moto',
    gear: 'Código del vestuario',
    valid: 'Los códigos son válidos durante el alquiler.',
    validOne: 'El código es válido durante el alquiler.'
  },
  fr: {
    title: 'Codes d\'accès succursale',
    moto: 'Code de la moto',
    gear: 'Code du vestiaire',
    valid: 'Les codes sont valables pendant la location.',
    validOne: 'Le code est valable pendant la location.'
  },
  pl: {
    title: 'Kody dostępu do oddziału',
    moto: 'Kod do motocykla',
    gear: 'Kod do szatni',
    valid: 'Kody są ważne przez czas trwania wypożyczenia.',
    validOne: 'Kod jest ważny przez czas trwania wypożyczenia.'
  },
  uk: {
    title: 'Коди доступу до філії',
    moto: 'Код до мотоцикла',
    gear: 'Код до роздягальні',
    valid: 'Коди дійсні протягом усього строку оренди.',
    validOne: 'Код дійсний протягом усього строку оренди.'
  }
};
export function renderDoorCodesReleasedBlock(lang, moto, gear) {
  const t = DOOR_CODES_BLOCK_LABELS[lang] || DOOR_CODES_BLOCK_LABELS.cs;
  const line = (label, code)=>`<p style="margin:4px 0;font-size:14px;font-weight:700;font-family:'Courier New',monospace;color:#0c4a6e">${label}: <span style="font-size:18px;letter-spacing:3px;color:#0369a1">${code}</span></p>`;
  return `
...
const DOCS_REQUIRED_BLOCK_LABELS = {
  cs: {
...
export function renderDocsRequiredBlock(lang, docsUrl) {
  const t = DOCS_REQUIRED_BLOCK_LABELS[lang] || DOCS_REQUIRED_BLOCK_LABELS.cs;
  const cta = docsUrl ? `<div style="text-align:center;margin:14px 0 4px"><a href="${docsUrl}" style="display:inline-block;background:#74FB71;color:#1a2e22;padding:12px 24px;border-radius:25px;text-decoration:none;font-weight:800;font-size:14px">${t.cta}</a></div>` : '';
  return `
...
function renderDiffRow(label, oldVal, newVal) {
...
<p>Protože je pro nás zpětná vazba velmi důležitá, budeme rádi, pokud nám zanecháte recenzi na <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> nebo <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
<p>Your feedback matters a lot to us — please leave a review on <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> or <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
<p>Ihr Feedback ist uns sehr wichtig — bitte hinterlassen Sie eine Bewertung auf <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> oder <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
<p>Jouw feedback is belangrijk — laat een review achter op <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> of <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
<p>Tu opinión es muy importante para nosotros — por favor déjanos una reseña en <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> o <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
<p>Votre avis compte beaucoup — laissez-nous un avis sur <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> ou <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
<p>Twoja opinia jest dla nas bardzo ważna — zostaw recenzję na <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> lub <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
<p>Ваш відгук для нас дуже важливий — залиште його на <a href="${v.google_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Google</a>, <a href="${v.facebook_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Facebook</a> або <a href="${v.instagram_review_url || '#'}" style="display:inline-block;background:#74FB71;color:#0a1f15;font-size:12px;font-weight:700;text-decoration:none;padding:6px 14px;border-radius:14px;margin:0 4px">Instagram</a>.</p>
...
// =============================================================================
function renderAbandonedFullBody(lang, v) {
  // State A: chybí platba i doklady. Dva CTA: pay_url + docs_url.
  const T = {
    cs: {
      intro: `vidíme, že jste rozjeli rezervaci č. <strong>${v.booking_number}</strong> motocyklu <strong>${v.motorcycle}</strong> na <strong>${v.start_date} – ${v.end_date}</strong>, ale ještě jste ji nedokončili. Chybí dvě věci:`,
      needs: [
        '<strong>Zaplatit</strong> přes zabezpečenou Stripe bránu',
        '<strong>Nahrát doklady</strong> (občanka/pas + řidičák) — sken přes mobil díky Mindee OCR zabere 30 vteřin'
      ],
      ...
      payCta: 'Pokračovat k platbě',
      docsCta: 'Nahrát doklady'
    },
    en: {
      intro: `we noticed you started booking <strong>#${v.booking_number}</strong> for <strong>${v.motorcycle}</strong> on <strong>${v.start_date} – ${v.end_date}</strong> but haven't finished it. Two things are missing:`,
      needs: [
        '<strong>Pay</strong> via secure Stripe',
        '<strong>Upload documents</strong> (ID + driver\'s license) — Mindee OCR scan via mobile takes 30 seconds'
      ],
      ...
      docsCta: 'Upload documents'
    },
    de: {
      intro: `wir sehen, dass Sie die Buchung Nr. <strong>${v.booking_number}</strong> für <strong>${v.motorcycle}</strong> vom <strong>${v.start_date} – ${v.end_date}</strong> begonnen, aber nicht abgeschlossen haben. Es fehlen zwei Dinge:`,
      needs: [
        '<strong>Zahlung</strong> über das sichere Stripe-Gateway',
        '<strong>Dokumente hochladen</strong> (Ausweis + Führerschein) — Mindee-OCR-Scan per Handy in 30 Sekunden'
      ],
      ...
      payCta: 'Zur Zahlung',
      docsCta: 'Dokumente hochladen'
    },
    nl: {
      intro: `we zien dat je boeking nr. <strong>${v.booking_number}</strong> voor <strong>${v.motorcycle}</strong> op <strong>${v.start_date} – ${v.end_date}</strong> bent begonnen maar niet hebt afgerond. Er ontbreken twee dingen:`,
      needs: [
        '<strong>Betalen</strong> via beveiligde Stripe',
        '<strong>Documenten uploaden</strong> (ID + rijbewijs) — Mindee OCR-scan via mobiel duurt 30 seconden'
      ],
      ...
      payCta: 'Doorgaan naar betaling',
      docsCta: 'Documenten uploaden'
    },
    es: {
      intro: `vemos que comenzaste la reserva nº <strong>${v.booking_number}</strong> para <strong>${v.motorcycle}</strong> del <strong>${v.start_date} al ${v.end_date}</strong>, pero aún no la has finalizado. Faltan dos cosas:`,
      needs: [
        '<strong>Pagar</strong> a través de la pasarela segura Stripe',
        '<strong>Subir documentos</strong> (DNI/pasaporte + permiso de conducir) — escaneo Mindee OCR desde el móvil en 30 segundos'
      ],
      ...
      docsCta: 'Subir documentos'
    },
    fr: {
      intro: `nous avons vu que vous avez commencé la réservation n° <strong>${v.booking_number}</strong> pour <strong>${v.motorcycle}</strong> du <strong>${v.start_date} au ${v.end_date}</strong> sans la terminer. Il manque deux choses :`,
      needs: [
        '<strong>Payer</strong> via la passerelle sécurisée Stripe',
        '<strong>Télécharger les documents</strong> (CNI/passeport + permis) — scan Mindee OCR depuis le mobile en 30 secondes'
      ],
      ...
      payCta: 'Continuer le paiement',
      docsCta: 'Télécharger les documents'
    },
    pl: {
      intro: `widzimy, że rozpocząłeś rezerwację nr <strong>${v.booking_number}</strong> motocykla <strong>${v.motorcycle}</strong> na <strong>${v.start_date} – ${v.end_date}</strong>, ale jej nie dokończyłeś. Brakuje dwóch rzeczy:`,
      needs: [
        '<strong>Zapłacić</strong> przez bezpieczną bramkę Stripe',
        '<strong>Przesłać dokumenty</strong> (dowód + prawo jazdy) — skan Mindee OCR z telefonu trwa 30 sekund'
      ],
      ...
      payCta: 'Przejdź do płatności',
      docsCta: 'Prześlij dokumenty'
    },
    uk: {
      intro: `бачимо, що ви почали бронювання № <strong>${v.booking_number}</strong> мотоцикла <strong>${v.motorcycle}</strong> на <strong>${v.start_date} – ${v.end_date}</strong>, але не завершили його. Бракує двох речей:`,
      needs: [
        '<strong>Оплатити</strong> через захищений шлюз Stripe',
        '<strong>Надіслати документи</strong> (посвідчення особи + права) — сканування Mindee OCR з телефона триває 30 секунд'
      ],
      ...
      payCta: 'Перейти до оплати',
      docsCta: 'Надіслати документи'
    }
  };
  const t = T[lang];
...
function renderMissingDocsBody(lang, v) {
  // State C: paid + chybí doklady. Jen docs CTA.
  const T = {
    cs: {
      paid: `vaše rezervace č. <strong>${v.booking_number}</strong> motocyklu <strong>${v.motorcycle}</strong> na <strong>${v.start_date} – ${v.end_date}</strong> je zaplacená — děkujeme!`,
      ...
      cta: 'Nahrát doklady',
      warn: 'Bez nahraných dokladů systém kódy nevydá — a platit za něco, co si nemůžete vyzvednout, by byla škoda.'
    },
    en: {
      paid: `your booking <strong>#${v.booking_number}</strong> for <strong>${v.motorcycle}</strong> on <strong>${v.start_date} – ${v.end_date}</strong> is paid — thank you!`,
      ...
      cta: 'Upload documents',
      warn: 'Without uploaded documents the system won\'t release codes — and paying for something you can\'t pick up would be a shame.'
    },
    de: {
      paid: `Ihre Buchung Nr. <strong>${v.booking_number}</strong> für <strong>${v.motorcycle}</strong> vom <strong>${v.start_date} – ${v.end_date}</strong> ist bezahlt — danke!`,
      ...
      cta: 'Dokumente hochladen',
      warn: 'Ohne hochgeladene Dokumente gibt das System keine Codes frei — und für etwas zu zahlen, das Sie nicht abholen können, wäre schade.'
    },
    nl: {
      paid: `je boeking nr. <strong>${v.booking_number}</strong> voor <strong>${v.motorcycle}</strong> van <strong>${v.start_date} – ${v.end_date}</strong> is betaald — bedankt!`,
      ...
      cta: 'Documenten uploaden',
      warn: 'Zonder geüploade documenten geeft het systeem geen codes vrij — en betalen voor iets dat je niet kunt ophalen zou zonde zijn.'
    },
    es: {
      paid: `tu reserva nº <strong>${v.booking_number}</strong> de <strong>${v.motorcycle}</strong> del <strong>${v.start_date} al ${v.end_date}</strong> está pagada — ¡gracias!`,
      ...
      cta: 'Subir documentos',
      warn: 'Sin documentos subidos el sistema no libera los códigos — y pagar por algo que no puedes recoger sería una pena.'
    },
    fr: {
      paid: `votre réservation n° <strong>${v.booking_number}</strong> pour <strong>${v.motorcycle}</strong> du <strong>${v.start_date} au ${v.end_date}</strong> est payée — merci !`,
      ...
      warn: 'Sans documents téléchargés, le système ne libère pas les codes — et payer pour quelque chose que vous ne pouvez pas récupérer, ce serait dommage.'
    },
    pl: {
      paid: `Twoja rezerwacja nr <strong>${v.booking_number}</strong> motocykla <strong>${v.motorcycle}</strong> na <strong>${v.start_date} – ${v.end_date}</strong> jest opłacona — dziękujemy!`,
      ...
      cta: 'Prześlij dokumenty',
      warn: 'Bez przesłanych dokumentów system nie zwolni kodów — szkoda płacić za coś, czego nie można odebrać.'
    },
    uk: {
      paid: `Ваше бронювання № <strong>${v.booking_number}</strong> мотоцикла <strong>${v.motorcycle}</strong> на <strong>${v.start_date} – ${v.end_date}</strong> оплачено — дякуємо!`,
      ...
      cta: 'Надіслати документи',
      warn: 'Без надісланих документів система коди не видасть — шкода платити за те, що не можна забрати.'
    }
  };
  const t = T[lang];
  return `<p>${HELLO[lang]}</p>
...
function renderVoucherBody(lang, v) {
  const T = {
    cs: {
      intro: 'děkujeme, že jste si pro svůj dárek vybrali právě MotoGo24.',
      received: `Vaši objednávku č. <strong>${v.order_number}</strong> jsme úspěšně přijali a platba byla zpracována.`,
      attached: [
        'dárkový poukaz,',
        'doklad o přijetí platby za nákup dárkového poukazu.'
      ],
    ...
    en: {
      ...
      usage: [
        'The gift voucher is valid for 3 years from issue date and can be used for any motorcycle rental of your choice. The recipient simply books a date in advance based on current motorcycle availability via the form at <a href="https://www.motogo24.cz" style="color:#2563eb">motogo24.cz</a>.',
    ...
    de: {
      intro: 'vielen Dank, dass Sie für Ihr Geschenk MOTO GO 24 gewählt haben.',
      ...
      attached: [
        ...
        'die Quittung für den Gutscheinkauf.'
      ],
      ...
      usage: [
        'Der Geschenkgutschein ist 3 Jahre ab Ausstellungsdatum gültig und kann für eine Motorradmiete Ihrer Wahl verwendet werden. Der Beschenkte bucht einen Termin im Voraus über das Formular auf <a href="https://www.motogo24.cz" style="color:#2563eb">motogo24.cz</a>.',
    ...
    nl: {
      ...
      usage: [
        'De cadeaubon is 3 jaar geldig vanaf uitgiftedatum en kan worden gebruikt voor elke motorhuur naar keuze. De ontvanger boekt eenvoudig een datum vooraf via het formulier op <a href="https://www.motogo24.cz" style="color:#2563eb">motogo24.cz</a>.',
    ...
    es: {
      ...
      usage: [
        'El cheque regalo es válido durante 3 años desde su emisión y puede usarse para alquilar cualquier motocicleta. El destinatario reserva una fecha por adelantado mediante el formulario en <a href="https://www.motogo24.cz" style="color:#2563eb">motogo24.cz</a>.',
    ...
    fr: {
      intro: 'merci d\'avoir choisi MOTO GO 24 pour votre cadeau.',
      ...
      usage: [
        'Le bon cadeau est valable 3 ans à compter de la date d\'émission et peut être utilisé pour la location d\'une moto au choix. Le bénéficiaire réserve une date à l\'avance via le formulaire sur <a href="https://www.motogo24.cz" style="color:#2563eb">motogo24.cz</a>.',
    ...
    pl: {
      ...
      usage: [
        'Voucher jest ważny 3 lata od daty wystawienia i można go wykorzystać na wynajem dowolnego motocykla. Obdarowany rezerwuje termin z wyprzedzeniem przez formularz na <a href="https://www.motogo24.cz" style="color:#2563eb">motogo24.cz</a>.',
    ...
    uk: {
      ...
      usage: [
        'Сертифікат дійсний 3 роки від дати видачі й діє на оренду будь-якого мотоцикла. Обдарований бронює термін заздалегідь через форму на <a href="https://www.motogo24.cz" style="color:#2563eb">motogo24.cz</a>.',
...
function renderShopConfirmedBody(lang, v) {
  const T = {
    cs: {
      intro: 'děkujeme za vaši objednávku v e-shopu MotoGo24. Vaši platbu jsme úspěšně přijali.',
      ...
      help: 'Pokud máte jakýkoliv dotaz, jsme vám k dispozici.',
      closing: 'S pozdravem,'
    },
    en: {
      intro: 'thank you for your order at MOTO GO 24 e-shop. We have received your payment.',
    ...
    de: {
      intro: 'vielen Dank für Ihre Bestellung im MOTO GO 24 Shop. Ihre Zahlung ist eingegangen.',
      ...
      help: 'Bei Fragen sind wir für Sie da.',
      closing: 'Mit freundlichen Grüßen,'
    },
    nl: {
      intro: 'bedankt voor je bestelling bij MOTO GO 24. Je betaling is ontvangen.',
      ...
      help: 'Heb je vragen? We helpen je graag.',
      closing: 'Met vriendelijke groet,'
    },
    es: {
      intro: 'gracias por tu pedido en la tienda MOTO GO 24. Hemos recibido tu pago.',
      ...
      help: 'Si tienes preguntas, estamos para ayudarte.',
      closing: 'Saludos cordiales,'
    },
    fr: {
      intro: 'merci pour votre commande dans la boutique MOTO GO 24. Votre paiement est bien reçu.',
      ...
      help: 'Pour toute question, nous restons à votre disposition.',
      closing: 'Cordialement,'
    },
    pl: {
      intro: 'dziękujemy za zamówienie w sklepie MOTO GO 24. Twoja płatność została przyjęta.',
      ...
      help: 'W razie pytań jesteśmy do dyspozycji.',
      closing: 'Pozdrawiamy,'
    },
    uk: {
      intro: 'дякуємо за замовлення в магазині MOTO GO 24. Вашу оплату прийнято.',
      ...
      help: 'Якщо виникнуть питання, ми до ваших послуг.',
      closing: 'З повагою,'
    }
  };
  const t = T[lang];
...
function renderShopShippedBody(lang, v) {
  const T = {
    cs: {
      intro: `vaše objednávka č. <strong>${v.order_number}</strong> byla odeslána a brzy dorazí k vám.`,
      trackNum: 'Číslo zásilky:',
      trackUrl: 'Sledování:',
      attached: 'V příloze najdete <strong>konečnou fakturu</strong> za tuto objednávku.',
      thanks: 'Děkujeme za nákup u MOTO GO 24 a věříme, že budete s nákupem spokojeni.',
      closing: 'S pozdravem,'
    },
    en: {
      intro: `your order <strong>#${v.order_number}</strong> has been shipped and will reach you soon.`,
      trackNum: 'Tracking number:',
      trackUrl: 'Track at:',
      attached: 'Attached you\'ll find the <strong>final invoice</strong> for this order.',
      thanks: 'Thank you for shopping at MOTO GO 24 — we hope you\'ll love your purchase.',
      closing: 'Best regards,'
    },
    de: {
      intro: `Ihre Bestellung Nr. <strong>${v.order_number}</strong> wurde versandt und erreicht Sie bald.`,
      trackNum: 'Sendungsnummer:',
      trackUrl: 'Verfolgen unter:',
      attached: 'Im Anhang finden Sie die <strong>Endrechnung</strong> zu dieser Bestellung.',
      thanks: 'Danke für Ihren Einkauf bei MOTO GO 24 — wir hoffen, Sie sind zufrieden.',
      closing: 'Mit freundlichen Grüßen,'
    },
    nl: {
      intro: `je bestelling nr. <strong>${v.order_number}</strong> is verzonden en komt binnenkort aan.`,
      trackNum: 'Trackingnummer:',
      trackUrl: 'Volgen op:',
      attached: 'In de bijlage vind je de <strong>eindfactuur</strong> voor deze bestelling.',
      thanks: 'Bedankt voor je aankoop bij MOTO GO 24 — we hopen dat je tevreden bent.',
      closing: 'Met vriendelijke groet,'
    },
    es: {
      intro: `tu pedido nº <strong>${v.order_number}</strong> ha sido enviado y llegará pronto.`,
      trackNum: 'Nº de seguimiento:',
      trackUrl: 'Sigue en:',
      attached: 'Adjuntamos la <strong>factura final</strong> de este pedido.',
      thanks: '¡Gracias por comprar en MOTO GO 24 — esperamos que disfrutes tu compra!',
      closing: 'Saludos cordiales,'
    },
    fr: {
      intro: `votre commande n° <strong>${v.order_number}</strong> a été expédiée et vous parviendra sous peu.`,
      trackNum: 'Numéro de suivi :',
      trackUrl: 'Suivi sur :',
      attached: 'Vous trouverez en pièce jointe la <strong>facture finale</strong> de cette commande.',
      thanks: 'Merci pour votre achat chez MOTO GO 24 — nous espérons qu\'il vous plaira.',
      closing: 'Cordialement,'
    },
    pl: {
      intro: `Twoje zamówienie nr <strong>${v.order_number}</strong> zostało wysłane i wkrótce dotrze.`,
      trackNum: 'Numer przesyłki:',
      trackUrl: 'Śledzenie:',
      attached: 'W załączniku znajdziesz <strong>fakturę końcową</strong> do tego zamówienia.',
      thanks: 'Dziękujemy za zakupy w MOTO GO 24 — mamy nadzieję, że będziesz zadowolony.',
      closing: 'Pozdrawiamy,'
    },
    uk: {
      intro: `Ваше замовлення № <strong>${v.order_number}</strong> відправлено й невдовзі надійде.`,
      trackNum: 'Номер посилки:',
      trackUrl: 'Відстеження:',
      attached: 'У вкладенні знайдете <strong>остаточний рахунок</strong> до цього замовлення.',
      thanks: 'Дякуємо за покупку в MOTO GO 24 — сподіваємося, ви будете задоволені.',
      closing: 'З повагою,'
    }
  };
  const t = T[lang];
...
const METHOD_LABELS = {
  ...
  de: {
    branch: 'In der Filiale',
    delivery: 'Lieferung an Adresse'
  },
  nl: {
    branch: 'Bij het filiaal',
    delivery: 'Bezorging op adres'
  },
  es: {
    branch: 'En la sucursal',
  ...
  pl: {
    branch: 'W oddziale',
    delivery: 'Dostawa na adres'
  },
  uk: {
    branch: 'У філії',
    delivery: 'Доставка на адресу'
  }
};
function renderModifiedBody(lang, v) {
  const L = DIFF_LABELS[lang];
  const mLbl = (m)=>m ? m === 'delivery' ? METHOD_LABELS[lang].delivery : METHOD_LABELS[lang].branch : '';
  // pickup_time/return_time je text 'HH:MM' — pokud je vyplněný, přilepíme za adresu
  const pickupOrig = [
    mLbl(v.original_pickup_method),
    v.original_pickup_address,
    v.original_pickup_time ? '@' + v.original_pickup_time : ''
  ].filter(Boolean).join(' — ');
  const pickupNew = [
    mLbl(v.pickup_method),
    v.pickup_address,
    v.pickup_time ? '@' + v.pickup_time : ''
  ].filter(Boolean).join(' — ');
  const returnOrig = [
    mLbl(v.original_return_method),
    v.original_return_address,
    v.original_return_time ? '@' + v.original_return_time : ''
  ].filter(Boolean).join(' — ');
  const returnNew = [
    mLbl(v.return_method),
    v.return_address,
    v.return_time ? '@' + v.return_time : ''
  ].filter(Boolean).join(' — ');
  const pd = Number((v.price_difference || '0').toString().replace(/\s/g, '').replace(',', '.')) || 0;
  const intros = {
  ...
  const refundTail = refundManual ? {
    ...
    uk: 'Суму повернемо банківським переказом на ваш рахунок протягом 14 днів.'
  } : {
    ...
    uk: 'Кошти повертаються на початкову картку.'
  };
  const priceMsgs = {
    cs: {
      plus: `K úpravě se vztahuje <strong>doplatek ${v.price_difference}</strong>. Po platbě dorazí doklad k přijaté platbě.`,
      minus: `K úpravě se vztahuje <strong>vrácení ${v.price_difference}</strong> formou dobropisu, který najdete v příloze. ${refundTail.cs}`
    },
    en: {
      plus: `An additional payment of <strong>${v.price_difference}</strong> applies. The payment receipt will follow after payment.`,
      minus: `A refund of <strong>${v.price_difference}</strong> applies as a credit note (attached). ${refundTail.en}`
    },
    de: {
      plus: `Es fällt eine Nachzahlung von <strong>${v.price_difference}</strong> an. Der Beleg folgt nach Zahlungseingang.`,
      minus: `Eine Rückerstattung von <strong>${v.price_difference}</strong> wird als Gutschrift (Anhang) erstattet — ${refundTail.de}`
    },
    nl: {
      plus: `Er volgt een bijbetaling van <strong>${v.price_difference}</strong>. Het betalingsbewijs komt na betaling.`,
      minus: `Een terugbetaling van <strong>${v.price_difference}</strong> wordt verwerkt als creditnota (bijlage) — ${refundTail.nl}`
    },
    es: {
      plus: `Se aplica un pago adicional de <strong>${v.price_difference}</strong>. El comprobante llegará tras el pago.`,
      minus: `Se aplica un reembolso de <strong>${v.price_difference}</strong> mediante nota de crédito (adjunta) — ${refundTail.es}`
    },
    fr: {
      plus: `Un supplément de <strong>${v.price_difference}</strong> s'applique. Le justificatif suivra après paiement.`,
      minus: `Un remboursement de <strong>${v.price_difference}</strong> est émis sous forme d'avoir (joint) — ${refundTail.fr}`
    },
    pl: {
      plus: `Wymagana jest dopłata <strong>${v.price_difference}</strong>. Potwierdzenie płatności nadejdzie po płatności.`,
      minus: `Zwrot <strong>${v.price_difference}</strong> w formie noty kredytowej (załącznik) — ${refundTail.pl}`
    },
    uk: {
      plus: `Потрібна доплата <strong>${v.price_difference}</strong>. Підтвердження оплати надійде після платежу.`,
      minus: `Повернення <strong>${v.price_difference}</strong> у формі кредит-ноти (у вкладенні) — ${refundTail.uk}`
    }
  };
  const attachInfo = {
...
// PUBLIC API
// =============================================================================
/** Render subject + body for a given (slug, lang). Falls back to 'cs' if lang missing. */ export function renderEmail(slug, lang, vars) {
...
/** Per-lang sign-off (for help card / footer in wrapInBrandedLayout). */ export function signOff(lang) {
  return SIGN[lang] || SIGN.cs;
}
export function helloPhrase(lang) {
  return HELLO[lang] || HELLO.cs;
}
/** Per-lang labels for the help card "Have a question?" + CTA. */ export function helpCardLabels(lang) {
  const map = {
    cs: {
      title: 'Máš dotaz?',
      body: 'Pokud budeš mít jakýkoliv dotaz, jsme ti k dispozici.',
      cta: 'info@motogo24.cz'
    },
    en: {
      title: 'Have a question?',
      body: 'If you have any questions, we\'re here to help.',
      cta: 'info@motogo24.cz'
    },
    de: {
      title: 'Haben Sie eine Frage?',
      body: 'Bei Fragen sind wir gerne für Sie da.',
      cta: 'info@motogo24.cz'
    },
    nl: {
      title: 'Heb je een vraag?',
      body: 'Heb je vragen? We helpen je graag.',
      cta: 'info@motogo24.cz'
    },
    es: {
      title: '¿Tienes preguntas?',
      body: 'Si tienes alguna pregunta, estamos para ayudarte.',
      cta: 'info@motogo24.cz'
    },
    fr: {
      title: 'Une question ?',
      body: 'Pour toute question, nous sommes à votre disposition.',
      cta: 'info@motogo24.cz'
    },
    pl: {
      title: 'Masz pytanie?',
      body: 'W razie pytań jesteśmy do dyspozycji.',
      cta: 'info@motogo24.cz'
    },
    uk: {
      title: 'Маєте питання?',
      body: 'Якщо виникнуть питання, ми до ваших послуг.',
      cta: 'info@motogo24.cz'
    }
  };
...
// =============================================================================
/** Per-lang label pro typ faktury (zálohová / konečná / doklad / e-shop). */ export function invoiceTypeLabel(invType, lang) {
  const map = {
    advance: {
      cs: 'Zálohová faktura',
      en: 'Proforma invoice',
...
/** Per-lang subject + intro/closing pro send-invoice-email. */ export function invoiceEmailSnippets(invType, lang, vars) {
