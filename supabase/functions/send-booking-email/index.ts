...
import { renderEmail, normalizeLang, helpCardLabels, renderDoorCodesReleasedBlock, renderDocsRequiredBlock } from './i18n.ts';
...
/** Plná URL domény zákazníka (pro odkazy / QR). cs → SITE_URL (.cz). */ function siteForLang(lang) {
  return lang === 'cs' ? SITE_URL : DOMAIN_INTL;
}
/** Webový label bez protokolu do patičky. */ function webLabelForLang(lang) {
 ...
 *  ikony) jsou jen ve `wrapInBrandedLayout`, kam se tato fce neaplikuje. */ function localizeBodyLinks(html, lang) {
...
/** SHA-1 hex zdroje (detekce změny CZ šablony → invalidace cache). */ async function sha1Hex(s) {
 ...
 *  Vrací {subject, body} nebo null při chybě (caller pak nechá CZ — mail aspoň odejde). */ async function translateEmailTemplate(subjectCz, bodyCz, lang) {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn('[translateEmailTemplate] anthropic', res.status);
      return null;
    }
    ...
    console.warn('[translateEmailTemplate]', e.message);
    return null;
  }
}
 ...
 *  vrátí CZ (mail vždy odejde). */ async function resolveTemplateForLang(supabase, tpl, lang) {
  ...
  if (fresh) {
    return {
      subject: typeof subjT[lang] === 'string' && subjT[lang] ? subjT[lang] : subjCz,
      body: bodyT[lang]
    };
  }
  ...
  try {
    ...
    await supabase.from('email_templates').update({
      subject_translations: newSubjT,
      body_translations: newBodyT
    }).eq('slug', tpl.slug);
  } catch  {}
  return tr;
}
const FOLLOW_US_LABEL = {
  cs: 'SLEDUJTE NÁS',
  en: 'FOLLOW US',
  de: 'FOLGEN SIE UNS',
  nl: 'VOLG ONS',
...
const REVIEW_BLOCK_LABELS = {
  cs: {
    title: 'Pomohlo by nám vaše hodnocení',
    body: 'Pokud jste byli spokojeni, prosíme zanechte nám recenzi na Googlu — pomáháte tím dalším motorkářům.',
    cta: '⭐ Ohodnotit na Google'
  },
  en: {
    title: 'Your review would help us',
    body: 'If you were happy with our service, please leave us a review on Google — it helps fellow riders.',
    cta: '⭐ Review on Google'
  },
  de: {
    title: 'Ihre Bewertung würde uns helfen',
    body: 'Wenn Sie zufrieden waren, hinterlassen Sie uns bitte eine Google-Bewertung — Sie helfen damit anderen Bikern.',
    cta: '⭐ Auf Google bewerten'
  },
  nl: {
    title: 'Je review helpt ons enorm',
    body: 'Was je tevreden? Laat dan een review achter op Google — je helpt andere motorrijders.',
    cta: '⭐ Beoordeel op Google'
  },
  es: {
    title: 'Tu reseña nos ayudaría mucho',
    body: 'Si quedaste satisfecho, déjanos una reseña en Google — ayudas a otros moteros.',
    cta: '⭐ Reseña en Google'
  },
  fr: {
    title: 'Votre avis nous aiderait',
    body: 'Si vous étiez satisfait, laissez-nous un avis sur Google — vous aidez d\'autres motards.',
    cta: '⭐ Avis sur Google'
  },
  pl: {
    title: 'Twoja recenzja by nam pomogła',
    body: 'Jeśli byłeś zadowolony, zostaw nam recenzję w Google — pomożesz innym motocyklistom.',
    cta: '⭐ Oceń w Google'
  }
};
...
function fixReviewLinks(html) {
  ...
  out = out.replace(/<a\b[^>]*>[\s\S]*?<\/a>/gi, (a)=>/instagram\.com/i.test(a) || /<a\b[^>]*>\s*Instagram\s*<\/a>/i.test(a) ? a.replace(/href="[^"]*"/i, `href="${FIRMY_REVIEW_URL_DEFAULT}"`).replace(/>[\s\S]*?<\/a>/i, '>Firmy.cz</a>') : a);
  if (!/firmy\.cz/i.test(out)) {
    out = out.replace(/<a\b[^>]*\bhref="[^"]*facebook\.com[^"]*"[^>]*>\s*Facebook\s*<\/a>/i, (fbAnchor)=>{
      const firmyAnchor = fbAnchor.replace(/href="[^"]*"/i, `href="${FIRMY_REVIEW_URL_DEFAULT}"`).replace(/>\s*Facebook\s*<\/a>/i, '>Firmy.cz</a>');
      return `${fbAnchor}, ${firmyAnchor}`;
    });
  }
  return out;
}
function googleReviewBlock(lang, url) {
...
/** Send email with exponential backoff retry (max 3 attempts) */ async function sendWithRetry(emailData, maxRetries = 3) {
...
/** Replace {{var}} placeholders; unknown vars -> empty string */ function renderTemplate(template, vars) {
  if (!template) return '';
  let result = template;
...
/** Wrap body HTML in unified MotoGo24 email layout (1:1 with invoice design + screen reference) */ function wrapInBrandedLayout(bodyHtml, lang = 'cs') {
...
function resolveSlug(type, source) {
...
/** Vrátí příponu souboru (bez tečky, lowercase) — `.pdf` nebo `.html`. */ function fileExt(path) {
  if (path && /\.pdf$/i.test(path)) return 'pdf';
  return 'html';
}
/** HTML fallback pro voucher — pokud PDF generation selže, přiložíme HTML voucher,
 *  aby v mailu nikdy nechyběl. Renderuje branded design 1:1 s ostatními maily. */ function renderVoucherHtmlFallback(code, amount, validUntil) {
  const fmtPriceLocal = (n)=>(n || 0).toLocaleString('cs-CZ', {
      minimumFractionDigits: 0
    });
  const fmtDateLocal = (d)=>d ? new Date(d).toLocaleDateString('cs-CZ') : '—';
 ...
 *  fallbacku se špatným vzhledem). Vrací Uint8Array nebo null při chybě / no key. */ async function htmlToPdf(html) {
 ...
 *  bez bílých okrajů a beze změny vzhledu šablony. */ function renderVoucherPdfHtml(code, amount, validUntil) {
  const fmtPrice = (n)=>(n || 0).toLocaleString('cs-CZ', {
      minimumFractionDigits: 0
    });
  const fmtDate = (d)=>d ? new Date(d).toLocaleDateString('cs-CZ') : '—';
 ...
 *  caller má HTML fallback. */ async function generateVoucherPdfAttachment(code, amount, validUntil) {
  ...
  return btoa(Array.from(pdfBytes, (b)=>String.fromCharCode(b)).join(''));
}
/** Upload base64 content to documents storage bucket and return path. */ async function uploadB64ToStorage(supabase, b64, path, contentType) {
  try {
    const bin = atob(b64);
    ...
    if (error) {
      console.warn('[uploadB64ToStorage]', path, error.message);
      return null;
    }
    return path;
  } catch (e) {
    console.warn('[uploadB64ToStorage]', path, e.message);
    return null;
  }
}
/** Auto-generate attachments based on email type */ async function autoGenerateAttachments(type, booking_id, supabase, opts = {}) {
  ...
  if (type === 'voucher_purchased' && opts.orderId) {
    try {
      ...
      const { data: dp } = await supabase.from('invoices').select('id, number, pdf_path').eq('order_id', opts.orderId).eq('type', 'payment_receipt').neq('status', 'cancelled').order('created_at', {
        ascending: false
      }).limit(1);
      if (dp?.length && dp[0].pdf_path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `Doklad-platby-${dp[0].number || 'DP'}.${fileExt(dp[0].pdf_path)}`,
          storage_path: dp[0].pdf_path
        });
      } else {
        ...
        if (data.success && data.invoice_id) {
          ...
          if (b64) atts.push({
            content: b64,
            filename: `Doklad-platby-${data.number || 'DP'}.${fileExt(path)}`,
            storage_path: path
          });
        }
      }
      ...
      const { data: vouchers } = await supabase.from('vouchers').select('code, amount, valid_until').eq('order_id', opts.orderId);
      for (const v of vouchers || []){
        ...
        try {
          ...
          atts.push({
            content: b64,
            filename: `Darkovy-poukaz-${v.code}.pdf`,
      ...
      console.warn('[autoGenerateAttachments] voucher_purchased failed:', e.message);
    }
    return atts;
  }
  ...
  if (type === 'rental_contract' && booking_id) {
    try {
      ...
      if (data.success && data.path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `Najemni-smlouva-${booking_id.slice(-8).toUpperCase()}.${fileExt(data.path)}`,
  ...
  if (type === 'vop' && booking_id) {
    try {
      ...
      if (data.success && data.path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `VOP-${booking_id.slice(-8).toUpperCase()}.${fileExt(data.path)}`,
          storage_path: data.path
        });
      }
    } catch  {}
  ...
  if (type === 'credit_note') {
    try {
      let q = supabase.from('invoices').select('id, number, pdf_path').eq('type', 'credit_note').neq('status', 'cancelled').order('created_at', {
        ascending: false
      }).limit(1);
      ...
      if (cn?.length && cn[0].pdf_path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `Dobropis-${cn[0].number || 'DB'}.${fileExt(cn[0].pdf_path)}`,
          storage_path: cn[0].pdf_path
        });
      }
    } catch  {}
    return atts;
  ...
  if (type === 'shop_order_shipped' && opts.orderId) {
    try {
      const { data: kf } = await supabase.from('invoices').select('id, number, pdf_path').eq('order_id', opts.orderId).eq('type', 'shop_final').neq('status', 'cancelled').order('created_at', {
        ascending: false
      }).limit(1);
      if (kf?.length && kf[0].pdf_path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `Konecna-faktura-${kf[0].number || 'KF'}.${fileExt(kf[0].pdf_path)}`,
          storage_path: kf[0].pdf_path
        });
      } else {
        ...
        if (data.success && data.invoice_id) {
          ...
          if (b64) atts.push({
            content: b64,
            filename: `Konecna-faktura-${data.number || 'KF'}.${fileExt(path)}`,
            storage_path: path
          });
        }
      }
  ...
  if (type === 'booking_advance') {
    try {
      ...
      const { data: existing } = await supabase.from('invoices').select('id, number, pdf_path').eq('booking_id', booking_id).in('type', [
        'advance',
        'proforma'
      ...
      if (existing?.length && existing[0].pdf_path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `Zalohova-faktura-${existing[0].number || 'ZF'}.${fileExt(existing[0].pdf_path)}`,
          storage_path: existing[0].pdf_path
        });
      } else {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/generate-invoice`, {
          method: 'POST',
        ...
        if (data.success && data.invoice_id) {
          ...
          if (b64) atts.push({
            content: b64,
            filename: `Zalohova-faktura-${data.number || 'ZF'}.${fileExt(path)}`,
            storage_path: path
          });
        }
      }
  ...
  if (type === 'booking_payment_receipt') {
    try {
      const { data: existing } = await supabase.from('invoices').select('id, number, pdf_path, source').eq('booking_id', booking_id).eq('type', 'payment_receipt').neq('status', 'cancelled').order('created_at', {
        ascending: false
      });
      // Vyfiltruj DP vázané na booking (ne shop) — preferuj source='booking'/'edit'/null
      const candidate = (existing || []).find((r)=>r.source !== 'shop') || (existing || [])[0];
      if (candidate?.pdf_path) {
        const b64 = await downloadAsBase64(supabase, candidate.pdf_path);
        if (b64) atts.push({
          content: b64,
          filename: `Doklad-platby-${candidate.number || 'DP'}.${fileExt(candidate.pdf_path)}`,
          storage_path: candidate.pdf_path
        });
      } else {
        const res = await fetch(`${SUPABASE_URL}/functions/v1/generate-invoice`, {
          method: 'POST',
    ...
    try {
      ...
      if (!attached) {
        try {
          await supabase.from('debug_log').insert({
            source: 'send-booking-email',
            action: 'abandoned_zf_attach_failed',
  ...
  if (type === 'booking_completed') {
    ...
    try {
      const { data: invoices } = await supabase.from('invoices').select('id, number, pdf_path').eq('booking_id', booking_id).in('type', [
        'final'
      ]).neq('status', 'cancelled').order('created_at', {
        ascending: false
      }).limit(1);
      ...
      if (invoices?.length && !kfPath) {
        try {
          ...
          if (data.success && data.pdf_path) {
            kfPath = data.pdf_path;
            kfNumber = data.number || kfNumber;
          }
        } catch  {}
      }
      if (kfPath) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `Konecna-faktura-${kfNumber || 'KF'}.${fileExt(kfPath)}`,
          storage_path: kfPath
        });
      }
    } catch  {}
  ...
  if (type === 'booking_modified') {
    ...
    if (priceDifference > 0) {
      ...
      try {
        ...
        if (dpData.success && dpData.invoice_id) {
          ...
          if (b64) atts.push({
            content: b64,
            filename: `Doklad-platby-uprava-${dpData.number || 'DP'}.${fileExt(path)}`,
            storage_path: path
          });
        }
      } catch  {}
      ...
      for(let attempt = 0; attempt < 3 && !attached; attempt++){
        try {
          const { data: cn } = await supabase.from('invoices').select('id, number, pdf_path').eq('booking_id', booking_id).eq('type', 'credit_note').neq('status', 'cancelled').order('created_at', {
            ascending: false
          }).limit(1);
          if (cn?.length && cn[0].pdf_path) {
            const b64 = await downloadAsBase64(supabase, cn[0].pdf_path);
    ...
    try {
      ...
      if (cData.success && cData.path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `Najemni-smlouva-${booking_id.slice(-8).toUpperCase()}.${fileExt(cData.path)}`,
          storage_path: cData.path
        });
      }
    } catch  {}
    // 4. VOP
    try {
      ...
      if (vData.success && vData.path) {
        ...
        if (b64) atts.push({
          content: b64,
          filename: `VOP-${booking_id.slice(-8).toUpperCase()}.${fileExt(vData.path)}`,
          storage_path: vData.path
        });
      }
    } catch  {}
  }
...
serve(async (req)=>{
  ...
  try {
    ...
    const { type, booking_id, customer_email, customer_name, motorcycle, start_date, end_date, total_price, price_difference, voucher_code, voucher_value, voucher_expiry, order_number, source = 'app', resume_link, pay_url, docs_url, discount_code, google_review_url, facebook_review_url, manual_url, // legacy `attachments` v body se ignoruje — Velín DB šablona je etalon (viz níže)
    // i18n — language zákazníka (cs/en/de/nl/es/fr/pl), default 'cs'
    language, // Shop order extras
    order_id, shipping_cost, tracking_number, tracking_url, // booking_modified — přehled úprav (původní vs nové)
    original_motorcycle, original_start_date, original_end_date, original_total_price, original_pickup_method, original_pickup_address, original_return_method, original_return_address, pickup_method, pickup_address, pickup_time, return_method, return_address, return_time, original_pickup_time, original_return_time } = body;
    if (!type || !customer_email) {
      return new Response(JSON.stringify({
        error: 'Missing type or customer_email'
      }), {
        status: 400,
        headers: {
          ...CORS,
    ...
    if (type === 'booking_reserved' && booking_id) {
      try {
        const { data: bk } = await supabase.from('bookings').select('extends_booking_id, start_date, end_date, total_price').eq('id', booking_id).maybeSingle();
        if (bk?.extends_booking_id) {
          const { data: prev } = await supabase.from('bookings').select('id, start_date, end_date, total_price, status').eq('id', bk.extends_booking_id).maybeSingle();
          ...
          const adjacent = !!prev && (dayAfter(prev.end_date) === day(bk.start_date) || dayAfter(bk.end_date) === day(prev.start_date));
          if (prev && [
            'reserved',
            'active'
          ].includes(prev.status) && adjacent) {
    ...
    if (type === 'booking_modified' && Number(price_difference || 0) < 0 && booking_id) {
      try {
        const { data: bkPay } = await supabase.from('bookings').select('stripe_payment_intent_id, stripe_session_id').eq('id', booking_id).maybeSingle();
    ...
    if (booking_id) {
      try {
        const { data: codes } = await supabase.from('branch_door_codes').select('code_type, door_code, sent_to_customer, withheld_reason, is_active').eq('booking_id', booking_id).eq('is_active', true);
        if (codes?.length) doorCodeRows = codes;
      } catch  {}
    }
    ...
    if (!vars.door_codes_block && booking_id && (type === 'booking_reserved' || type === 'door_codes')) {
      ...
      if (!needsDocs && doorCodeRows.length === 0) {
        try {
          const { data: b } = await supabase.from('bookings').select('user_id, end_date, status').eq('id', booking_id).maybeSingle();
    ...
    let dbAttachmentsList = [];
    for (const trySlug of slugsToTry){
      const { data: tpl } = await supabase.from('email_templates').select('slug, name, subject, body_html, active, subject_translations, body_translations, attachments').eq('slug', trySlug).eq('active', true).maybeSingle();
      if (tpl?.body_html) {
        ...
        const resolved = await resolveTemplateForLang(supabase, tpl, custLang);
        ...
        if (Array.isArray(tpl.attachments)) {
          dbAttachmentsList = tpl.attachments;
        }
        break;
      }
    }
    ...
    if (renderType !== type) {
      dbAttachmentsList = [];
      for (const trySlug of source === 'web' ? [
        slug,
        type
      ] : [
        type
      ]){
        const { data: tplAtt } = await supabase.from('email_templates').select('attachments').eq('slug', trySlug).eq('active', true).maybeSingle();
        if (tplAtt) {
          if (Array.isArray(tplAtt.attachments)) dbAttachmentsList = tplAtt.attachments;
          break;
        }
      }
    }
...
<p>Ještě jeden krok: doplňte prosím údaje z dokladů (občanka/pas + řidičák) — teprve pak vám pošleme <strong>nájemní smlouvu a přístupové kódy</strong> k pobočce (motorka, případně šatna).</p>
...
<p>T\u00fdm MotoGo24</p>`;
        ...
        const pickupOrig = [
          vars.original_pickup_method,
          vars.original_pickup_address
        ].filter(Boolean).join(' — ');
        const pickupNew = [
          vars.pickup_method,
          vars.pickup_address
        ...
        const returnOrig = [
          vars.original_return_method,
          vars.original_return_address
        ].filter(Boolean).join(' — ');
        const returnNew = [
          vars.return_method,
          vars.return_address
...
<p>S pozdravem,<br>T\u00fdm MotoGo24</p>`;
    ...
    if (custLang !== 'cs') {
      ...
      // Zkusit DB CZ
      for (const trySlug of slugsToTry){
        const { data: tpl } = await supabase.from('email_templates').select('slug, subject, body_html, active').eq('slug', trySlug).eq('active', true).maybeSingle();
        if (tpl?.body_html) {
      ...
      if (!csTemplateHtml) {
        ...
        if (i18nCs) {
          csSubject = i18nCs.subject;
          csTemplateHtml = i18nCs.body;
        }
      }
      if (!csSubject) {
        const fallbackFn = FALLBACK_SUBJECTS[renderType];
    ...
    const wantsAutoAtt = booking_id && (type === 'booking_reserved' || type === 'booking_abandoned' || type === 'booking_completed' || type === 'booking_modified' || type === 'booking_cancelled') || order_id && (type === 'shop_order_confirmed' || type === 'shop_order_shipped' || type === 'voucher_purchased');
    if (wantsAutoAtt) {
      try {
    ...
    if (finalAttachments.length > 0) {
      ...
      emailPayload.attachments = finalAttachments.map((a)=>({
          content: a.content,
          filename: a.filename
        }));
    }
    ...
    try {
      await supabase.from('sent_emails').insert({
        ...
        attachments_meta: finalAttachments.map((a)=>({
            filename: a.filename,
            storage_path: a.storage_path || null
          }))
      });
