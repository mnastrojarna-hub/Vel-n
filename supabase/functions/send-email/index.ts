 ...
 *  Vrací {subject, body} nebo null při chybě (caller pak nechá CZ). */ async function translateEmailTemplate(subjectCz, bodyCz, lang) {
 ...
 *  čerstvý cache překlad, jinak přeloží přes API a zacachuje. Při selhání → CZ. */ async function resolveTemplateForLang(supabase, tpl, lang) {
...
/** Send email via Resend with 2 retries (exponential backoff) */ async function sendWithRetry(emailData) {
...
serve(async (req)=>{
  ...
  try {
    ...
    const { to, template_slug, template_vars = {}, customer_id, booking_id, order_id, language, subject: subjectOverride, raw_html, raw_body, test = false, // send-broadcast/ManualSendTab compatibility fields
    channel, type, invoice_id, // Přílohy: buď přímo base64 ({filename, content}), nebo cesty v bucketu `documents`
    // ({filename, path}) — ty se zde stáhnou a zakódují do base64.
    attachments: inlineAttachments = [], attachment_paths = [] } = body;
    ...
    if (raw_html) {
      ...
      const { data: tpl, error: tplErr } = await supabase.from('email_templates').select('slug, name, subject, body_html, active, subject_translations, body_translations').eq('slug', template_slug).eq('active', true).maybeSingle();
      if (tplErr || !tpl) {
        return jsonResponse({
          success: false,
      ...
      return jsonResponse({
        success: false,
        error: 'Must provide raw_html, raw_body, or template_slug'
      }, 400);
    }
 ...
 */ async function handleInvoiceEmail(supabase, invoiceId) {
  // Load invoice with customer profile
  const { data: invoice, error: invErr } = await supabase.from('invoices').select('id, number, type, total, issue_date, due_date, status, variable_symbol, customer_id, booking_id, items, pdf_path, profiles(full_name, email)').eq('id', invoiceId).maybeSingle();
  if (invErr || !invoice) {
    return jsonResponse({
      success: false,
  ...
  try {
    ...
    const { data: dup } = await supabase.from('message_log').select('id').eq('channel', 'email').eq('recipient_email', profile.email).eq('status', 'sent').ilike('content_preview', `%${invoice.number}%`).gte('created_at', since).limit(1);
    if (dup && dup.length > 0) {
      return jsonResponse({
        success: true,
        deduped: true
  ...
  const attachmentsMeta = [];
  if (invoice.pdf_path) {
    const ext = /\.pdf$/i.test(invoice.pdf_path) ? 'pdf' : 'html';
