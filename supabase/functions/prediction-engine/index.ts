...
/** Sezónní koeficienty pro ČR (motocykly — peak sezóna květen–září). */ const SEASONAL_COEFFICIENTS = {
  1: 0.15,
  2: 0.20,
  3: 0.35,
  4: 0.60,
...
/** Načte měsíční historická data dle typu predikce. */ async function loadHistoricalData(admin, type, branchId) {
  ...
  switch(type){
    case 'demand':
      {
        let query = admin.from('bookings').select('id, start_date').gte('start_date', fromDate);
        if (branchId) {
    ...
    case 'revenue':
      {
        let query = admin.from('accounting_entries').select('amount, date').eq('type', 'income').gte('date', fromDate);
    ...
    case 'maintenance':
      {
        let query = admin.from('maintenance_log').select('cost, date').gte('date', fromDate);
        if (branchId) query = query.eq('branch_id', branchId);
    ...
    case 'stock':
      {
        let query = admin.from('inventory').select('stock, min_stock, unit_price');
        if (branchId) query = query.eq('branch_id', branchId);
        const { data: items } = await query;
        // Pro stock predikce — aktuální stav jako baseline
        const totalValue = (items ?? []).reduce((s, i)=>s + Number(i.stock ?? 0) * Number(i.unit_price ?? 0), 0);
        const now = new Date();
        return [
          {
...
/** Vypočítá trend (lineární regrese). */ function calculateTrend(data) {
...
/** Generuje predikce na N měsíců dopředu. */ function generatePredictions(historicalData, periodMonths) {
  if (historicalData.length === 0) {
    return [];
  }
  // Průměr posledních 12 měsíců
  const avgValue = historicalData.reduce((s, d)=>s + d.value, 0) / historicalData.length;
  // Trend
