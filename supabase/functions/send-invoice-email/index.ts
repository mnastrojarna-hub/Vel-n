...
import { normalizeLang, helpCardLabels, invoiceEmailSnippets } from './i18n.ts';
...
/** Replace {{var}} placeholders */ function renderTemplate(template, vars) {
  if (!template) return '';
  let result = template;
...
async function sendWithRetry(emailData) {
...
/** Map invoice type to email_templates slug */ const TYPE_TO_SLUG = {
...
serve(async (req)=>{
  ...
  try {
    ...
    const custLang = normalizeLang(language);
    if (!invoice_id) return jsonResponse({
      success: false,
    ...
    if (html_content && customer_email) {
      const snip = invoiceEmailSnippets('issued', custLang, {
      ...
      if (!RESEND_API_KEY) return jsonResponse({
        success: false,
    ...
    const { data: invoice, error: invErr } = await supabase.from('invoices').select('id, number, type, total, issue_date, due_date, status, variable_symbol, customer_id, booking_id, pdf_path, profiles(full_name, email)').eq('id', invoice_id).maybeSingle();
    if (invErr || !invoice) return jsonResponse({
      success: false,
    ...
    if (!recipientEmail) return jsonResponse({
      success: false,
    ...
    try {
      ...
      const { data: dup } = await supabase.from('message_log').select('id').eq('channel', 'email').eq('recipient_email', recipientEmail).eq('status', 'sent').ilike('content_preview', `%${invoice.number}%`).gte('created_at', since).limit(1);
      if (dup && dup.length > 0) return jsonResponse({
        success: true,
        deduped: true
      });
    } catch (e) {}
    ...
    let templateHtml = '';
    let subject = '';
    const { data: tpl } = await supabase.from('email_templates').select('slug, subject, body_html, active, subject_translations, body_translations').eq('slug', templateSlug).eq('active', true).maybeSingle();
    if (tpl?.body_html) {
    ...
    const snip = invoiceEmailSnippets(invoice.type || 'issued', custLang, {
      invoice_number: invoice.number || ''
    });
...
<p>${snip.closing}</p>`;
    ...
    if (invoice.pdf_path) {
      ...
      try {
        ...
        if (blob) {
          ...
          attachments.push({
            content: b64,
            filename: `${invoiceLabel.replace(/ /g, '-')}-${invoice.number}.${ext}`
          });
        }
      } catch (e) {
        attachmentFailed = e.message;
      }
    } else {
    ...
    if (custLang !== 'cs') {
      const csSnip = invoiceEmailSnippets(invoice.type || 'issued', 'cs', {
        invoice_number: invoice.number || ''
      });
