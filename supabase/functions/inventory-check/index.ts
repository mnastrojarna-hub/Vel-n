...
/** Ověří SERVICE_ROLE_KEY v Authorization headeru. */ function verifyServiceRole(authHeader) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: lowStockItems, error: queryError } = await admin.from('inventory').select('id, name, sku, stock, min_stock, unit, unit_price, supplier_id, branch_id').filter('stock', 'lte', 'min_stock');
    if (queryError) {
    ...
    for (const [, items] of groupedItems){
      ...
      if (supplierId) {
        const { data: existingOrder } = await admin.from('purchase_orders').select('id').eq('supplier_id', supplierId).in('status', [
          'draft',
          'sent',
          'confirmed'
        ]).maybeSingle();
      ...
      const { data: order, error: orderError } = await admin.from('purchase_orders').insert({
