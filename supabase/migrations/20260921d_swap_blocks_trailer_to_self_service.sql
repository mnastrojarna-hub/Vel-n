-- =============================================================================
-- MIGRACE: Výměna motorky nesmí přehodit rezervaci s vozíkem na samoobsluhu
-- Datum: 2026-09-21
-- Branch: claude/samoobsluzna-pobocka-vozik-2mdsxo
--
-- Navazuje na 20260921b (vozík jen na obslužné pobočce) a 20260921c (zúžení
-- triggeru). Zbývala mezera: „Výměna motorky" mění `bookings.moto_id`, takže
-- rezervace s vozíkem šla přehodit na motorku ze SAMOOBSLUŽNÉ pobočky.
--
-- Trigger to hlídat NEMŮŽE: web (`pages-upravit-rezervaci-swap.js`) i appka
-- (`reservation_swap_section.dart`) dělají krok 1 jako `p_dry_run=true` (nic
-- nezapíše → trigger nevidí), pošlou zákazníka na Stripe a skutečný zápis
-- provedou až PO zaplacení. Tvrdý pád na zápisu = peníze strženy, výměna ne.
--
