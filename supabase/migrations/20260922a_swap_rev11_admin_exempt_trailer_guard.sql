-- =============================================================================
-- MIGRACE: split_booking_moto_swap rev.11 — výjimka admin/service_role u vozíkového guardu
-- Datum: 2026-09-22
-- Branch: claude/samoobsluzna-pobocka-vozik-2mdsxo
-- Navazuje na 20260921h (rev.10, nasazeno). Nález 5. kola review (2 optiky
-- nezávisle): trigger trg_check_trailer_overlap má od `h` výjimku pro
-- service_role u změny motorky pod vozíkem (webhook-receiver commituje AŽ PO
-- zaplacení), ale guard `trailer_staffed_only` v této RPC byl bezpodmínečný —
-- webhook commit výměny (`_swap` metadata, service_role) by v závodu „motorka
-- přesunuta na samoobslužnou pobočku mezi dry-runem a webhookem" vrátil
-- {success:false} → swap_commit_failed, peníze stržené, výměna neprovedená.
-- Hlavička `h` tvrdila, že výjimka kryje „výměnu/změnu"; kryla jen změnu
-- (přímý UPDATE). Tady se politika sjednocuje: `IF NOT v_admin AND …`.
-- Tělo jinak PŘEVZATO z 20260921h beze změny (diff = jen tento řádek + komentář;
-- duplicitní REVOKE z `h` neopakujeme).
-- Idempotentní (CREATE OR REPLACE).
-- =============================================================================

-- ─── split_booking_moto_swap rev.11 ──────────────────────────────────────────
...
BEGIN
  ...
  -- rev.11 2026-09-22 (20260922a): `NOT v_admin` — stejná politika jako trigger
  -- trg_check_trailer_overlap (is_admin / service_role vyjmuti). Každý ZÁKAZNICKÝ
  -- dry-run (web, appka, process-payment pod JWT volajícího) dál blokuje PŘED
  -- platbou; jen COMMIT po zaplacení (webhook-receiver pod service_role) v závodu
  -- „motorka přesunuta na samoobsluhu mezi dry-runem a webhookem" projde, aby
  -- zákazník nezůstal se strženými penězi a neprovedenou výměnou. Vozík u takové
  -- rezervace řeší obsluha (Velín u úpravy rezervace varuje).
  IF NOT v_admin AND b.trailer_moto_id IS NOT NULL AND public.moto_is_self_service(p_new_moto_id) THEN
