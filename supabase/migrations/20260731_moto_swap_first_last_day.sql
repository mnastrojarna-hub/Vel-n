-- =============================================================================
-- MIGRACE 2026-07-31: Výměna motorky — povolit PRVNÍ den rezervace
--
-- PROBLÉM: split_booking_moto_swap odmítal p_swap_date = start_date
-- (`swap_date_out_of_range`), takže u nadcházející rezervace (např. 1.–4.8.)
-- nešla výměna od 1.8. a u rezervace začínající DNES nešla výměna vůbec
-- (jednodenní rezervace neměly žádné povolené datum). Poslední den fungoval.
--
-- ŘEŠENÍ — swap_date smí být kterýkoli den [start .. end]:
--   • swap_date > start  → SPLIT jako dosud (A zkrátit na [start..swap-1], B nová).
--   • swap_date = start + status 'reserved' → REPLACE: žádný split, jen se
--     v rezervaci vymění moto_id + cena o net (zákazník ještě nic nepřevzal,
--     druhá rezervace ani předávací protokol nedávají smysl). Mail o úpravě,
--     rozdílový doklad a app zprávu pošle stávající trg_booking_modified_email
--     (sleduje moto_id/total_price) — RPC sama nic neposílá.
--   • swap_date = start + status 'active' (převzato DNES, výměna týž den) →
--     SPLIT se stejnodennním předáním: A si nechá svůj první den jako den
--     předání (end = swap_date, jinak by end < start rozbil daterange()),
--     finančně je celý den účtován na B (a_removed pokrývá [swap..end]).
--     Párování handoffu s.start ∈ [A.end, A.end+1] dál platí (B.start = A.end).
--   • Plná výměna (swap_date = start) jen od CURRENT_DATE — retroaktivní
--     kompletní náhrada proběhlé rezervace nedává smysl.
--   • Dry-run nově vrací i 'mode' ('replace' | 'split'); web čte jen 'net'.
--
-- Nahrazuje verzi z 20260722_moto_swap_calendar_fix.sql (jinak beze změn).
-- =============================================================================
