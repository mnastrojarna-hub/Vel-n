-- =============================================================================
-- MIGRACE 2026-08-06: Věrnostní sleva (rank) na DOPLATKU při úpravě rezervace
--                     + oprava adresy pobočky (Mezná u Pelhřimova, ne Hřensko)
--
-- 1) VĚRNOSTNÍ SLEVA NA DOPLATKU — dosud ji počítal JEN split_booking_moto_swap;
--    běžné úpravy (prodloužení / změna termínu / výbava) účtovaly doplatek v
--    plné ceníkové ceně („option B" v _recalc_booking_discount řeší jen
--    promo/voucher slevy, loyalty vědomě ne). Nově pro APP rezervace
--    (booking_source='app'):
--    - `_apply_booking_changes_core`: kladný rozdíl pronájmu (dates_diff po
--      stornu a late-pickup) se sníží o pct dle AKTUÁLNÍHO ranku zákazníka
--      (stejný vzorec jako swap: level = min(20, ceil((qualifying+1)/2)),
--      pct = loyalty_levels.discount_percent). Sleva se přičte do
--      bookings.loyalty_discount_amount + aktualizuje level/percent.
--      Delivery poplatky slevě nepodléhají (parita s výpočtem při vzniku
--      rezervace — loyalty base = pronájem + výbava, bez přistavení).
--    - `update_booking_gear`: kladný rozdíl placené výbavy se sníží stejně.
--    - NOVÁ RPC `get_booking_loyalty_rate(p_booking_id)` → {level, percent}
--      pro Velín (BookingModifyModal) a klienty — vlastník rezervace nebo
--      admin; ne-app rezervace vrací 0.
--    Vratky (záporný rozdíl) beze změny — kompenzace loyalty u vratek řeší
--    strop „nevracet víc, než bylo zaplaceno" (swap) / storno tabulka.
--
-- 2) ADRESA POBOČKY — zákazníka navigace z appky dovedla na „Mezná 9, Hřensko"
--    (obec Mezná existuje i u Hřenska). Správně: Mezná 9, 393 01 Pelhřimov.
--    - branches: doplnit GPS (49.3464, 15.2119) kde chybí, city 'Mezná' →
--      'Pelhřimov', address '393 01 Mezná' → '393 01 Pelhřimov'.
--    - app_settings.company_info.address, cms_variables, email_templates,
--      document_templates: '393 01 Mezná' → '393 01 Pelhřimov' (idempotentní
--      replace, jen kde se řetězec vyskytuje).
--
-- Těla funkcí jsou 1:1 kopie posledních verzí (core z 20260804, gear z
-- 20260625_wire_recalc_into_edit_funcs) s výše popsanými cílenými změnami.
-- =============================================================================

-- ── 1a) RPC pro zjištění aktuální věrnostní sazby k rezervaci ────────────────
CREATE OR REPLACE FUNCTION public.get_booking_loyalty_rate(p_booking_id uuid)
RETURNS jsonb
...
END;
$$;
REVOKE ALL ON FUNCTION public.get_booking_loyalty_rate(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_booking_loyalty_rate(uuid) TO authenticated, service_role;

-- ── 1b) _apply_booking_changes_core — loyalty sleva na kladném rozdílu pronájmu
...
-- ── 1c) update_booking_gear — loyalty sleva na kladném rozdílu placené výbavy ─
...
-- ── 2) OPRAVA ADRESY POBOČKY (Mezná u Pelhřimova) — idempotentní data fix ────
-- GPS pobočky doplnit tam, kde chybí (navigace v appce preferuje souřadnice —
-- textová adresa „Mezná 9" je nejednoznačná, existuje i Mezná u Hřenska).
UPDATE branches SET gps_lat = 49.3464, gps_lng = 15.2119
 WHERE (gps_lat IS NULL OR gps_lng IS NULL)
   AND (address ILIKE '%mezná%' OR city ILIKE '%mezná%');

UPDATE branches SET city = 'Pelhřimov' WHERE btrim(COALESCE(city, '')) = 'Mezná';
UPDATE branches SET address = replace(address, '393 01 Mezná', '393 01 Pelhřimov')
 WHERE address LIKE '%393 01 Mezná%';
UPDATE branches SET address = replace(address, 'Mezná 9, Mezná', 'Mezná 9, Pelhřimov')
 WHERE address LIKE '%Mezná 9, Mezná%';

UPDATE app_settings
   SET value = jsonb_set(value, '{address}',
                to_jsonb(replace(value->>'address', '393 01 Mezná', '393 01 Pelhřimov')))
...
UPDATE email_templates
   SET body_translations = replace(body_translations::text, '393 01 Mezná', '393 01 Pelhřimov')::jsonb
 WHERE body_translations IS NOT NULL AND body_translations::text LIKE '%393 01 Mezná%';

UPDATE document_templates SET content_html = replace(content_html, '393 01 Mezná', '393 01 Pelhřimov')
 WHERE content_html LIKE '%393 01 Mezná%';
UPDATE document_templates
   SET content_translations = replace(content_translations::text, '393 01 Mezná', '393 01 Pelhřimov')::jsonb
 WHERE content_translations IS NOT NULL AND content_translations::text LIKE '%393 01 Mezná%';
