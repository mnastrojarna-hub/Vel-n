-- =============================================================================
-- MIGRACE: Změna motorky nesmí přehodit rezervaci s vozíkem na samoobsluhu
-- Datum: 2026-09-21
-- Branch: claude/samoobsluzna-pobocka-vozik-2mdsxo
--
-- Navazuje na 20260921b/c/d. Adversariální review odhalilo, že motorku
-- rezervace mění DVĚ různé cesty a ošetřena byla zatím jen jedna:
--   1) „Výměna motorky" → `split_booking_moto_swap`      … pokryto 20260921d
--   2) „Změna motorky"  → `apply_booking_changes`        … TATO migrace
--      (web /upravit-rezervaci záložka `moto`, appka `EditMotoChangeSection`,
--       a `apply_booking_changes_anon` = veřejný AI chat `ai-public-agent`,
--       který `new_moto_id` přeposílá dál)
-- Obě končí v `_apply_booking_changes_core`, kde kontrola vozíku chyběla úplně
-- (`grep -i trailer` v celé funkci nevrací nic), takže rezervace s vozíkem šla
-- přehodit na motorku ze SAMOOBSLUŽNÉ pobočky — přesně stav, kterému má celá
-- tahle série zabránit.
--
-- Trigger to hlídat NEMŮŽE (a 20260921c mu `moto_id` záměrně odebralo): web
-- commituje změnu po Stripe přímým `UPDATE bookings`, který RPC vůbec nevolá,
-- a tvrdý pád na zápisu by znamenal stržené peníze a neprovedenou změnu.
-- Kontrola proto sedí mezi ranými validacemi, které se vyhodnotí i v dry-runu,
-- tedy PŘED platbou. Nový chybový kód: `trailer_staffed_only` (shodný se
-- `split_booking_moto_swap`; web i appka ho mají namapovaný).
--
-- Tělo funkce je PŘEVZATO BEZE ZMĚNY z 20260912_moto_swap_no_storno.sql;
-- jediný rozdíl je vložený blok IF v_b.trailer_moto_id IS NOT NULL ... níže.
