-- =============================================================================
-- MIGRACE 2026-07-31b: Výměna motorky — STROP VRATKY dle skutečně zaplacené ceny
--
-- PROBLÉM: net výměny se počítal z CENÍKOVÝCH denních cen staré motorky.
-- Rezervace zaplacená méně (promo kód, voucher, 100% sleva, late-pickup sleva,
-- ručně upravená cena) by při výměně dostala zpět víc, než reálně zaplatila —
-- zákazník by na výměně vydělal.
--
-- ŘEŠENÍ: hodnota odebraných dní staré motorky se dobropisuje MAXIMÁLNĚ do výše
-- POMĚRNÉ části skutečně zaplacené ceny pronájmu:
--   paid_rental  = GREATEST(total_price − delivery_fee − extras_price, 0)
--   removed_paid = round(paid_rental × ceník(odebrané dny) / ceník(celý termín))
--   a_credit     = LEAST(ceník(odebrané) − věrnostní sleva, removed_paid)
--   net          = (cena B po slevě) − a_credit
-- Plně platící zákazník: strop se ≈ nedotkne (poměr = 1). Zákazník se slevou:
-- vratka jen poměrná část toho, co zaplatil; 100% promo → vratka 0 a nová
-- motorka za zbývající dny se doplácí v plné ceně. Doplatek (net>0) tím u
-- zlevněných rezervací mírně roste — záměr (zákazník nesmí na výměně vydělat).
-- Zkrácení A v commitu odečítá stejný a_credit (cena A klesne jen o dobropis).
--
-- Nahrazuje rev.3 z 20260731_moto_swap_first_last_day.sql (jinak beze změn).
-- =============================================================================
