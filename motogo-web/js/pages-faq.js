// ===== MotoGo24 Web — FAQ stránka s taby =====
...
MG.route('/jak-pujcit/faq', function(app){
  var bc = MG.renderBreadcrumb([{label:'Domů',href:'/'},{label:'Jak si půjčit',href:'/jak-pujcit'},'Často kladené dotazy']);

  var d = MG._faqData;
  var allItems = [].concat(d.reservations, d.borrowing, d.conditions, d.delivery, d.travel, d.vouchers);

  var tabs = [
    {id:'all', label:'Vše (' + allItems.length + ')', items: allItems},
    {id:'reservations', label:'Rezervace (' + d.reservations.length + ')', items: d.reservations},
    {id:'borrowing', label:'Výpůjčka a vrácení (' + d.borrowing.length + ')', items: d.borrowing},
    {id:'conditions', label:'Výbava a podmínky (' + d.conditions.length + ')', items: d.conditions},
    {id:'delivery', label:'Přistavení (' + d.delivery.length + ')', items: d.delivery},
    {id:'travel', label:'Cesty do zahraničí (' + d.travel.length + ')', items: d.travel},
    {id:'vouchers', label:'Poukazy (' + d.vouchers.length + ')', items: d.vouchers}
  ];
  ...
  tabs.forEach(function(t){
    ...
    t.items.forEach(function(faq){
      panesHtml += MG.renderFaqItem(faq.q, faq.a);
    });
    panesHtml += '</div></div>';
  });
  panesHtml += '</div>';
