...
// Adaptace pro PHP routing: query parametry z window.location.search místo hash
(function(){
  var MG = window.MG || {};
  window.MG = MG;

  // Helper: parse query params from window.location.search
  ...
  async function init(){
    ...
    if(!sid && !oid && !bid){
      el.innerHTML = confirmError('Chybí identifikátor platby.');
      return;
    }
    ...
    if(isFreeBooking){
      ...
      for(var i = 0; i < 6; i++){
        try {
          ...
          if(r.data && r.data.payment_status === 'paid'){ el.innerHTML = confirmSuccess(r.data); found = true; break; }
          if(r.data && r.data.payment_status === 'unpaid' && i < 5){ await new Promise(function(ok){ setTimeout(ok, 1500); }); continue; }
          if(r.data){ el.innerHTML = confirmPending(r.data); found = true; break; }
        } catch(e){ console.warn('[CONFIRM] free booking poll error', e); }
        await new Promise(function(ok){ setTimeout(ok, 1500); });
      }
      if(!found) el.innerHTML = confirmPending(null);
      return;
    }
      ...
      for(var i = 0; i < 12; i++){
        try {
          ...
          if(r.data && r.data.payment_status === 'paid'){
            ...
            el.innerHTML = confirmShopSuccess(r.data, vc.data || []);
            found = true; break;
          }
          if(r.data && r.data.payment_status !== 'paid' && i < 11){ await new Promise(function(ok){ setTimeout(ok, 2000); }); continue; }
          if(r.data){ el.innerHTML = confirmShopPending(r.data); found = true; break; }
        } catch(e){ console.warn('[CONFIRM] shop poll error', e); }
        await new Promise(function(ok){ setTimeout(ok, 2000); });
      }
      if(!found) el.innerHTML = confirmShopPending(null);
      return;
    }
    ...
    for(var i = 0; i < 10; i++){
      try {
        ...
        if(r.data && r.data.payment_status === 'paid'){ el.innerHTML = confirmSuccess(r.data); found = true; break; }
        if(r.data && r.data.payment_status === 'unpaid' && i < 9){ await new Promise(function(ok){ setTimeout(ok, 2000); }); continue; }
        if(r.data){ el.innerHTML = confirmPending(r.data); found = true; break; }
  ...
  function confirmShopSuccess(order, vouchers){
    var voucherHtml = '';
    if(vouchers && vouchers.length > 0){
      ...
      vouchers.forEach(function(v){
        ...
        voucherHtml += '<div style="background:#fff;border:2px solid #1a8c1a;border-radius:12px;padding:1rem;margin-bottom:.75rem;text-align:center">' +
          ...
          '<div style="font-size:.8rem;color:#6b7280;margin-top:4px">Platný do ' + validUntil + '</div></div>';
      });
      voucherHtml += '</div>';
    }
    return '<div style="text-align:center;padding:2rem 0">' +
      ...
      '<p><strong>E-mail:</strong> ' + (order.customer_email||'') + '</p></div>' +
      '<p>Na váš e-mail jsme odeslali potvrzení s detaily objednávky' + (vouchers && vouchers.length > 0 ? ' a kódem poukazu' : '') + '.</p>' +
      '<p style="margin-top:1.5rem"><a class="btn btngreen" href="/">Zpět na úvodní stránku</a></p></div>';
  }

  function confirmShopPending(order){
    return '<div style="text-align:center;padding:2rem 0"><div style="font-size:4rem;margin-bottom:1rem">&#9203;</div>' +
      '<h1>Platba se zpracovává</h1><p>Vaše platba byla odeslána a čeká na potvrzení od banky.</p>' +
      '<p>Potvrzení obdržíte e-mailem během několika minut.</p>' +
      (order ? '<p><strong>Objednávka:</strong> ' + (order.order_number || '') + '</p>' : '') +
      '<p style="margin-top:1.5rem"><a class="btn btngreen" href="/">Zpět na úvodní stránku</a></p></div>';
  }

  function confirmSuccess(b){
    return '<div style="text-align:center;padding:2rem 0"><div style="font-size:4rem;margin-bottom:1rem">&#10004;</div>' +
      ...
      '<p><strong>Stav:</strong> Zaplaceno &#10004;</p>' +
      '<p><strong>E-mail:</strong> ' + (b.customer_email||'') + '</p></div>' +
      '<p>Na váš e-mail jsme odeslali potvrzení s detaily rezervace.</p>' +
      '<p style="margin-top:1.5rem"><a class="btn btngreen" href="/">Zpět na úvodní stránku</a></p></div>';
  }

  function confirmPending(b){
    return '<div style="text-align:center;padding:2rem 0"><div style="font-size:4rem;margin-bottom:1rem">&#9203;</div>' +
      '<h1>Platba se zpracovává</h1><p>Vaše platba byla odeslána a čeká na potvrzení od banky.</p>' +
      '<p>Potvrzení obdržíte e-mailem během několika minut.</p>' +
  ...
  function confirmError(msg){
    return '<div style="text-align:center;padding:2rem 0"><div style="font-size:4rem;margin-bottom:1rem">&#9888;</div>' +
      '<h1>Něco se nepovedlo</h1><p>' + msg + '</p>' +
      '<p>Kontaktujte nás prosím na <a href="tel:+420774256271">+420 774 256 271</a></p>' +
      '<p style="margin-top:1.5rem"><a class="btn btngreen" href="/rezervace">Zkusit znovu</a></p></div>';
  }

  // Run after Supabase SDK is ready
