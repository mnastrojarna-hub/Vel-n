...
// PHP version: auto-init on page load
MG._initPotvrzeni = async function(){
  var search = window.location.search || '';
  var sid = ''; var sm = search.match(/[?&]session_id=([^&]+)/);
  if(sm) sid = decodeURIComponent(sm[1]);
  var oid = ''; var om = search.match(/[?&]order_id=([^&]+)/);
  if(om) oid = decodeURIComponent(om[1]);
  var bid = ''; var bm = search.match(/[?&]booking_id=([^&]+)/);
