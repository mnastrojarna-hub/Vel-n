<?php
// ===== MotoGo24 Web PHP — E-shop košík =====
// Server emituje jen layout + i18n labely; obsah košíku se vykresluje
// z localStorage v js/cart.js (`MGCart.renderCartPage`). Tím přežije
// košík refresh, zůstává guest-friendly a šetří DB requesty.

$bc = renderBreadcrumb([
    ['label' => t('breadcrumb.home'), 'href' => '/'],
    ['label' => t('breadcrumb.shop'), 'href' => '/eshop'],
    t('cart.title'),
]);

$emptyState = '<div class="cart-empty" data-cart-empty hidden>'
    . '<p>' . tc('cart.empty') . '</p>'
    . '<p><a class="btn btngreen" href="' . BASE_URL . '/eshop">' . tc('cart.continueShopping') . '</a></p>'
    . '</div>';

$listState = '<div class="cart-loaded" data-cart-loaded hidden>'
    . '<div class="cart-grid">'
    . '<div class="cart-items" data-cart-items></div>'
    . '<aside class="cart-summary" aria-label="' . te('cart.summaryAria') . '">'
    . '<h2>' . tc('cart.summaryTitle') . '</h2>'
    . '<dl class="cart-totals">'
    . '<div class="cart-row"><dt>' . tc('cart.subtotal') . '</dt><dd data-cart-subtotal>—</dd></div>'
    . '<div class="cart-row cart-total-row"><dt>' . tc('cart.totalNow') . '</dt><dd data-cart-total>—</dd></div>'
    . '</dl>'
    . '<p class="cart-note">' . tc('cart.shippingNote') . '</p>'
    . '<p class="cart-actions">'
    . '<a class="btn btngreen cart-checkout-btn" href="' . BASE_URL . '/objednavka" data-cart-checkout>' . tc('cart.checkout') . '</a>'
    . '</p>'
    . '<p class="cart-continue"><a href="' . BASE_URL . '/eshop">&larr; ' . tc('cart.continueShopping') . '</a></p>'
    . '</aside>'
    . '</div>'
    . '</div>';

$loadingState = '<div class="cart-loading" data-cart-loading><span class="spinner"></span> ' . tc('cart.loading') . '</div>';

$content = '<main id="content"><div class="container">' . $bc
    . '<article class="cart-page ccontent">'
    . '<h1>' . tc('cart.title') . '</h1>'
    . $loadingState . $emptyState . $listState
    . '</article>'
    . '</div></main>';

renderPage(t('cart.title') . ' | ' . t('shop.title'), $content, '/kosik', [
    'description' => mb_substr(t('cart.title') . ' — ' . t('shop.intro'), 0, 160),
    'robots' => 'noindex, follow',
    'breadcrumbs' => [
        ['name' => t('breadcrumb.home'), 'url' => siteCanonicalUrl('/')],
        ['name' => t('breadcrumb.shop'), 'url' => siteCanonicalUrl('/eshop')],
        ['name' => t('cart.title'), 'url' => siteCanonicalUrl('/kosik')],
    ],
]);
