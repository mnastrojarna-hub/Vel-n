...
$emptyHtml = '<div class="checkout-empty" data-checkout-empty hidden>'
    . '<p>' . tc('cart.empty') . '</p>'
    . '<p><a class="btn btngreen" href="' . BASE_URL . '/eshop">' . tc('cart.continueShopping') . '</a></p>'
    . '</div>';

// Section 1 — kontakt (reusable inputs ze stylu rezervace)
$contactHtml = '<section class="rez-section">'
    . '<div class="rez-section-head"><span class="rez-step-num">1</span><h2>' . tc('checkout.contact.title') . '</h2></div>'
    . '<p class="rez-section-sub">' . tc('checkout.contact.sub') . '</p>'
    . '<div class="checkout-grid-2">'
    . '<label class="checkout-field"><span>' . tc('checkout.contact.name') . ' *</span><input type="text" class="rez-input" id="co-name" name="name" autocomplete="name" required></label>'
    . '<label class="checkout-field"><span>' . tc('checkout.contact.email') . ' *</span><input type="email" class="rez-input" id="co-email" name="email" autocomplete="email" required></label>'
    . '<label class="checkout-field"><span>' . tc('checkout.contact.phone') . ' *</span><input type="tel" class="rez-input" id="co-phone" name="phone" autocomplete="tel" placeholder="+420 …" required></label>'
    . '</div>'
    . '</section>';
...
$shippingHtml = '<section class="rez-section">'
    ...
    .   '<div class="checkout-ship-info"><strong>' . tc('checkout.shipping.pickup') . '</strong>'
    .     '<small>' . tc('checkout.shipping.pickupSub') . '</small>'
    .     '<span class="checkout-ship-price">' . tc('checkout.shipping.free') . '</span></div>'
    . '</label>'
    . '<label class="rez-loc-card">'
    ...
    . '<div class="checkout-grid-2">'
    . '<label class="checkout-field checkout-field-wide"><span>' . tc('checkout.address.street') . ' *</span><input type="text" class="rez-input" id="co-street" name="street" autocomplete="street-address"></label>'
    . '<label class="checkout-field"><span>' . tc('checkout.address.zip') . ' *</span><input type="text" class="rez-input" id="co-zip" name="zip" autocomplete="postal-code" inputmode="numeric"></label>'
    . '<label class="checkout-field"><span>' . tc('checkout.address.city') . ' *</span><input type="text" class="rez-input" id="co-city" name="city" autocomplete="address-level2"></label>'
    . '<label class="checkout-field"><span>' . tc('checkout.address.country') . ' *</span>'
...
$summaryHtml = '<section class="rez-section">'
    . '<div class="rez-section-head"><span class="rez-step-num">3</span><h2>' . tc('checkout.summary.title') . '</h2></div>'
    . '<div class="checkout-summary-list" data-checkout-items></div>'
    . '<dl class="cart-totals checkout-totals">'
    . '<div class="cart-row"><dt>' . tc('cart.subtotal') . '</dt><dd data-checkout-subtotal>—</dd></div>'
    . '<div class="cart-row"><dt>' . tc('cart.shipping') . '</dt><dd data-checkout-shipping>—</dd></div>'
    . '<div class="cart-row cart-total-row"><dt>' . tc('cart.total') . '</dt><dd data-checkout-total>—</dd></div>'
    . '</dl>'
    . '<label class="checkout-field"><span>' . tc('checkout.notes') . '</span>'
    ...
    .   '<a class="btn btngrey" href="' . BASE_URL . '/kosik">&larr; ' . tc('checkout.backToCart') . '</a>'
    .   '<div class="rez-step2-pay">'
    .     '<span class="rez-step2-amount" data-checkout-pay>—</span>'
    .     '<button type="submit" class="btn btngreen" data-checkout-submit>' . tc('checkout.pay') . '</button>'
    .   '</div>'
    . '</div>'
    . '</section>';
...
$loadingHtml = '<div class="checkout-loading" data-checkout-loading><span class="spinner"></span> ' . tc('cart.loading') . '</div>';

$content = '<main id="content"><div class="container">' . $bc
    . '<article class="checkout-page ccontent pcontent">'
    . '<h1>' . tc('checkout.title') . '</h1>'
    . '<p class="checkout-intro">' . tc('checkout.intro') . '</p>'
    . $loadingHtml
    . $emptyHtml
