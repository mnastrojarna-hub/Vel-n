...
- **norm_name** (text GENERATED ALWAYS AS `public.poi_norm_name(name)` STORED, index `idx_poi_catalog_norm_name`) — **NEW 2026-09-20 (`20260920f_poi_dedupe.sql`)**: normalizovaný název pro hledání duplicit (bez diakritiky, interpunkce a vedoucího druhového slova: „Zřícenina hradu Kumburk" → `kumburk`). Uložený schválně — počítat regulární výrazy pro každou kandidátní dvojici znamenalo 50 s na dotaz (PostgREST by ho utnul). **POZOR: mění-li se tělo `poi_norm_name()`, je nutné sloupec přepočítat, uložené hodnoty se samy neaktualizují.**
