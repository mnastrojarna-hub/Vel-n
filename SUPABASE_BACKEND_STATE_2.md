...
- **continues_booking_id** (UUID FK→bookings ON DELETE SET NULL, **NEW 2026-07-21** — `20260720_moto_handoff_switch.sql`) — výměna motorky bez přerušení: tato rezervace navazuje na `continues_booking_id` (jiná motorka téhož zákazníka). Plní `swap_handoff_bookings` (Model A, switch) i `split_booking_moto_swap` (Model B, split). Řídí výjimku v `check_user_booking_overlap` (dvojice A↔B není kolize) a rozšíření blokace v `get_moto_booked_dates` (stará moto blokuje do vrácení protokolem). Index `idx_bookings_continues`.
- **extends_booking_id** (UUID FK→bookings ON DELETE SET NULL, **NEW 2026-08-10** — `20260810_consecutive_booking_extension.sql`) — NAVAZUJÍCÍ rezervace = prodloužení: tato rezervace navazuje den po dni na ŽIVOU rezervaci (reserved/active) TÉHOŽ zákazníka na TUTÉŽ motorku. Plní automaticky BEFORE INSERT trigger `trg_detect_consecutive_booking` (exception-safe). JEN PREZENTACE — žádný zásah do plateb/faktur/kódů: `send-booking-email` u type=booking_reserved vyrenderuje šablonu (web_)booking_modified s payloadem prodloužení (slug v message_log zůstává (web_)booking_reserved kvůli dedupu cronu), Velín ukazuje badge „PRODLOUŽENÍ". Záměrně ODDĚLENÉ od `continues_booking_id` (ten = výměna motorky, věší se na něj kalendář/kódy). Partial index `idx_bookings_extends_booking_id`.
- ~~pickup_date, return_date (timestamptz pro overlap check)~~ — **OPRAVA 2026-06-08:** tyto sloupce v reálné DB **NEEXISTUJÍ** (ověřeno `column "pickup_date" does not exist`). Overlap se kontroluje výhradně přes `start_date`/`end_date` — triggery `check_booking_overlap` a `check_user_booking_overlap` — obě od 2026-09-11 (`20260911_booking_overlap_inclusive_days.sql`) porovnávají INKLUZIVNÍ dny `start_date::date <= X.end_date::date AND end_date::date >= X.start_date::date` (dřív `tstzrange(start_date,end_date)` = '[)' → jednodenní rezervace = prázdný rozsah, nikdy nekolidovala).
- **trailer_moto_id** (UUID FK→motorcycles ON DELETE SET NULL, **NEW 2026-06-16** — migrace `20260616_trailer_addon.sql`) — přiřazený kus vozíku, když si zákazník přidá **vozík jako příslušenství** (gear add-on, ne samostatná rezervace). Blokuje kalendář vozíku: `get_moto_booked_dates` má UNION větev `trailer_moto_id`, BEFORE INSERT/UPDATE trigger `check_trailer_overlap` odmítne už obsazený kus (i kolizi standalone × gear). Web 400 Kč/den (řádek v `booking_extras`+`extras_price` → propíše se do ZF/DP/dobropisu i smlouvy), app zdarma (řádek 0 Kč). Volný kus vybírá RPC `get_trailer_availability`. Index `idx_bookings_trailer_moto_id`. **UPDATE 2026-09-21 (`20260921b_trailer_only_on_staffed_branches.sql`):** vozík se smí přidat POUZE k motorce z **obslužné** pobočky — samoobsluha ho nemá kde mít ani kdo předat. Hlídá to `get_trailer_availability` (nový arg `p_moto_id` → 0 pro samoobslužnou; pool kusů zůstává společný pro celou firmu — rozhoduje jen pobočka rezervované motorky) a tvrdě trigger `trg_check_trailer_overlap`; web (`MG._refreshTrailer`) i obě appky (`BookingFormExtrasSection`) dlaždici „Vozík“ na samoobslužné pobočce vůbec nezobrazí.
- **actual_return_date** — skutečné datum vrácení
- **pickup_method, pickup_address** — způsob vyzvednutí. **POZOR (ověřeno 2026-09-23 proti živé DB):** `create_web_booking` (web, `ai-public-agent`, `mcp-server`) metody NEzapisuje → web přistavení = `pickup_method='store'` (DEFAULT) + vyplněná `pickup_address` (totéž `return_*`); RPC úprav (`_apply_booking_changes_core`) adresu přes COALESCE nemaže. Kód, který rozlišuje „na pobočce“ (samoobsluha — návrat 23:59), proto bere přistavení = `method='delivery'` NEBO vyplněná adresa.
...
- **mod_surcharge_due / mod_surcharge_vs / mod_surcharge_requested_at / mod_surcharge_paid_at / mod_email_payload** (NUMERIC / TEXT / TIMESTAMPTZ / TIMESTAMPTZ / JSONB, vše DEFAULT NULL, **NEW 2026-07-27** — `20260727_booking_edit_surcharge_flow.sql`) — **doplatek za úpravu rezervace ve Velíně** (úprava ZAPLACENÉ rezervace s „Zákazník doplatí"). `mod_surcharge_due` = dlužný doplatek (kumulativní přes víc úprav; NULL/0 = nic nedluží), plní `BookingModifyModal` ve STEJNÉM UPDATE jako úprava → trigger `trg_booking_modified_email` mail NEODEŠLE, payload uschová do `mod_email_payload` (deferred). `mod_surcharge_vs` = číselný VS pro platbu doplatku (sekvence `booking_vs_seq`, přiděluje RPC `set_booking_mod_surcharge` volaná z edge `qr-payment` v režimu surcharge; zákazník dostane mail ze šablony `booking_qr_payment_surcharge` s QR + účtem + VS). `mod_surcharge_paid_at` nastaví RPC `confirm_booking_surcharge` (Velín → detail rezervace → „Potvrdit doplatek") — teprve ta odešle odložený `booking_modified` mail (send-booking-email si vygeneruje rozdílový DP source='edit') + in-app zprávu, pak `mod_email_payload` vyčistí. Velín ukazuje badge „DOPLATEK — ČEKÁ · X Kč · VS …".
...
- **restored_at** (TIMESTAMPTZ DEFAULT NULL, **NEW 2026-08-10** — `20260810_restore_to_pending.sql`) — okamžik posledního OBNOVENÍ zrušené rezervace ve Velíně (tlačítko „Obnovit" → `pending`/`unpaid`). Non-NULL = `auto_cancel_expired_pending()` rezervaci NIKDY automaticky nezruší (lhůty 10 min/4 h se počítají z `created_at`, který je u obnovené rezervace hodiny starý — bez markeru by ji cron do 2 min zase zrušil) a `send_abandoned_booking_emails()` Path AB ji přeskočí. Obnovenou rezervaci řeší admin ručně (Potvrdit platbu / Zrušit). Plní Velín `BookingDetail.handleRestore`.
- **handover_protocol_started_at** / **handover_protocol_filled_at** (TIMESTAMPTZ DEFAULT NULL) + **handover_protocol_autofilled** (BOOLEAN NOT NULL DEFAULT false) — **NEW 2026-06-23 (`20260623c_autonomous_handover_protocol.sql`)** — autonomní předávací protokol na samoobslužné pobočce. `started_at` = start 1h okna (zákazník zadal kód ke dveřím / otevřel aktivní samoobslužnou rezervaci, RPC `start_handover_protocol_window`). `filled_at` = vyplněno (zákazníkem v appce přes `submit-handover-protocol`, nebo auto-fillem cronu `autofill_overdue_handover_protocols` po 1h). `autofilled=true` = vyplnil systém („vše dle rezervace, OK", bez podpisu). Po `filled_at` je protokol zamčený (zákazník needituje). Stav čte RPC `get_handover_protocol_state`. **+ 2026-06-23 (d):** `handover_protocol_reminder_sent_at` (TIMESTAMPTZ) — dedup push připomínky „vyplňte protokol" (~10 min před koncem okna, posílá cron). **UPDATE 2026-09-25 (`20260925b_kiosk_handover_protocol.sql`):** 1h okno i autofill ZRUŠENY (rozhodnutí majitele) — `started_at` = jen historie první výzvy z kiosku (trigger `trg_handover_from_door_event`; RPC `start_handover_protocol_window` už nezapisuje), `autofilled` zůstává jen historicky false, `reminder_sent_at` se nepoužívá. `filled_at` nastavuje výhradně edge `submit-handover-protocol` (atomický claim) / Velín; přechod NULL→hodnota → trigger `trg_handover_signed_notify_kiosk`. Zákazník tyto sloupce přes PostgREST změnit NEMŮŽE (`trg_protect_handover_columns`).
- **own_gear** (BOOLEAN DEFAULT NULL, **NEW 2026-09-25** — `20260925a_locker_codes_own_gear.sql`) — vlastní výbava řidiče: `NULL` = neuvedeno (staré appky/web) → nárok na šatnu se odvodí z prázdných velikostí řidiče (helmet/jacket/pants/gloves), `true/false` = explicitní volba (appka `payment_screen` INSERT + `update_booking_gear(p_own_gear)`, Velín `BookingModifyModal`). Jediné místo výpočtu nároku: `_booking_needs_locker(uuid)` = (NOT rider_own) OR boots_size OR některá `passenger_*_size`. Změna → `trg_sync_locker_code` (kód šatny doplní / zadrží), historie `modification_history.gear_changes.own_gear`.
- **gear_collected_at** (TIMESTAMPTZ DEFAULT NULL, **NEW 2026-09-25** — `20260925a_locker_codes_own_gear.sql`) — první zavření dveří šatny kódem této rezervace (`branch_door_events` kind=accessories, `detail.event=DOOR_CLOSED` → `trg_handover_from_door_event`, i když je protokol už podepsaný). Po nastavení `trg_sync_locker_code` kód šatny už nemění. Zákazník měnit nesmí.
- **handover_protocol_prompted_at** (TIMESTAMPTZ DEFAULT NULL, **NEW 2026-09-25** — `20260925a_locker_codes_own_gear.sql`) — poslední výzva k podpisu z kiosku (zavření šatny / `PROTOCOL_SHOWN`), přepisuje se při KAŽDÉ výzvě dokud `filled_at IS NULL`; appka (stream `bookings`) na každou novou hodnotu otevře obrazovku protokolu; push „Předávací protokol" jde nejvýš 1× za 10 min. Vrací i `get_handover_protocol_state.prompted_at` a `kiosk_*.protocol.prompted_at`. Zákazník měnit nesmí.
...
- **docs_completed_at** (timestamptz, **NEW 2026-07-10** — `20260710_web_pay_before_docs.sql`) — kdy zákazník dokončil doklady v NOVÉM flow (platba PŘED doklady, krok 3 po platbě). Nastaví reserved cron `send_missing_booking_reserved_emails` při odeslání `web_booking_reserved` (doklady OK). Jen web. Velín `BookingSummary` „Doklady doplněny (po platbě)".
- **docs_fill_seconds** (integer, **NEW 2026-07-10**) — doba platba → doklady (rezerva pro budoucí měření).
- **chosen_payment_method** (text, **NEW 2026-07-10**) — zvolená platební metoda na kroku 2 (`card`/`qr`), i u NEZAPLACENÝCH web rezervací (QR má i `pay_channel='qr'`; toto doplní i volbu karty, kterou jinak Velín u nezaplacené nepozná). Plní RPC `set_web_booking_chosen_method` z `_rezSubmitPayment`. Velín `WebRezervacniFunnel` + `BookingSummary` „Zvolená platba".
...
- **registration_source** (TEXT DEFAULT NULL) — zdroj registrace: 'app' nebo 'web'. **POZN. (oprava 2026-07-01):** DB default byl HISTORICKY chybně `'auth_trigger'` (ne NULL) — trigger `handle_new_user()` sloupec nenastavuje, takže nové profily dostaly placeholder `'auth_trigger'`, který non-null hodnotou blokoval `COALESCE(...,'app'/'web')` v RPC (`app_save_full_profile`, `create_web_booking`) → zdroj se nikdy nedoplnil. Migrací `20260701_fix_registration_source_default.sql` default vrácen na NULL + backfill z nejstarší rezervace (kdo nemá rezervaci → NULL). **FIX 2026-07-28 (`20260728_registration_source_placeholder_fix.sql`, APLIKUJE AUTO-DEPLOY po merge):** placeholder se PO 1. 7. vracel — živý `handle_new_user()` (žije v dashboardu mimo migrace) ho vkládá EXPLICITNĚ v těle, ne přes DEFAULT → 28. 7. znovu 65× 'auth_trigger'. Řešení bez zásahu do handle_new_user: BEFORE trigger `trg_normalize_registration_source` (fn `normalize_registration_source()`) zahodí 'auth_trigger' při každém INSERT/UPDATE OF registration_source → NULL projde COALESCE v RPC; + re-backfill (aktivní instalace v `app_installations` → 'app'; jinak nejstarší rezervace; jinak NULL). Hodnota 'auth_trigger' se v datech už nesmí objevit — AI Copilot i Velín ji berou jako „neznámý".
...
- description (text), **ideal_usage (text[] — POZOR: pole krátkých tagů, NE text!** mig. `20260306_moto_specs_and_debug.sql`; zápis textu sem shodil deploy 2026-08-16), features (text[]), manual_url (text)
...
- **type** (TEXT, živě DEFAULT 'samoobslužná' + CHECK `type IN ('samoobslužná','obslužná')` — ověřeno snapshotem 2026-09-23; Velín může uložit i NULL) — REŽIM pobočky: `'samoobslužná'` (výdej/vrácení 24/7 kódem) / `'obslužná'` (předává obsluha, servisní místo) — jediné hodnoty, které ukládá Velín (`BranchModal.jsx`); podle nich se řídí missing-docs maily, handover protokol i `ai-public-agent` (oprava 2026-08-03: dřívější popis „turistická/městská/…" neodpovídal realitě)
- **translations** (JSONB DEFAULT '{}') — auto-překlady pro web (notes), plní `translate-content`
- **DATA 2026-09-23 (`20260923_branch_brno_velke_nemcice.sql`, APLIKUJE AUTO-DEPLOY po merge):** pobočka **„Brno Pohořelice“ → „Brno Velké Němčice“** (`name` regexp „Pohořelice“→„Velké Němčice“, `address='Boudky'`, `zip='691 63'`, `city='Velké Němčice'` → **od `20260923b` `city='Brno - Velké Němčice'`** (živě byl název „MotoGo24 Pohořelice“ + city „Brno - Pohořelice“ → nyní „MotoGo24 Velké Němčice“ + „Brno - Velké Němčice“; ulice ověřena reverzním geokódováním souřadnic 49°0'16.821"N, 16°40'19.750"E), `gps_lat=49.0046725`, `gps_lng=16.6721528`, `coordinates` (point, v repu se nečte) ve stejném pořadí os; stejně `kiosk_devices.name`; shoda `~* 'poho[řr]elic'`). Živé schéma (snapshot 2026-09-23) má u `branches` navíc sloupce `zip` (web `kontakt.php` i AI agenti skládají adresu „address, zip city“), `coordinates` point, `email`, `opening_hours` jsonb; `type` DEFAULT 'samoobslužná' + CHECK (samoobslužná|obslužná). Volné texty (`notes`/`translations`, FAQ, CMS, šablony) se ručně opravují ve Velíně podle diagnostického SELECTu (STATE_6 2026-09-23 (A)). **Pravidlo časů u `type='samoobslužná'` (od 2026-09-23, upřesněno týž den — 2026-09-23 (D)):** čas VYZVEDNUTÍ zákazník volí vždy (řídí slevu za pozdní vyzvednutí), hodina předem potřeba není (jen ne do minulosti), u přistavení min. teď + 6 h. Čas VRÁCENÍ na pobočku appka i web SKRYJÍ a do `bookings.return_time` ukládají **`23:59`** (odvoz z adresy zákazníka čas vrácení ponechává); edge `generate-document` dává ve smlouvě u samoobsluhy konec dne 23:59 i pro STARŠÍ řádky. `pickup_time='00:01'` je jen stará hodnota „bez času“ z krátkého období (A)–(D) (rezervace přes AI/MCP) — všude se bere jako bez času.
...
- **audience** (TEXT NOT NULL DEFAULT 'adult', CHECK IN ('adult','child','both'), 2026-05-05) — pro koho jsou velikosti určené. Web `/rezervace` filtruje podle `motorcycles.license_required` (`N` = dětská) až s feature flagem `inventory_v2`. Velín ManageTypesModal nabízí dropdown 👤/👶/👤👶. **2026-09-25 (`20260925b_kiosk_handover_protocol.sql`):** `kiosk_sync_config.gear_sizes {adult:{helmet,jacket,pants,boots,gloves}, child:{…}}` bere jen těch 5 klíčů (= sloupce `bookings.*_size`) s `is_active` a neprázdnými `sizes`; `adult` = audience adult/both, `child` = adult přepsané řádky audience child/both (dětský protokol na displeji nikdy bez velikostí). Appka čte tabulku přímo (policy „Public read").
...
- code_type (TEXT CHECK: motorcycle/accessories) — **2026-09-25 (`20260925a_locker_codes_own_gear.sql`):** řádek `accessories` (kód ŠATNY) vzniká JEN při nároku na šatnu (`_booking_needs_locker`: půjčená výbava řidiče NEBO boty NEBO výbava spolujezdce); čistě „vlastní výbava" = jen `motorcycle`. Pojmenování všude „kód šatny" (dřív „kód k příslušenství").
...
- **hardware** (jsonb NOT NULL DEFAULT '{}') — **NEW 2026-09-09** HW mapa pobočky pro RPi řídicí jednotku (Velín → editor; přepisuje lokální `raspberry/motogo-box/config/brno-9zone.yaml` po top-level klíčích; **`devices` neprázdné nahrazují lokální seznam celý — 2026-09-26**, dřív se slučovaly a Shelly/wav617b ze šablony se přilepily ke každé pobočce). Tvar: `devices {name:{type:wav645|wav617|shelly_rgbww, host, port:502, unit_id:1}}`, `timings {lock_pulse_ms, lock_hold_until_open (bool, 2026-09-26: zámek bez paměti — relé drží od kódu do otevření dveří, max. door_open_timeout_s; šablona true), door_open_timeout_s, door_close_debounce_ms, light_after_close_s, music_after_close_s, maximum_session_s, forced_open_debounce_ms, pin_entry_timeout_s, overtime_alert_minutes[]}`, `polling {door_input_poll_ms, software_debounce_ms, modbus_timeout_ms, retry_delays_ms[], device_offline_after_failures}`, `contacts {closed_level:0|1}`, `security {maximum_failed_attempts, attempt_window_minutes, lockout_minutes, service_token_minutes}` (**2026-09-11 (A):** `pin_length` a `mask_pin_on_screen` odstraněny — kód na displeji je viditelný; staré klíče v uložených mapách jednotka ignoruje), `audio {volume, fade_in_ms, fade_out_ms, selector_settle_ms, selector_on_ms, device, shuffle}` + **(F) 2026-09-10:** `audio.mode` (`'selector'` výchozí = 1 zesilovač + relé \| `'multi'` = vlastní výstup + mpv na každou místnost), `audio.music_enabled` (bool, **2026-09-14**; chybí = true = zapnuto — false znamená, že se po zadání kódu hudba nespustí NIKDE, ani venku: zesilovače jsou napájené trvale, takže jinak hudbu nešlo vypnout; jednotlivé zóny přepisuje `branch_doors.hw.music_enabled`, ruční `music_on` z Velína funguje dál), `audio.outputs {out1:{device:'alsa/plughw:CARD=Box1'}, …}` (jen multi; název → ALSA zařízení dle `aplay -L`), `audio.channels {outdoor:{out:'out9', trigger:'any', dev?, coil?}}` (jen multi; kanál venek bez dveří — hraje, dokud běží aspoň jedna relace; dev/coil = volitelné enable relé) — editor sekce „Audio“ (`BranchRpiAudioHw.jsx`); jednotka blokuje mapu s neznámým/sdíleným výstupem, venek v selectoru je jen upozornění; `signal {brightness, blink_ms, pulse_ms, transition_s}`; **NEW 2026-09-11 (A), rozšířeno 2026-09-14:** `outdoor {zone:int, light:{dev, coil}, audio:{out, dev?, coil?}, light_after_close_s?:int, light_mode?:'auto'|'always'|'off', music_mode?:'session'|'always'|'off'}` (Velín rozhoduje i o NEpřítomnosti venku: jakmile má pobočka HW mapu nebo HW mapy dveří v DB a `outdoor` chybí, venek z lokální šablony jednotky se NEpoužije; jen čistě lokální mapa = šablona vč. venku) = **venek** (zóna bez dveří — v Brně zóna 9: prostor před displejem + venkovní osvětlení; NENÍ řádek `branch_doors`): `light` = relé Waveshare venkovního světla (svítí od první relace do `light_after_close_s` po poslední; chybí = `timings.light_after_close_s`), `audio.out` = výstup venku (jen `audio.mode: multi`, hraje při jakémkoli kódu), `audio.dev/coil` = volitelné enable relé; kanonický tvar bez null klíčů — editor blok „Venek“ (`BranchRpiOutdoorHw.jsx`, „Vymazat venek“ klíč smaže); legacy `audio.channels.outdoor {out, trigger, dev?, coil?}` = alias (jednotka ho dál čte, Velín už nezapisuje a při uložení bloku Venek ho převede); jednotka blokuje světlo na cívce zóny / mimo Waveshare / mimo rozsah a číslo zóny kolidující s dveřmi. Šablona Brno = 8 zón (kóje 1–7 + šatna 8) + `outdoor {zone: 9, light: {dev: 'wav617b', coil: 0}}`. **2026-09-14 (režimy venku):** venek se nastavuje JINAK než kóje a šatna (ty se řídí `timings`): `light_mode` — `auto` (výchozí, svítí od první relace do `light_after_close_s` po poslední) \| `always` (NONSTOP bez ohledu na relace; doběh se neuplatní) \| **`branch` (2026-09-25: svítí, dokud je pobočka ve Velíně otevřená — `branches.is_open` → `kiosk_sync_config.branch_is_open`, změna → okamžitý `sync_config` přes `trg_branch_open_kiosk_sync`)** \| `off` (trvale zhasnuto, ani relace nerozsvítí); `music_mode` — `session` (výchozí, hraje při jakémkoli kódu) \| `always` (nonstop) \| `off` (venku nehraje nic). Výchozí hodnoty se do mapy NEZAPISUJÍ (kanonický tvar zůstává krátký), neznámá hodnota → výchozí. Ruční příkaz z Velína (`light_on`/`light_off`) má přednost i v těchto režimech; `all_off` v režimu `always` uloží ruční vypnutí, aby „Vše vypnout“ venku opravdu zhasnulo. Editor: blok „Venek“ → Režimy (`BranchRpiOutdoorHw.jsx`). Jednotka: `config_outdoor.py` / `outdoor.py`. **2026-09-26:** `network {lan_gateway}` = ZRUŠENO 2026-09-26 (brána kabelem vznikla z chybné diagnózy a blokovala LTE) — jednotka hodnotu ignoruje (varování), Velín klíč `network` při uložení mapy odstraní; internet jde výhradně přes LTE — editor sekce „Síť pobočky“. Klíč `zones` sem NEPATŘÍ (zóny = `branch_doors.hw`); `{}` = lokální výchozí mapa.
...
- **status** (jsonb NOT NULL DEFAULT '{}') — **NEW 2026-09-09** poslední snapshot stavu RPi řídicí jednotky (RPC `kiosk_report_status`, á `status_report_s`=30 s). Tvar (= `BoxController.snapshot()`, kontrakt §14): `{ts, version, uptime_s, ready, branch_name, internet, config_source:'local'|'remote', config_problems[], modules:{wav645:bool, wav617a:bool, wav617b:bool, shelly1..4:bool}, audio:{playing_zone, player_ok}, health:{lte:{state, operator, rssi, rsrp, reconnects, usb_resets}, sys:{cpu_temp, throttled, disk_free_pct, mem_free_pct, load1, uptime_s}, internet, ts}, zones:[{zone, door_id, box_number, kind, label, state:SECURED|WAITING_FOR_OPEN|DOOR_OPEN|CLOSED_CONFIRMATION|FAULT, door_closed, fault, light, signal, music, music_enabled (2026-09-14), session_started_at, booking_id, last_event}], notice}`. U tabletů `{}`. **UPDATE 2026-09-10 (D):** navíc `update:{state:idle|waiting|running|rebooting|done|failed, kind:software|system|reboot, ref, since, error, last:{kind, state, ref, rollout_id, started_at, finished_at, error, reboot_required, output_tail, reboot_at?}}` (`SoftwareUpdater.status()`; `last` = poslední dokončený běh software/OS ze SQLite kv `last_update`, přežije restart i reboot — `kiosk_rollout_tick` z něj čte výsledek OS aktualizace; běh `reboot` ho nepřepisuje) a v `health.sys` nová pole `reboot_required` (bool — existuje `/run/reboot-required`; na Debianu ho zakládá motogo-sysupdate / hook po unattended-upgrades), `os` (PRETTY_NAME), `kernel` (`uname -r`), `last_unattended_at` (ISO mtime `/var/lib/apt/periodic/upgrade-stamp`). Velín: `FleetUpdates.jsx` (tabulka jednotek, chip „Restart OS potřebný“), `BranchRpiZones.jsx` (řádek „Aktualizace: …“), `BranchSelfService.jsx` (chip). **UPDATE 2026-09-10 (E):** `diagnostics:{running, id, mode:'full'|'network', step, step_title, done[], steps[], elapsed_s, error, last:{id, ts, ok, mode, problems:int, warnings:int, hosts, zones_ok, zones_total, duration_s, source}|null}` (= `NetworkDiagnostics.status()`, kontrakt §24; Velín z něj během čekání na report ukazuje „krok X (n/m)“). **UPDATE 2026-09-10 (F):** `audio` = `{mode:'selector'|'multi', playing_zone (selector: hrající zóna, multi: první hrající — kompatibilita), playing_zones[], channels[] (hrající kanály bez dveří, např. ['outdoor']), player_ok, playlist_count (multi: součet), device, players:{<out|'mpv'>:{alive, playlist_count, device, playing?, target?}}, library:{tracks, synced, pending, failed, last_sync_at, targets:{all, legacy, outdoor, 'door:<uuid>'…}, syncing, reason}|null}` (kontrakt §14; `library` = stav stahování skladeb z bucketu `branch-music`) — Velín `BranchMusicParts.jsx` z `library`/`mode` kreslí chip „Jednotka: n/m staženo / stahuje k / k selhalo“ + režim; `zones[].music` = hraje v té zóně (`audio.is_playing`). **UPDATE 2026-09-11 (A), rozšířeno 2026-09-14:** `outdoor:{zone, configured:bool, light:bool, active:bool, manual:bool|null, audio_out, music:bool, light_ref:'wav617b[0]'|null, off_in_s:int|null, light_mode:'auto'|'always'|'off', music_mode:'session'|'always'|'off', music_manual:bool|null}` (= `OutdoorController.status()`, kontrakt §14/§26; vždy přítomen, `configured:false` = jednotka bez venku; `manual` null = automaticky dle relací, true = ručně drží, false = ručně zhasnuto do další relace; `light_mode`/`music_mode` = režimy venku z HW mapy — Velín z nich kreslí chipy na dlaždici, když nejsou výchozí; `music_manual` null = řídí `music_mode`, true/false = ruční Hudba ▶/⏹ z Velína a drží do dalšího příkazu nebo restartu) — Velín `BranchRpiOutdoorTile.jsx` (dlaždice „Venek“ v `BranchRpiZones.jsx`, jen při `configured`): Světlo ▶/⏹ = `light_on`/`light_off {zone}`, Hudba = `music_on`/`music_off {zone}`, Test = `zone_test {zone}` (`kiosk_commands` se `zone` = číslo venku). **UPDATE 2026-09-20 (bez SQL změny — jen nová pole ve `status`):** `shell:{free:bool, free_s:int}` = servisní terminál, je volné psaní odemčené z Velína? (CONTRACT §27; servisní heslo ho má samo a ve `status` se neprojeví); `health.lan:{interface, state, ipv4, ok, problem:'no_link'|'no_address'|'missing'|null, action}` = hlídka I/O sítě (eth0 bez adresy; `ok:null` = nezjištěno, NENÍ porucha) — Velín z toho kreslí chip „I/O síť: eth0 bez linku“; `health.lte` navíc `unlock_required`/`unlock_retries` (zámek SIM — PIN se pozná i když se modem tváří jako `searching`), `usb_present`/`modem_gone` (modem na USB vs. v ModemManageru) a `last24h:{reconnect, modem_reset, usb_reset, reboot}` (počty obnov LTE za 24 h).
...
- **hw** (jsonb NOT NULL DEFAULT '{}') — **NEW 2026-09-09** HW mapa zóny pro RPi: `{zone:int, lock:{dev,coil}, contact:{dev,input}, light:{dev,coil}, audio:{dev,coil}, red:{dev,light}, green:{dev,light}, closed_level?:0|1}` (**(F) 2026-09-10:** `audio` v režimu multi = `{out, dev?, coil?}` — `out` = název výstupu z `hardware.audio.outputs` (žádné dvě zóny ani zóna + venek nesmí sdílet výstup), `dev/coil` = volitelné enable relé zesilovače; v selectoru zůstává relé `{dev,coil}` a `out` se ignoruje); `dev` = klíč z `branch_kiosk_config.hardware.devices` (lock/light/audio = relé Waveshare coil 0-based, contact = vstup WAV617, red/green = Shelly light id 0–4), `closed_level` = override globálního `contacts.closed_level`. **2026-09-14:** navíc volitelný `music_enabled` (bool; chybí = řídí se hlavním vypínačem pobočky `hardware.audio.music_enabled`) = hraje v této zóně po zadání kódu hudba, a volitelný `timings {door_open_timeout_s?, light_after_close_s?, music_after_close_s?, maximum_session_s?}` = INDIVIDUÁLNÍ časování jen této zóny (chybějící klíč = globální `hardware.timings`); kóje 1–7 se nechávají prázdné (stejné nastavení), šatna se tak dá nastavit jinak (zákazník se v ní převléká déle). Jednotka: `models.ZONE_TIMING_KEYS` / `zone_timings()`, `ZoneController.timings`; do `hw_signature` se NEPOČÍTÁ, takže změna času NEVYVOLÁ přestavbu HW (ta dělá `all_off` a zhasla by světlo v obsazené kóji). Editor: řádek „Vlastní čas“ v mapování dveří. **2026-09-25:** volitelný `light_until_moto_code` (bool; šatna) — světlo po zavření dveří NEzhasne doběhem, zhasne ho až kód motorky (pojistka `maximum_session_s` zóny); editor „Světlo: Šatna“. `{}` = dveře bez HW zóny (tabletový režim přes `relay_url`/`light_url`). Editor ve Velíně (`BranchRpiHardware.jsx`, tlačítko „Načíst výchozí mapu (šablona Brno, 8 zón + venek)“ — **2026-09-11 (A):** šablona má 8 zón, zóna 9 = venek v `hardware.outdoor` (ne dveře); `zone` dveří nesmí kolidovat s `hardware.outdoor.zone`).

#### branch_service_codes
- **code** (text), **label**, **is_active**, **created_by** — UNIQUE(branch_id, code) WHERE is_active
- **action** (text NOT NULL DEFAULT 'service' CHECK service/diagnostics) — **NEW 2026-09-10** účel hesla: `service` = servisní panel (otevírání, světla, hudba, restart, diagnostika), `diagnostics` = na RPi JEN spuštění diagnostiky pobočky (od 2026-09-10 (E) kompletní běh; nic neotevírá). Do zařízení jde přes `kiosk_sync_config.service_codes[{h, action, label}]` a `kiosk_resolve_code.action`; tablet ignoruje.

#### kiosk_commands
- **device_id** (uuid FK→kiosk_devices CASCADE), **command** (CHECK open_door/music_on/music_off/identify/reload/camera_control/http_get/restart + **NEW 2026-09-09 pro RPi:** light_on/light_off/set_signal/zone_test/audio_test/all_off/reboot/sync_config/update_software + **NEW 2026-09-10:** diagnostics (params od (E): `{mode?: 'full'|'network', cameras?: [{name, kind, snapshot_url, stream_url}], reason?}`) + **NEW 2026-09-10 (D):** update_system + **NEW 2026-09-20 (`20260920c_kiosk_shell_unlock.sql`):** shell_unlock — `params {minutes}` (0 = zamknout; jinak 5–240, výchozí 30), povolí volné psaní v servisním terminálu na displeji pobočky (CONTRACT §27) + **NEW 2026-09-25 (`20260925b_kiosk_handover_protocol.sql`):** protocol_signed — `params {booking_id}`; vkládá trigger `trg_handover_signed_notify_kiosk` všem aktivním jednotkám pobočky motorky při `bookings.handover_protocol_filled_at` NULL→hodnota (jen samoobsluha, status reserved/active) + `kiosk_request_sync`; příkaz je jen urychlení (po 10 min expiruje), zdroj pravdy = `kiosk_sync_config.protocols[]`; starší software jednotky hlásí `unknown_command` — není porucha)
- **params** (jsonb — např. {relay_url, light_url, music_url, url}; **2026-09-10 (D):** `update_software {ref, rollout_id, wait_idle_s}`, `update_system {rollout_id, wait_idle_s, auto_reboot}`, `reboot {wait_idle, wait_idle_s}` — jednotka je provede, až je kóje volná; **2026-09-11 (A):** `light_on/light_off/music_on/music_off/zone_test {zone}` se `zone` = `hardware.outdoor.zone` ovládají venek — světlo ručně (on drží, off zhasne do další relace), hudba venku jen v režimu multi (jinak `result.error='outdoor_requires_multi'`), test = světlo 1 s + tón 3 s (`result {zone, outdoor:true, light, audio, error?}`)), **status** (pending/done/failed/expired), **result** (jsonb), **executed_at**

#### branch_door_events
- **device_id** (uuid FK→kiosk_devices SET NULL), **door_id** (FK→branch_doors SET NULL), **kind**, **booking_id**, **success** (bool), **detail** (jsonb), **code_masked** — **2026-09-25 (`20260925b_kiosk_handover_protocol.sql`):** `detail.event` nově i `PROTOCOL_SHOWN` (zobrazení protokolu na displeji, `kind` = motorcycle/accessories dle zadaného kódu; Velín ho NEpočítá jako otevření); trigger `trg_handover_from_door_event` (STATE_4).
...
- **ok** (bool — summary.ok), **problems** (jsonb DEFAULT '[]' — texty problémů), **summary** (jsonb DEFAULT '{}' — `{ok, problems[], hosts, internet, lte, devices_ok, devices_total}` + **od 2026-09-10 (E):** `warnings[], checks{total, ok, warn, fail, skip}, mode:'full'|'network', zones_total, zones_tested, zones_ok, sections{key: ok|warn|fail|skip}` + **2026-09-11 (A):** `outdoor: null|'ok'|'fail'|'skip'` (venek — zóna bez dveří; do `zones_*` se nepočítá, fail venku je v `problems`); `problems`/`warnings` = „Kontrola: zjištění“ z fail/warn položek protokolu, souhrnné položky zón (`group:true`) se nepočítají; starší reporty mají jen původní klíče)
- **report** (jsonb DEFAULT '{}' — celý report RPi: `{id, ts, source, reason, version, device_id, branch_name, paired, steps{name:{ok,ms,error}}, system{hostname, kernel, time, ntp_synced, metrics{cpu_temp, throttled, disk_free_pct, mem_free_pct, load1, uptime_s}, config_source, config_problems[]}, interfaces{interfaces[{name, mac, state, ipv4[], ipv6[]}], default_routes[{gateway, dev, metric}], dns[]}, lte{state, operator, access_tech, signal_quality, rssi, rsrp, rsrq, snr, nm_state}, internet{dns[], tcp, http[], ok}, supabase{paired, ok, ms, branch_name, outbox_pending}, devices[{name, type, host, port, reachable, ms, ping_ms, identified, online}], lan{subnets[], ports[], scanned_hosts, hosts[{ip, mac, ports, configured_as, modbus, shelly, http}]}, arp[], summary, duration_s, finished_at}`; **od 2026-09-10 (E)** navíc `mode` a v režimu full kroky `software{version, uptime_s, ready, services{motogo-controller|motogo-health|motogo-ui}, failed_units, audio{player_ok, playlist_count, device, music_files}, realtime{connected}, api{online, paired}, outbox_pending, events_total, recent_errors[≤10], lockout_until, code_cache{saved_at, age_s, codes, service_codes}, last_update, reboot_required, health_age_s}`, `config{branch_name, source, zones_total, zones[{zone, label, kind, door_id, box_number, roles{lock,contact,light,audio,red,green}, missing[]}], doors_without_hw[], duplicates[], timings, timings_problems[], devices, power_status_url, cameras_provided, security, outdoor{zone, light, audio, light_after_close_s, configured, present} (2026-09-11 (A))}`, `zones[{zone, label, kind, state, fault, door_closed, session_active, contact_raw, contact_consistent, io_problems[], lock{configured, module_online, coil_off}, tested, skipped_reason, light, signal, audio, shelly{red, green, expected, matches}, findings[{key, status, message}], problems[]}]`, **2026-09-11 (A)** `outdoor{zone, light{ref, module_online, coil_on}, audio_out, mode, active, tested, skipped_reason, light_ok, audio_ok, findings[], problems[]}` (jen když je venek nastaven; protokol: skupina „Venek (zóna N)“ v sekci zones + položka „Venek“ v config), `power{configured, url, ok, status, ms, error, values{battery_soc, battery_voltage, battery_power_w, pv_power_w, load_power_w, grid_present}, raw_keys[]}`, `cameras[{name, kind, url_kind, url, ok, status, ms, error, content_type}]` a **`protocol[{key, title, status, items[{id, label, status: ok|warn|fail|skip, value, message, hint?, group?}]}]`** (sekce system, software, network, lte, internet, velin, modules, config, zones, power, cameras, lan, steps — JEDINÝ formát, který Velín `BranchRpiDiagProtocol.jsx` i displej vykreslují jako protokol; export .txt ve Velíně). Tvar dle `raspberry/motogo-box/CONTRACT.md` §24; report < 512 KiB (limit RPC).)
...
#### branch_music_tracks (NEW 2026-09-10 (F), `20260910d_branch_music.sql`)
- **id** (uuid PK DEFAULT gen_random_uuid() — Velín ho generuje předem, aby se shodoval s názvem souboru), **branch_id** (uuid NOT NULL FK→branches CASCADE)
- **target** (text NOT NULL DEFAULT 'all' CHECK `target = 'all' OR target = 'outdoor' OR target ~ '^door:[0-9a-f-]{36}$'`) — `all` = společná (hraje v každém cíli bez vlastních skladeb), `outdoor` = venek (kanál bez dveří, hraje při jakémkoli kódu — jen režim multi), `door:<branch_doors.id>` = kóje / šatna
- **title** (text NOT NULL — název bez přípony, ve Velíně přejmenovatelný), **file_path** (text NOT NULL UNIQUE — cesta v bucketu `branch-music`: `<branch_id>/<id>.<ext>`), **ext** (text NOT NULL), **mime** (text), **size_bytes** (bigint NOT NULL DEFAULT 0 — jednotka ověřuje velikost staženého souboru), **duration_s** (numeric — zatím se neplní)
- **sort_order** (int NOT NULL DEFAULT 0 — pořadí ve skupině cíle, Velín ▲▼ přečísluje skupinu), **is_active** (bool NOT NULL DEFAULT true — vypnutá skladba se jednotce neposílá → smaže ji), **created_by** (uuid), **created_at**, **updated_at** (trigger `trg_branch_music_tracks_touch`)
- Index `idx_branch_music_tracks_branch` (branch_id, target, sort_order). RLS `branch_music_tracks_admin` FOR ALL `is_admin()`. Čte RPC `kiosk_sync_config` (klíč `music`, STATE_3).

#### points_of_interest (doplněk 2026-07-25)
- **norm_name** (text GENERATED ALWAYS AS `public.poi_norm_name(name)` STORED, index `idx_poi_catalog_norm_name`) — **NEW 2026-09-20 (`20260920f_poi_dedupe.sql`)**: normalizovaný název pro hledání duplicit (bez diakritiky, interpunkce a vedoucího druhového slova: „Zřícenina hradu Kumburk" → `kumburk`). Uložený schválně — počítat regulární výrazy pro každou kandidátní dvojici znamenalo 50 s na dotaz (PostgREST by ho utnul). **POZOR: mění-li se tělo `poi_norm_name()`, je nutné sloupec přepočítat, uložené hodnoty se samy neaktualizují.** Tabulky v `translate()` MUSÍ být stejně dlouhé — první verze měla o jedno „s" navíc a „Krkonošský" se normalizovalo na `krkonossku` („ý"→„u", „ť"→„s", „ú"→„t", „ž"→„y"); stejná tabulka je v appce v `places_filter.dart` (`_foldName`, hlídá ji `assert`).
- **wikidata_id** (text, částečný index `idx_poi_catalog_wikidata`) — **NEW 2026-09-20 (`20260920g_poi_photos_https_and_cron.sql`)**: QID entity ve Wikidatech. Plní ho nové seed dávky; smysl = dohledat fotku podle IDENTITY bodu (P18 / Commons kategorie), ne podle „co je do 600 m a jmenuje se podobně" (ta cesta u izolovaných vrcholů měřeně nenajde nic a občas přilepí fotku jiného místa).
- **photo_checked_at** (timestamptz, částečný index `idx_poi_photo_todo`) — marker „fotka se u tohoto bodu už zkoušela" (`20260705b`), nastavuje ho edge fn `backfill-poi-photos` po KAŽDÉM pokusu.
- **category** — od 2026-09-20 CHECK ∈ (food, castle, lookout, water, **spring**, sights, nature, other, military, aviation, tech, moto); `spring` = chip „Studánky a prameny" (`20260920e_poi_spring_category_and_recategorize.sql`). Naplnění: 4 643 položek, z toho 4 602 z OpenStreetMap (`20260920la–ld`, source `osm-springs-cz-sk-b1..b4-spring`) — Wikidata mají v celém ČR a SR jen 7 pojmenovaných studánek. Památkové studny (hradní, náměstní, vahadlové; `wikidata-tech-cz-sk-well`, 107 řádků) do `spring` ZÁMĚRNĚ nejdou — jsou to kulturní památky a patří do `sights`.
