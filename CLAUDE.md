...
**Ostatní workflows:** `snapshot-supabase.yml` (DENNĚ 03:15 UTC + ručně: snapshot živého stavu do větve `supabase-live-snapshot` pro diff s main; seznam edge funkcí bere živě z projektu), `daily-backup.yml` (DENNĚ kompletní šifrovaná záloha DB + Storage + git jako artifact, 30 dní; crony 03:30 + 13:30 UTC, guard = „existuje už dnes artifact `motogo24-backup-<datum>`?“ — NIKDY ne guard na hodinu, GitHub spouští cron se zpožděním 1–5 h; každá záloha se verifikuje (počet tabulek, COPY bloků, souborů Storage, git bundle)). Obě workflows při selhání založí issue s labelem **`backup-failed`** a po zeleném běhu ho zavřou.
