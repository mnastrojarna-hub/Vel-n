...
- **raspberry/motogo-box/** — Univerzální řídicí jednotka samoobslužných poboček pro Raspberry Pi 5 (Python/asyncio): Modbus TCP relé Waveshare (zámky, světla, audio selektor, dveřní kontakty), Shelly RGBWW signalizace, mpv audio, dotykové UI (Chromium kiosk), Supabase sync přes `kiosk_*` RPC. Počet zón, zařízení i HW mapa se nastavují VÝHRADNĚ z Velína (Pobočky → Samoobsluha); IP modulů Waveshare jednotka nastavuje SAMA (automatické zřízení přes ZLAN/UDP 1092 i z tovární 192.168.1.254, klient umí Modbus TCP i tovární RTU přes TCP 4196 — `io_provision.py`, HARDWARE.md §1); síť: internet VÝHRADNĚ přes LTE (`motogo-lte`), eth0 = jen statické adresy modulů (`motogo-lan` never-default, bez DNS); hybrid/„brána kabelem“ z 26. 9. zrušeny — cizí výchozí trasu přes eth0 dispečer i health sami mažou (`NET_FIX`); **PRAVIDLO po incidentu 26. 9.: síťové profily/trasy jednotky se NIKDY nemění bez dat z jednotky (`ip route`, `nmcli`, `resolvectl`, journal) — scan LAN nestačí; každá síťová změna v update.sh jde přes canary s rollbackem (`scripts/lib/netcanary.sh`)**; nový vzdálený příkaz = VŽDY i migrace CHECK `kiosk_commands_command_check`; `config/brno-9zone.yaml` je jen výchozí šablona (8 zón + venek; zóna 9 = venek — sekce `outdoor` HW mapy: venkovní osvětlení + hudba venku, bez dveří). Kompletní diagnostika pobočky (rozhraní, LTE, internet, Velín, moduly, software/služby, konfigurace zón, HW test prázdných kójí, FV, kamery, scan LAN + identifikace Waveshare/Shelly; režim „jen síť“): kód na displeji (`diagnostics.code` v config.yaml), servisní heslo s účelem `diagnostics` nebo Velín → „Kompletní diagnostika pobočky (Raspberry)“; protokol „kde je problém a co s tím“ na displeji i ve Velíně (+ export .txt), tabulka `kiosk_diagnostics`. Od 2026-09-25 jednotka vede zákazníka tokem **šatna → předávací protokol → motorka**: kód šatny dostane jen rezervace s výbavou k vyzvednutí (`_booking_needs_locker`, `bookings.own_gear`), po zavření šatny (nebo po kódu motorky u vlastní výbavy) se na displeji podepisuje předávací protokol (kresba prstem + kód motorky jako identita) a kóje motorky se bez podepsaného protokolu neotevře (`protocol_required`, pole `protocol` v `kiosk_resolve_code`/`protocols[]` v `kiosk_sync_config`); podpis jde trvalou frontou do edge `submit-handover-protocol` (mode `kiosk`), podpis odjinud kiosk dostane příkazem `protocol_signed` — CONTRACT §28. Viz `raspberry/motogo-box/README.md`, `SPEC.md`, `CONTRACT.md`. Původní tabletový kiosk (`motogo-locker-kiosk`) byl 2026-09-10 z repa odstraněn.
...
- Evidence aplikovaných souborů drží DB tabulka `public._git_migrations` (filename PK). Workflow projde `supabase/migrations/*.sql` v bytovém pořadí názvu (`LC_ALL=C sort`: `20260910_…` < `20260910b_…` < `20260910c_…`) a aplikuje JEN soubory, které v evidenci nejsou — každý v jedné transakci (`ON_ERROR_STOP`), při chybě se nic zpola neaplikuje a workflow spadne.
...
**Ostatní workflows:** `snapshot-supabase.yml` (DENNĚ 03:15 UTC + ručně: snapshot živého stavu do větve `supabase-live-snapshot` pro diff s main; seznam edge funkcí bere živě z projektu), `daily-backup.yml` (DENNĚ kompletní šifrovaná záloha DB + Storage + git jako artifact, 30 dní; crony 03:30 + 13:30 UTC, guard = „existuje už dnes artifact `motogo24-backup-<datum>`?“ — NIKDY ne guard na hodinu, GitHub spouští cron se zpožděním 1–5 h; každá záloha se verifikuje (počet tabulek, COPY bloků, souborů Storage, git bundle); **Storage stahuje `backup/storage-backup.mjs` PARALELNĚ** (pool, `STORAGE_BACKUP_CONCURRENCY`, výchozí 16, retry na 429/5xx i na spojení přerušené při čtení těla souboru + na konci bucketu dohánění neuložených — běh 215 z 24. 9. padl na 11 souborech „terminated“) — sekvenční verze narazila 2026-09-20 do timeoutu (`media` má ~40 tis. objektů, ~0,9 s/soubor = 9,5 h); job má timeout 240 min a krok zakládající issue reaguje i na `cancelled()`, protože **překročení timeoutu NENÍ failure** a výpadek zálohy tím dřív zůstal tichý). Obě workflows při selhání založí issue s labelem **`backup-failed`** a po zeleném běhu ho zavřou.
