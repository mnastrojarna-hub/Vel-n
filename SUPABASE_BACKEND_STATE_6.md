...
| 2026-03-22 | **NEW: Servisní díly + automatické objednávání:** 1) Nová tabulka `service_parts` (schedule_id, inventory_item_id, quantity, notes, UNIQUE constraint). 2) Nová RPC `auto_check_service_parts()` — denní kontrola dílů pro blížící se servisy (~30 sezónních dní), auto-vytvoření PO seskupených dle dodavatele, dedup 7 dní. 3) Nová Edge Function `auto-check-service-parts` — volá RPC + rozesílá emaily dodavatelům přes send-order-email. 4) Nový cron job denně 06:00 UTC. 5) Velín: ServiceTab rozšířen o správu dílů per servisní plán (přidat/odebrat/upravit množství ze skladu) |
