...
| `send-invoice-email` | OFF | Odesílání faktur emailem zákazníkům. **FIX 2026-06-27:** stejně jako `send-email` nově (a) zapisuje `sent_emails.attachments_meta` (PDF z `invoice.pdf_path`) → přílohy vidět ve výpisu Zaslané maily; (b) DEDUP přes `message_log` (10 min okno, číslo dokladu v `content_preview`) → faktura jen jednou. |
