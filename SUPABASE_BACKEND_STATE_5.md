...
| `send-abandoned-booking-emails` | každé 2 min (`*/2 * * * *`) | `SELECT send_abandoned_booking_emails()` — pošle „nedokončená rezervace" mail web bookingu po 20 min od kroku 1, resp. 10 min od kliknutí „Pokračovat k platbě" (Stripe session). Dedup přes `bookings.abandoned_email_sent_at`. |
