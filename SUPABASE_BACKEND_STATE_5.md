...
| `translate-document` | ON | **NEW 2026-05-12** — Překlad celých dokumentů (Velín → Dokumenty) do 6 jazyků (en/de/es/fr/nl/pl) přes Anthropic API (`claude-haiku-4-5-20251001`). Umí: a) `custom_documents` typu `pdf` — PDF se pošle Claude přímo jako document content block, výstupem je přeložený obsah jako HTML (cizojazyčná verze webu se zobrazí jako HTML stránka, český originál PDF zůstává); b) `custom_documents` typu `html` (RTE obsah); c) `document_templates` (VOP/smlouva/GDPR/protokoly) — `{placeholder}` proměnné zůstávají nepřeložené. Výsledek UPDATEuje do JSONB `translations` cílové tabulky přes service_role (merge per jazyk). Vyžaduje JWT (admin z Velínu) nebo service_role klíč. Sekret: `ANTHROPIC_API_KEY`. Limit PDF 20 MB. Vstup: `{table:'custom_documents'|'document_templates', id, target_langs?}`. UI: Velín → Dokumenty → Smluvní texty → tlačítko „🌍 Přeložit" u každého dokumentu. |
