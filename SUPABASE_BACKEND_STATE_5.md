...
| `ai-public-agent` | OFF | **AI booking widget backend** (anonymní, bez JWT). Anthropic Claude Haiku 4.5 + 6 tools (search_motorcycles, get_availability, calculate_price, get_faq, **create_booking_request**, redirect_to_booking). Anti-halucinace: model nikdy nevymýšlí ceny ani datum — system prompt obsahuje hlavičku "DNES JE …" v Europe/Prague. **Konfigurovatelný z Velínu** přes `app_settings.ai_public_agent_config` (persona_name, system_prompt, situations, mustDo, forbidden, tone, max_tokens, enabled, welcome_cs/en/de). `create_booking_request` ověří dostupnost a zavolá RPC `create_web_booking` přes service_role — vrací `payment_url` na dokončení (doklady + platba kartou). `redirect_to_booking` zůstává jako alternativa pro zákazníky, kteří chtějí dokončit sami. Multi-language system prompt (cs/en/de + fallback). Rate-limit 20 req/min/IP. Loguje do `ai_traffic_log` (source='widget', outcome=`view`/`quote`/`booking_created`). |
