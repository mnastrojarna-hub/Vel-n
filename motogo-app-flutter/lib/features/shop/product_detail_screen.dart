                ...
                sliver: SliverList.list(children: [
                    ...
                    Container(
                      ...
                      child: Column(children: [
                        ...
                        if (product.material != null)
                          _InfoRow(label: t(context).tr('material'), value: product.material!),
                      ]),
                    ),
                  const SizedBox(height: 12),
                  ...
                  if (!product.inStock)
                    Container(
                      ...
                      child: Text(t(context).tr('soldOut'), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: MotoGoColors.red), textAlign: TextAlign.center),
                    ),

                  // Add to cart button — inline in content (matches original sticky style)
                  if (product.inStock) ...[
                    const SizedBox(height: 8),
                    ElevatedButton(
                      onPressed: () {
                        if (product.needsSize && _selectedSize == null) {
                          showMotoGoToast(context, icon: '⚠️', title: t(context).tr('error'), message: t(context).tr('error'));
                          return;
                        }
                        ...
                        showMotoGoToast(context, icon: '✓', title: t(context).tr('added'), message: cartName);
                      },
                      style: ElevatedButton.styleFrom(minimumSize: const Size.fromHeight(52)),
                      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        const Icon(Icons.add_shopping_cart, size: 18),
                        const SizedBox(width: 8),
                        Text('${t(context).tr('addToCart')} · ${product.price.toStringAsFixed(0)} Kč'),
                      ]),
                    ),
                  ],
