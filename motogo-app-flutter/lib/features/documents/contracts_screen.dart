...
class ContractsScreen extends ConsumerWidget {
  ...
  /// Load legal text from document_templates table in Supabase.
  ...
  Future<void> _showLegalFromSupabase(BuildContext ctx, String type, String fallbackTitle) async {
    ...
    try {
      ...
      if (res != null) {
        ...
        body = (res['content_html'] as String? ?? '')
            .replaceAll(RegExp(r'<[^>]*>'), ' ')
    ...
    if (body.isEmpty) {
      body = type == 'vop' ? LegalTexts.vopSummary : LegalTexts.gdprSummary;
    }
                ...
                TextButton.icon(
                  onPressed: () {
                    ...
                    ScaffoldMessenger.of(c).showSnackBar(
                      SnackBar(content: Text(t(ctx).tr('copiedToClipboard')), backgroundColor: MotoGoColors.dark),
                    );
                  },
                  icon: const Icon(Icons.copy, size: 16),
                  label: Text(t(ctx).tr('copy')),
                ),
                const SizedBox(width: 8),
                ElevatedButton.icon(onPressed: () => Navigator.pop(c), icon: const Icon(Icons.close, size: 16), label: Text(t(ctx).tr('close'))),
              ],
            ),
          ]),
        ),
