...
class _EditState extends ConsumerState<ReservationEditScreen> {
  ...
  String _getCalendarInstruction() {
    if (_tab == 'extend') {
      if (_isActive) {
        return 'Klikněte na den PO ${_booking!.endDate.day}.${_booking!.endDate.month}. pro prodloužení';
      }
      return 'Klikněte na den PŘED ${_booking!.startDate.day}.${_booking!.startDate.month}. nebo PO ${_booking!.endDate.day}.${_booking!.endDate.month}. pro prodloužení';
    } else {
      if (_isActive) {
        return 'Klikněte na nový den vrácení před ${_booking!.endDate.day}.${_booking!.endDate.month}.';
      }
      if (_shortenDir == null) {
        return 'Vyberte směr zkrácení a poté klikněte na datum';
      }
      if (_shortenDir == 'start') {
        return 'Klikněte na nový den začátku rezervace';
      }
      return 'Klikněte na nový den konce rezervace';
    }
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (calc.priceDiff > 0) {
        // Needs extra payment — update total_price, redirect to payment
        // Do NOT change payment_status — Stripe webhook sets it to 'paid'
        ...
        if (mounted) {
          ...
          // Set payment context so PaymentScreen knows amount + booking ID
        ...
        if (mounted) {
          ...
          _showConfirmation(
            ...
            message: calc.priceDiff < 0
                ? 'Rezervace byla zkrácena.\nVrácení: ${(-calc.priceDiff).toStringAsFixed(0)} Kč (${StornoCalc.refundPercent(_newEnd ?? _booking!.endDate)}%)\nČástka bude vrácena na původní platební metodu.'
                : 'Vaše změny byly úspěšně uloženy.\nRezervace: ${_fmt(_newStart)} – ${_fmt(_newEnd)}',
            ...
            EditCard(child: Column(children: [
              ...
              if (calc.priceDiff < 0 && calc.diffDays < 0)
                Padding(padding: const EdgeInsets.only(top: 4),
                  child: Text('Storno: vrácení ${StornoCalc.refundPercent(_newEnd ?? _booking!.endDate)}%',
