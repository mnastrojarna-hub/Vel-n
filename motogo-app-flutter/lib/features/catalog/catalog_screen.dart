                    ...
                    child: Container(
                      ...
                      child: DropdownButtonHideUnderline(
                        child: DropdownButton<String?>(
                          ...
                          isExpanded: true,
                          hint: Text(t(context).tr('catalogLicenseGroupHint'), style: const TextStyle(fontSize: 12)),
                          style: const TextStyle(fontSize: 12, color: MotoGoColors.black),
                          items: [
                            DropdownMenuItem(value: null, child: Text(t(context).tr('homeFilterLicenseAll'))),
                            const DropdownMenuItem(value: 'A', child: Text('A')),
                            const DropdownMenuItem(value: 'A2', child: Text('A2')),
                            const DropdownMenuItem(value: 'A1', child: Text('A1')),
                            const DropdownMenuItem(value: 'AM', child: Text('AM')),
                            const DropdownMenuItem(value: 'B', child: Text('B')),
                            DropdownMenuItem(value: 'N', child: Text(t(context).tr('catalogNoLicense'))),
                          ],
                          onChanged: (v) {
                            ref.read(catalogFilterProvider.notifier).state =
          ...
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: motosAsync.when(
                data: (motos) => Text(
                  t(context).tr('catalogMotorcycleCount').replaceAll('{n}', '${motos.length}'),
                  style: const TextStyle(
                    fontSize: 12, fontWeight: FontWeight.w700,
                    color: MotoGoColors.g400,
                  ),
                ),
                loading: () => Text(t(context).tr('catalogLoading'), style: const TextStyle(fontSize: 12, color: MotoGoColors.g400)),
                error: (_, __) => Text(t(context).tr('catalogLoadingError'), style: const TextStyle(fontSize: 12, color: MotoGoColors.red)),
              ),
            ),
          ),

          // Motorcycle list (full-width cards)
          motosAsync.when(
            ...
            error: (e, _) => SliverFillRemaining(
              child: Center(
                child: Text('${t(context).tr('catalogErrorPrefix')}$e', style: const TextStyle(color: MotoGoColors.red)),
              ),
            ),
          ),
        ],
