...
class _PromoCodeInputState extends State<PromoCodeInput> {
  ...
  Future<void> _apply() async {
    ...
    if (widget.appliedCodes.any((d) => d.code == code.toUpperCase())) {
      setState(() => _error = 'Kód "$code" je již použit');
      return;
    }

    // Check: can't combine two percentage codes
    if (widget.appliedCodes.any((d) => d.type == DiscountType.percent)) {
      // Allow only fixed codes after a percentage one
    }
    ...
    if (result.success && result.discount != null) {
      ...
      _ctrl.clear();
      widget.onCodeApplied(result.discount!);
    } else {
      setState(() {
