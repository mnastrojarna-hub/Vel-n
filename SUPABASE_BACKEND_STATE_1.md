...
| `poi_ratings` | **NEW 2026-06-29 (`20260629_community_routes_pois.sql`)** — Hvězdičkové hodnocení bodů zájmu (1–5). Sloupce: `id`, `user_id` FK→auth.users CASCADE, `route_poi_id` FK→route_pois CASCADE **NEBO** `user_poi_id` FK→user_pois CASCADE (CHECK právě jeden), `rating` int 1–5, `created_at`, `updated_at`. UNIQUE(`user_id`,`route_poi_id`) + UNIQUE(`user_id`,`user_poi_id`) = **max 1 hodnocení na uživatele a bod** (lze měnit). RLS: public read, vlastník write. Zápis přes RPC `rate_poi`, čtení přes `poi_rating_summary` / agregace v `get_branch_routes` a `get_user_pois`. |
