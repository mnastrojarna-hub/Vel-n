...
| `kiosk_commands` | Fronta vzdálených příkazů Velín→kiosk. `device_id` FK→kiosk_devices CASCADE, `command` (open_door/music_on/music_off/identify/reload/camera_control/http_get/restart — **UPDATE 2026-09-09 nové pro RPi:** light_on/light_off/set_signal/zone_test/audio_test/all_off/reboot/sync_config/update_software), `params` jsonb, `status` (pending/done/failed/expired), `result` jsonb, `created_by`, `executed_at`. Realtime přes **DB Broadcast** (trigger → `realtime.send` na topic `kiosk:<device_id>`), NE přes publication (kiosk je anon). |
