...
| `email_send_locks` | **NEW 2026-08-12 (PŘEDLOŽENO JAKO TEXT — nutno aplikovat).** Atomické zámky odesílání zákaznických mailů — `(booking_id, template_slug)` PK + `locked_at`. Řeší read-then-write race 30min idempotency kontroly v `send-cancellation-email` (incident #DDC5A69D: 4 souběžné invokace = 4 duplicitní storno maily). Zamyká RPC `claim_email_send` (viz STATE_3); edge fn má fallback na starou kontrolu, dokud SQL není aplikována. |
...
| `fio_transactions` | **NEW 2026-08-29 (`20260829_fio_bank_sync.sql`, APLIKUJE AUTO-DEPLOY po merge)** — Příchozí pohyby z Fio API (nový firemní účet u Fio) pro AUTOMATICKÉ potvrzování QR / bankovních plateb. PK `fio_id` bigint = Fio „ID pohybu" (idempotence — každý pohyb se zpracuje max 1×). Sloupce: `tx_date`, `amount` numeric(12,2), `currency`, `vs`, `counter_account/bank/name`, `message`, `tx_type`, `status` CHECK (new/outgoing/confirmed_booking/confirmed_surcharge/already_paid/no_match/amount_mismatch/needs_review/error), `booking_id` FK→bookings SET NULL, `note`, `created_at`, `processed_at`. Plní edge fn `fio-sync` (cron `fio-bank-sync` à 5 min); párování přes VS na `bookings.payment_vs` / `bookings.mod_surcharge_vs` / fallback `invoices.variable_symbol` (ZF). RLS: admin ALL (`is_admin()`); zápis jen service_role (edge). Indexy: `idx_fio_transactions_vs`, partial `idx_fio_transactions_pending`. |
| `fio_payment_orders` | **NEW 2026-08-30 (`20260830_fio_payment_orders.sql`, APLIKUJE AUTO-DEPLOY po merge)** — Odchozí platební příkazy podané přes Fio API = AUTOMATICKÉ VRACENÍ plateb (parita se Stripe). Sloupce: `id` uuid PK, `credit_note_id` uuid **UNIQUE** FK→invoices SET NULL (idempotence — jeden dobropis = max JEDEN příkaz, souběžné dispatche process-refund dostanou konflikt), `booking_id` FK→bookings SET NULL, `amount`, `account_to`/`bank_code` (protiúčet z `fio_transactions`), `vs`, `message`, `status` CHECK (pending/submitted/failed), `fio_instruction_id` (číslo dávky z Fio importu), `error`, `created_at`, `submitted_at`. Plní edge fn `process-refund` (`tryFioRefund` v manuální větvi). RLS: admin ALL (`is_admin()`); zápis jen service_role (edge). Index `idx_fio_payment_orders_booking`. |
...
> **UPDATE 2026-09-10 (D) (hromadná aktualizace řídicích jednotek + OS — `20260910c_kiosk_fleet_updates.sql`, APLIKUJE AUTO-DEPLOY po merge):** Velín (Pobočky → „Aktualizace řídicích jednotek (všechny pobočky)“) rozešle aktualizaci software (`update_software`) nebo OS (`update_system` = apt full-upgrade) všem RPi jednotkám postupně **kanárek → soak → zbytek**, vědomě tlačítkem nebo noční automatikou (výchozí 03:00 Prahy); box ji provede, až je kóje volná. Nové tabulky `kiosk_releases`, `kiosk_fleet_settings`, `kiosk_rollouts`, `kiosk_rollout_devices` (níže), `kiosk_commands` CHECK + `update_system`, RPC `kiosk_version_matches` + `kiosk_rollout_start/cancel/tick` (STATE_3), pg_cron `kiosk-fleet-update-tick` */5 min + GitHub Action `release-motogo-box.yml` plnící `kiosk_releases` (STATE_5).
> **UPDATE 2026-09-10 (diagnostika sítě RPi — `20260910_kiosk_diagnostics.sql`, APLIKUJE AUTO-DEPLOY po merge):** kód na displeji / servisní heslo s účelem `diagnostics` / příkaz `diagnostics` z Velína spustí na RPi kompletní diagnostiku sítě (rozhraní, LTE, internet, Velín, moduly, scan LAN + identifikace Waveshare/Shelly, ARP); report se zobrazí na displeji a uloží do nové tabulky `kiosk_diagnostics` (RPC `kiosk_report_diagnostics`). Nový sloupec `branch_service_codes.action`, `kiosk_commands` CHECK + `diagnostics`, `kiosk_sync_config.service_codes` = `{h, action, label}`, `kiosk_resolve_code` vrací `action`.
...
| Tabulka | Popis |
|---------|-------|
| `branch_kiosk_config` | 1:1 konfigurace kiosku per pobočka (PK `branch_id`). Hudba (`music_on_url`/`music_off_url`), časování (`door_open_seconds`/`light_seconds`/`music_seconds`), stav FV (`power_status_url`/`power_poll_seconds`), `relay_base_url`, `is_active`. Token NENÍ zde (je per zařízení). **UPDATE 2026-09-09:** `hardware` jsonb NOT NULL DEFAULT '{}' — HW mapa pobočky pro RPi (devices/timings/polling/contacts/security/audio/signal; přepisuje lokální `config/brno-9zone.yaml`, klíč `zones` sem nepatří). |
| `kiosk_devices` | Zařízení (tablety) — **unikátní identita per app**. `id` uuid PK = identifikátor zařízení, `branch_id` FK→branches CASCADE, `name`, `device_token` uuid (tajný párovací token), `platform`, `app_version`, `last_seen_at` (heartbeat → online stav), `is_active` (revokace párování). **UPDATE 2026-09-09:** `status` jsonb NOT NULL DEFAULT '{}' + `status_at` timestamptz — poslední snapshot stavu RPi řídicí jednotky (zóny, moduly, LTE, audio; plní `kiosk_report_status`); u tabletů `{}`/NULL. **2026-09-10 (D):** ve `status` navíc klíč `update` (stav aktualizace software/OS/restartu z Velína, `last` = poslední dokončený běh) a `health.sys.reboot_required/os/kernel/last_unattended_at` (STATE_2). |
| `branch_doors` | Mapování logických dveří → Shelly relé/světlo (LAN). `door_kind` (motorcycle/accessories), `box_number` (= `motorcycles.box_number`; NULL u oblečení), `label`, `relay_url`, `light_url`, `is_active`, `sort_order`. UNIQUE per (branch, box) u motorcycle, max 1 accessories per pobočka. **UPDATE 2026-09-09:** `hw` jsonb NOT NULL DEFAULT '{}' — HW mapa zóny pro RPi (`{zone, lock{dev,coil}, contact{dev,input}, light{dev,coil}, audio{dev,coil}, red{dev,light}, green{dev,light}, closed_level?}`); `{}` = tabletový režim přes relay/light URL. |
| `branch_service_codes` | Servisní hesla (otevírají VŠE; app se zeptá které dveře). `branch_id`, `code`, `label`, `is_active`, `created_by`. UNIQUE(branch_id, code) WHERE is_active. **UPDATE 2026-09-10:** `action` text NOT NULL DEFAULT 'service' CHECK (service/diagnostics) — účel hesla: `service` = servisní panel (vše), `diagnostics` = na RPi řídicí jednotce JEN spustí diagnostiku sítě (nic neotevírá; tablet pole ignoruje). |
| `kiosk_commands` | Fronta vzdálených příkazů Velín→kiosk. `device_id` FK→kiosk_devices CASCADE, `command` (open_door/music_on/music_off/identify/reload/camera_control/http_get/restart — **UPDATE 2026-09-09 nové pro RPi:** light_on/light_off/set_signal/zone_test/audio_test/all_off/reboot/sync_config/update_software; **2026-09-10:** diagnostics; **2026-09-10 (D):** update_system), `params` jsonb, `status` (pending/done/failed/expired), `result` jsonb, `created_by`, `executed_at`. Realtime přes **DB Broadcast** (trigger → `realtime.send` na topic `kiosk:<device_id>`), NE přes publication (kiosk je anon). |
| `kiosk_logs` | **NEW v docs 2026-09-09 (z `20260630_kiosk_diag_ota_offline.sql`)** — Vzdálená diagnostika: logy/chyby/pády z kiosku i RPi. `device_id` FK→kiosk_devices CASCADE, `branch_id` FK→branches SET NULL, `level` (CHECK info/warn/error/crash), `source` (relay/camera/power/rpc/flutter/platform/modbus/shelly/zone/pin/lte/config…), `message`, `detail` jsonb, `app_version`, `created_at`. Plní RPC `kiosk_log_event`; RLS admin SELECT only. |
| `kiosk_diagnostics` | **NEW 2026-09-10 (`20260910_kiosk_diagnostics.sql`, APLIKUJE AUTO-DEPLOY po merge)** — Reporty diagnostiky sítě z RPi řídicích jednotek. `device_id` FK→kiosk_devices CASCADE, `branch_id` FK→branches SET NULL, `report_id` (id běhu z RPi), `source` (local_code/service_code/service_panel/velin), `ok` bool, `problems` jsonb (texty problémů), `summary` jsonb ({ok, problems[], hosts, internet, lte, devices_ok, devices_total}), `report` jsonb (celý report: system/interfaces/lte/internet/supabase/devices/lan/arp/steps), `app_version`, `started_at`, `finished_at`, `created_at`. Indexy (branch_id, created_at DESC), (device_id, created_at DESC). Plní RPC `kiosk_report_diagnostics` (drží posledních 30 na zařízení); RLS admin FOR ALL. Velín: blok „Diagnostika sítě (Raspberry)“ (`BranchRpiDiagnostics.jsx`). |
| `kiosk_releases` | **NEW 2026-09-10 (D) (`20260910c_kiosk_fleet_updates.sql`, APLIKUJE AUTO-DEPLOY po merge)** — Releasy RPi programu = commity v `main`, které změnily `raspberry/motogo-box/**`. `commit` text UNIQUE (plný sha 40 hex), `version` (`__version__`), `message`, `author`, `committed_at`, `files_changed`, `created_at`; index (created_at DESC). Plní GitHub Action `release-motogo-box.yml` (`INSERT … ON CONFLICT (commit) DO NOTHING`). „Nejnovější“ pro Velín i noční automatiku = `committed_at DESC NULLS LAST, created_at DESC`. RLS admin FOR ALL. |
| `kiosk_fleet_settings` | **NEW 2026-09-10 (D)** — Singleton (`id boolean PK DEFAULT true CHECK (id)`, řádek vložen migrací): noční automatika (`nightly_enabled`, `nightly_hour`), výchozí kanárek (`canary_device_id` FK→kiosk_devices SET NULL), `soak_minutes`, `wait_idle_s`, OS aktualizace (`system_enabled`, `system_every_days`, `system_auto_reboot`), `last_nightly_date`, `last_system_date`, `updated_at`. Velín upsert (blok Aktualizace → Nastavení automatiky); čte `kiosk_rollout_tick`. RLS admin FOR ALL. |
| `kiosk_rollouts` | **NEW 2026-09-10 (D)** — Jedna hromadná aktualizace: `kind` software/system, `mode` manual/nightly, `status` canary → soak → rollout → done/failed/cancelled, `release_id` FK→kiosk_releases SET NULL, `target_commit`, `canary_device_id` FK→kiosk_devices SET NULL, `soak_minutes`, `wait_idle_s`, `auto_reboot`, `created_by`, časy fází (`canary_started_at`, `canary_done_at`, `soak_until`, `rollout_started_at`, `finished_at`), `error`, `result` jsonb (`{updated, failed, offline, skipped, commanded, pending, total, errors[], offline_ids[], expired_commands}`), `note`. Částečný index (status) WHERE aktivní; vždy max. jeden aktivní (advisory lock v RPC). RLS admin FOR ALL. |
| `kiosk_rollout_devices` | **NEW 2026-09-10 (D)** — Stav každé jednotky v rolloutu: PK (`rollout_id` FK→kiosk_rollouts CASCADE, `device_id` FK→kiosk_devices CASCADE), `role` canary/fleet, `status` pending/commanded/updated/failed/offline/skipped, `command_id` (příkaz v `kiosk_commands`), `commanded_at`, `version_before`, `version_after`, `detail` jsonb (`{error}` / `{reason:'never_seen'}` / `{error:'unreachable'}`), `updated_at`. RLS admin FOR ALL. |
