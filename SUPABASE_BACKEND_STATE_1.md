...
| `document_templates` | Šablony dokumentů (id uuid, type TEXT, name TEXT, content_html TEXT, active BOOL, version INT, updated_by uuid, created_at, updated_at, **translations JSONB** = `{ "<lang>": { "name", "content_html" } }` pro en/de/es/fr/nl/pl — plní edge fn `translate-document`, mig. `20260512_document_translations.sql`) |
| `custom_documents` | Vlastní dokumenty z Velína mimo 5 pevných smluvních typů (id uuid, title, slug UNIQUE, description, kind TEXT 'html'\|'pdf', content_html, pdf_path [veřejná URL v bucketu `media`], show_on_web BOOL, sort_order INT, active BOOL, version INT, updated_by uuid→admin_users, created_at, updated_at, **translations JSONB** = `{ "<lang>": { "title", "description", "content_html" } }` — plní `translate-document`, mig. `20260512_document_translations.sql`). Web: /jak-pujcit/dokumenty (výpis vedle pevných karet) + /dokumenty/<slug> (HTML render nebo redirect na PDF; u přeloženého PDF se na cizojazyčné verzi zobrazí přeložené HTML). Migrace `20260512_custom_documents.sql` |
