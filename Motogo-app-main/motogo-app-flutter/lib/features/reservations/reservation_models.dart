...
class Reservation {
  ...
  final String? cancelledBySource; // 'admin', 'customer', 'system'
  final String? replacementForBookingId;
  ...
  const Reservation({
    ...
    this.cancelledBySource,
    this.replacementForBookingId,
  ...
  factory Reservation.fromJson(Map<String, dynamic> json) {
    ...
    return Reservation(
      ...
      originalEndDate: json['original_end_date'] != null
          ? DateTime.tryParse(json['original_end_date'] as String)
          : null,
      modificationHistory: _parseModHistory(json['modification_history']),
      cancelledBySource: json['cancelled_by_source'] as String?,
      replacementForBookingId: json['replacement_for_booking_id'] as String?,
  ...
  static List<ModificationEntry> _parseModHistory(dynamic raw) {
    ...
    return raw
        ...
        .map((e) => ModificationEntry.fromJson(e))
        .toList();
  }
...
/// Extended beyond Capacitor original to also track location/method changes.
class ModificationEntry {
  final DateTime at;
  final String fromStart;
  final String fromEnd;
  final String toStart;
  final String toEnd;
  final String? fromMoto;
  final String? toMoto;
  final String? fromPickupMethod;
  final String? toPickupMethod;
  final String? fromPickupAddress;
  final String? toPickupAddress;
  final String? fromReturnMethod;
  ...
  const ModificationEntry({
    ...
    this.fromMoto,
    this.toMoto,
    this.fromPickupMethod,
    this.toPickupMethod,
    this.fromPickupAddress,
    this.toPickupAddress,
    this.fromReturnMethod,
    this.toReturnMethod,
  ...
  factory ModificationEntry.fromJson(Map<String, dynamic> json) {
    return ModificationEntry(
      ...
      fromStart: json['from_start'] as String? ?? '',
      fromEnd: json['from_end'] as String? ?? '',
      toStart: json['to_start'] as String? ?? '',
      toEnd: json['to_end'] as String? ?? '',
      fromMoto: json['from_moto'] as String?,
      toMoto: json['to_moto'] as String?,
      fromPickupMethod: json['from_pickup_method'] as String?,
      toPickupMethod: json['to_pickup_method'] as String?,
      fromPickupAddress: json['from_pickup_address'] as String?,
      toPickupAddress: json['to_pickup_address'] as String?,
      fromReturnMethod: json['from_return_method'] as String?,
  ...
  Map<String, dynamic> toJson() => {
    ...
    'to_start': toStart,
    'to_end': toEnd,
    if (fromMoto != null) 'from_moto': fromMoto,
    if (toMoto != null) 'to_moto': toMoto,
    if (fromPickupMethod != null) 'from_pickup_method': fromPickupMethod,
    if (toPickupMethod != null) 'to_pickup_method': toPickupMethod,
    if (fromPickupAddress != null) 'from_pickup_address': fromPickupAddress,
    if (toPickupAddress != null) 'to_pickup_address': toPickupAddress,
    if (fromReturnMethod != null) 'from_return_method': fromReturnMethod,
  ...
  bool get hasReturnAddressChange =>
      fromReturnAddress != null && toReturnAddress != null &&
      fromReturnAddress != toReturnAddress;
}

/// Describes a modification: type, color, detail text.
/// Mirrors _descMod() from reservations-edit.js.
class ModDescription {
  ...
  const ModDescription({
    ...
    required this.origDays,
    required this.newDays,
    required this.color,
  });

  static ModDescription describe(
    String fromStart, String fromEnd, String toStart, String toEnd,
  ) {
    DateTime _nd(String d) {
      final dt = DateTime.tryParse(d) ?? DateTime.now();
      return DateTime(dt.year, dt.month, dt.day);
    }

    final fs = _nd(fromStart);
    final fe = _nd(fromEnd);
    final ts = _nd(toStart);
    final te = _nd(toEnd);
    ...
    final parts = <String>[];
    if (sd < 0) parts.add('začátek dříve o ${sd.abs()} d');
    else if (sd > 0) parts.add('začátek později o $sd d');
    if (ed > 0) parts.add('konec později o $ed d');
    else if (ed < 0) parts.add('konec dříve o ${ed.abs()} d');

    String type;
    Color color;
    if (dd > 0) {
      type = 'prodlouženo o $dd d';
      color = const Color(0xFF2563EB);
    } else if (dd < 0) {
      type = 'zkráceno o ${dd.abs()} d';
      ...
      type = 'beze změny';
      color = const Color(0xFF4A6357);
    }

    final detail = parts.isNotEmpty ? parts.join(', ') : type;
    return ModDescription(
      type: type,
      detail: detail,
      origDays: origD < 1 ? 1 : origD,
      newDays: newD < 1 ? 1 : newD,
      color: color,
    );
  }
}
