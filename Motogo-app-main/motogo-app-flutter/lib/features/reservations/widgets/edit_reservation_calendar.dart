...
class _EditReservationCalendarState extends State<EditReservationCalendar> {
  ...
  void _onDayTap(DateTime date) {
    ...
    if (_isOccupiedOrPending(d) && !_isInOrigRange(d)) {
      widget.onError(t(context).tr('dayOccupied'));
      return;
    }
  ...
  void _handleExtendTap(DateTime d, DateTime origS, DateTime origE) {
    ...
    if (!widget.isActive && d.isBefore(origS)) {
      ...
      if (!_isRangeFree(d, origS.subtract(const Duration(days: 1)))) {
        widget.onError(t(context).tr('occupiedDaysBetween'));
        return;
      }
    ...
    if (!d.isBefore(origS) && !d.isAfter(origE)) {
      if (widget.isActive) {
        widget.onError(
            t(context).tr('clickDayAfterForExtend').replaceAll('{date}', '${origE.day}.${origE.month}.'));
      } else {
        widget.onError(
            t(context).tr('clickDayBeforeOrAfterForExtend').replaceAll('{startDate}', '${origS.day}.${origS.month}.').replaceAll('{endDate}', '${origE.day}.${origE.month}.'));
      }
      return;
    }
  ...
  void _handleShortenTap(DateTime d, DateTime origS, DateTime origE) {
    ...
    if (!inRes) {
      widget.onError(t(context).tr('clickInsideForShorten'));
      return;
    }
    ...
    if (widget.isActive) {
      if (!d.isBefore(origE)) {
        widget.onError(
            t(context).tr('clickDayBeforeForShorten').replaceAll('{date}', '${origE.day}.${origE.month}.'));
        return;
      }
      if (d.isBefore(today)) {
        widget.onError(t(context).tr('cannotShortenToPast'));
        return;
      }
      ...
      final dir = widget.shortenDir;
      if (dir == null) {
        widget.onError(t(context).tr('selectShortenFirst'));
        return;
      }
      if (dir == 'start') {
