  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      Container(
        ...
        child: Row(children: [
          const Icon(Icons.shield, size: 20, color: MotoGoColors.greenDarker),
          const SizedBox(width: 10),
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(t(context).tr('freeGear'), style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: MotoGoColors.greenDarker)),
            Text(t(context).tr('freeGearList'), style: const TextStyle(fontSize: 11, color: MotoGoColors.g600)),
          ])),
          const Icon(Icons.check_circle, size: 18, color: MotoGoColors.green),
        ])),
      ...
      if (widget.isDelivery) ...[
        ...
        _GearSizePicker(label: t(context).tr('helmet'), icon: Icons.sports_motorsports,
          selectedSize: widget.helmetSize,
          onSizeSelected: (s) => widget.onHelmetSizeChanged?.call(s)),
        const SizedBox(height: 6),
        _GearSizePicker(label: t(context).tr('jacket'), icon: Icons.checkroom,
          selectedSize: widget.jacketSize,
          onSizeSelected: (s) => widget.onJacketSizeChanged?.call(s)),
        const SizedBox(height: 6),
        _GearSizePicker(label: t(context).tr('pants'), icon: Icons.straighten,
          selectedSize: widget.pantsSize,
          onSizeSelected: (s) => widget.onPantsSizeChanged?.call(s)),
        const SizedBox(height: 6),
        _GearSizePicker(label: t(context).tr('boots'), icon: Icons.do_not_step,
          sizes: bootSizes,
          selectedSize: widget.bootsSize,
          onSizeSelected: (s) => widget.onBootsSizeChanged?.call(s)),
        const SizedBox(height: 6),
        _GearSizePicker(label: t(context).tr('gloves'), icon: Icons.back_hand_outlined,
        ...
        if (_isSelected('extra-spolujezdec')) ...[
          const SizedBox(height: 10),
          Text(t(context).tr('passenger'), style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: MotoGoColors.greenDarker)),
          const SizedBox(height: 6),
          _GearSizePicker(label: t(context).tr('helmet'), icon: Icons.sports_motorsports,
