...
enum ApplePayFailureAction {
  ...
  /// Stav nejistý (Processing, nebo výpadek sítě po Face ID) → počkat na
  /// serverové potvrzení, NIKDY nenabízet další platbu naslepo.
  processing,
...
/// (ApplePayViewController.swift), a `Failed` s prázdnou chybou, když PI
/// skončil v processing — „nic nebylo strženo" se tedy smí říct až po ověření.
Future<ApplePayFailureOutcome> resolveApplePayFailure(
  ...
  PaymentIntent? pi;
  try {
    pi = await Stripe.instance.retrievePaymentIntent(clientSecret);
  } catch (_) {
    // Bez sítě nevíme, jak platba dopadla — viz větev Unknown níže.
  }
  final s = pi?.status;
  ...
  if (s == PaymentIntentsStatus.Processing ||
      (pi == null && err.code == FailureCode.Unknown)) {
  ...
  // neotevřel, iPhone nemá použitelnou kartu. Jinak sheet proběhl, ale platbu
  // nezpracoval Stripe/Apple — „zkontrolujte kartu" by zákazníka jen mátlo.
...
void logWalletError(String wallet, StripeException e, {bool? supported}) {
  final err = e.error;
  String? cut(String? s) =>
