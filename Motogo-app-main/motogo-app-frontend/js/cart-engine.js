...
function updateCartFab(){
  try {
    ...
    // Reset dismissed state when navigating away from cart/checkout flow
    // so FAB reappears on other screens (e.g. shop, home)
    var shopFlowScreens = ['s-cart','s-checkout','s-payment','s-success'];
    if(count > 0 && shopFlowScreens.indexOf(cur) === -1) {
      cartFabDismissed = false;
    }
    if(fab){
