...
| `create_web_booking(p_moto_id, p_start_date, p_end_date, p_name, p_email, p_phone, ..., p_password)` | Vytvoří web rezervaci: najde/vytvoří auth usera + profil, zkontroluje dostupnost, vypočte cenu (per-day pricing price_mon..price_sun, inclusive start+end), vytvoří booking (source='web'). **Po vytvoření bookingu vloží jednotlivé extras z p_extras do booking_extras (name, unit_price, quantity).** p_extras formát: `[{name, unit_price}]`. p_password: nastaví heslo pro auth.users (editace rezervace + budoucí přihlášení do app). Vrací {booking_id, amount, user_id}. SECURITY DEFINER |
