...
| internet | **kabel z routeru** (je-li na switchi — Velké Němčice: Teltonika 192.168.1.2; hybridní `motogo-lan` = DHCP + statické adresy, metrika 50) a LTE SIM7600E-H (USB) jako záloha | profil `motogo-lte` (metrika 100); PIN SIM u všech poboček **1234** (výchozí install.sh; jiný = `MOTOGO_SIM_PIN`) → profil `[gsm] pin=` |
