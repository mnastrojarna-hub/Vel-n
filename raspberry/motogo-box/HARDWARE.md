...
Adresu v I/O síti si Raspberry drží **samo** (NM dispatcher `50-motogo-lan-addr`: 192.168.50.10/24 + 192.168.1.253/24 na eth0, i když eth0 jede na DHCP kvůli internetu — `set-static-lan.sh` už není nutný). **Hybridní `motogo-lan` (2026-09-26):** `method=auto` + statické adresy, `dhcp-timeout=infinity`, `route-metric=50` — když je na kabelu router s DHCP, internet jde kabelem a LTE (metrika 100) je záloha (SIM7600 vypadává z USB, viz „Známá závada“); bez routeru brána přes eth0 nevznikne a jede LTE. Migraci starých profilů dělá `update.sh`; návrat na „jen LTE“: `nmcli con modify motogo-lan ipv4.never-default yes`. Health při `no_address` nejdřív spustí dispečer (adresy hned), pak `nmcli con up motogo-lan`. Diagnostika pobočky má sekci „Automatické zřízení modulů“ (adresy eth0, výsledek vyhledávání ZLAN, stav každého nalezeného modulu — jen čtení). Waveshare se **nenastavuje ručně** (automatické zřízení, `io_provision.py`): modul z HW mapy, který na
