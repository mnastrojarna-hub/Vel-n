...
obnovy stojí a v payloadu je `lte.error` (`sim_locked`/`sim_puk`/`sim_missing`/`sim_error`) — viz Řešení problémů.
Sonda na Supabase vrací **401** (bez `apikey`) — a je to tak správně: jakákoli HTTP odpověď dokazuje,
že se tam jednotka dosáhla, takže se počítá jako dostupné. O výpadku internetu rozhodují jen **google generate_204 a TCP 1.1.1.1**; `probe_url` (Supabase) se měří
dál, ale nerozhoduje — hlásila „nedostupné" i když internet fungoval, a výpadek Supabase není důvod
resetovat modem. V payloadu je nově `lte.usb_present` / `lte.modem_gone` (modem na USB vs. v MM) a
`lte.last24h` = kolikrát se za 24 h dělal reconnect / modem_reset / usb_reset / reboot (celková
počítadla rostou donekonečna — 1584 reconnectů nikomu nic neřeklo).
Zámek SIM se pozná z mmcli `unlock-required` NEZÁVISLE na `state` (zamčená SIM se po restartu modemu umí tvářit
jako `searching`); `lte.unlock_retries` říká, kolik pokusů o PIN zbývá. **Doporučené nastavení pobočky: PIN na SIM
vypnutý** — jinak LTE po každém restartu závisí na tom, že PIN v profilu `motogo-lte` pořád sedí.
...
- **Staré prahy LTE v `config.yaml` se při aktualizaci samy odstraní** (2026-09-20): jednotky instalované dřív mají
  v `health:` natvrdo `reconnect_after: 5` / `usb_reset_after: 5`, tedy STARÉ výchozí hodnoty, a nová rychlá eskalace by
  se u nich nikdy nespustila (software aktuální, chování staré). `update.sh` smaže JEN řádky přesně s těmito hodnotami
  (vlastní nastavení zůstává) a udělá zálohu `config.yaml.bak-<čas>`. Ostatní klíče (`missing_modem_after`,
  `action_cooldown_s`, `usb_reset_timeout_s`, `lte_mode`) se nedoplňují — chybějící berou výchozí hodnotu z kódu.
...
| výměna motorky / nový kód se musí projevit na pobočce „hned" | jednotka jede z offline cache | Online: změna kódů spustí `kiosk_request_sync` → realtime příkaz `sync_config` → jednotka má nový kód do několika sekund. Při výpadku: cache platí dál a **po návratu spojení se synchronizuje okamžitě** (`resync_on_reconnect` v heartbeat smyčce, ne až dalším `sync_loop` po 60 s). Nejhorší případ = délka výpadku + pár sekund. |
...
| modem je `connected`, ale health píše „obnova pozastavena" a nic neřeší | **PIN2/PUK2** na SIM. ModemManager hlásí `unlock-required: sim-pin2` i na plně funkčním modemu (PIN2 chrání jen FDN a servisní funkce, data neblokuje). Do 2026-09-20 to health bral jako `sim_locked` a **vypnul tím celou politiku obnovy** — ověřeno živě na pobočce, žádný z šesti výpadků toho dne health neřešil | Opraveno: blokátor je jen PIN1/PUK1, PIN2 zůstává v `lte.unlock_required` jako informace. Na starší jednotce se to pozná z logu health („modem hlásí sim_locked … obnova pozastavena“) při současném `mmcli -m any` = `connected` → doinstalovat aktualizaci |
| `mmcli -m any` = `locked`, health hlásí `lte.error=sim_locked` (nebo `sim_missing`) | SIM má PIN a profil ho nezná / SIM chybí | viz řádek výš; `sudo ./scripts/install.sh` — `MOTOGO_SIM_PIN` zapíše `[gsm] pin=` do `motogo-lte`, prázdná hodnota PIN z profilu odstraní |
...
| potřebuju na místě příkazovou řádku (síť, LTE, služby) | — | Na displeji: diagnostický kód / servisní heslo → **⌨ Terminál** (nebo servisní panel → „⌨ Servisní terminál"). Připravená tlačítka (ip, nmcli, mmcli, systemctl, journalctl, ping, nahození `motogo-lan`/`motogo-lte`, USB reset, restart) jsou vždy; **volné psaní příkazů má rovnou servisní heslo** (funguje i offline — kvůli tomu terminál vznikl); s pouhým diagnostickým kódem ho povolí Velín (Samoobsluha → „⌨ Terminál na displeji", 30 min, pak se samo zamkne). Běží pod uživatelem `motogo` (ne root), každý příkaz jde do Hlášení a chyb. Detail: CONTRACT §27 |
