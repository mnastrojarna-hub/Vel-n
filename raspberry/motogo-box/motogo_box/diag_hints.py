...
HINTS = {
    ...
    "net_outages": "Výpadky internetu: pobočka jede JEN přes LTE a modem SIM7600 v režimu QMI občas padá z USB (kernel -71); health ho obnoví (reconnect → reset → USB reset, do ~2 min). Zkontrolujte anténu, SIM (PIN vypnutý, data) a signál (RSRP > −105 dBm). Když výpadků přibývá, řešení je jiný modem/USB kabel — RNDIS je jen experimentální (26. 9.: modem po přepnutí zmizel z USB).",
    "modem_gone": "Modem SIM7600 mizí z USB / z ModemManageru (kernel -71, HARDWARE.md „Známá závada“): health ho resetuje (obnova do ~2 min). Trvale: jiný USB kabel/port přímo na Raspberry, jinak jiný modem. RNDIS jen experimentálně s technikem u jednotky (karta → „Modem → RNDIS (experimentální)“).",
