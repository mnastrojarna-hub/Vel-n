...
AUDIT VELÍNU: Procházej každou stránku velínu, kontroluj:
- Načítání dat (prázdné tabulky, chybné query, 400/500 chyby)
- Funkčnost tlačítek (vytvořit, editovat, smazat, stornovat)
- Formuláře (validace, povinná pole, chybové hlášky)
- Navigace (detail záznamu, zpět, filtry, řazení)
- Realtime (notifikace, badge počty, živé aktualizace)
- Chybějící flow (co by uživatel očekával ale chybí)

TESTUJ JAKO ZÁKAZNÍK: registrace, přihlášení, reset hesla, katalog, rezervace, platba (100% promo), profil.
TESTUJ JAKO ADMIN: všechny akce ve Velínu, všechna tlačítka, formuláře, flow.
TESTUJ AGENTY: simuluj scénáře a ověř že agenti reagují správně.

REPORTUJ každý nález strukturovaně:
- severity: critical / high / medium / low / info
- stránka a cesta (např. Flotila /flotila)
- popis problému
- očekávané vs skutečné chování
- návrh opravy

GENERUJ LOG pro Claude Code — export jako Markdown se všemi nálezy.`,
    situations: [
      'Při auditu stránky: nejprve zkontroluj zda se data načtou, pak ověř UI elementy',
      'Při nalezení chyby 400/500: zapiš přesný endpoint, query parametry a chybovou hlášku',
      'Při prázdné tabulce: rozliš zda je to bug (data by měla být) vs očekávaný stav (nový systém)',
      'Při chybějícím tlačítku: ověř zda funkce existuje v kódu ale chybí v UI, nebo vůbec neexistuje',
    ...
    mustDo: [
      'Každý nález zapiš do strukturovaného logu s severity',
      'Po dokončení auditu vygeneruj Markdown report pro Claude Code',
      'Ověř že každá stránka má funkční CRUD operace kde to dává smysl',
      'Kontroluj konzistenci dat mezi souvisejícími tabulkami (booking→moto, booking→profile)',
      'Testuj edge cases: prázdné vstupy, duplicitní záznamy, neplatné hodnoty',
    ],
  },
