...
export async function generatePaymentReceipt(bookingId, source = 'booking', payment = null) {
  ...
  // Jen AKTIVNÍ řádky: kód šatny zadržený triggerem _sync_locker_code („Vlastní výbava",
  // is_active=false) ani staré kódy po regeneraci nesmí doklad označit za „zadržený".
  let doorCodesNote = ''
  try {
...
export async function loadInvoiceData(invoiceId) {
  ...
  if (data && data.type === 'payment_receipt' && data.booking_id) {
    ...
    if (!rentalOver) {
      try {
        // Jen aktivní řádky (zadržený kód šatny „Vlastní výbava" / staré kódy nejsou „zadržené doklady")
        const { data: codes } = await supabase
          .from('branch_door_codes')
          .select('code_type, door_code, withheld_reason')
