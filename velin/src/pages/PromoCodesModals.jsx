...
export function PromoModal({ existing, onClose, onSaved }) {
  ...
  const [form, setForm] = useState(
    existing ? { code: existing.code || '', discount_type: existing.type || 'percent', discount_value: existing.value?.toString() || '', valid_from: existing.valid_from || '', valid_to: existing.valid_to || '', usage_limit: existing.max_uses?.toString() || '', status: existing.active ? 'active' : 'inactive' }
  ...
  async function handleSave() {
    ...
    try {
      const payload = { code: form.code.trim().toUpperCase(), type: form.discount_type, value: Number(form.discount_value) || 0, max_uses: form.usage_limit ? Number(form.usage_limit) : null, valid_from: form.valid_from || null, valid_to: form.valid_to || null, active: form.status === 'active' }
      if (isEdit) {
  ...
  return (
    <Modal open title={isEdit ? `Upravit: ${existing.code}` : 'Nový promo kód'} onClose={onClose}>
      <div className="grid grid-cols-2 gap-3">
        ...
        <div><Label>Stav</Label><select value={form.status} onChange={e => set('status', e.target.value)} className="w-full rounded-btn text-sm outline-none" style={inputStyle}><option value="active">Aktivní</option><option value="inactive">Neaktivní</option></select></div>
      </div>
      {isEdit && existing.used_count > 0 && <div className="mt-3 p-3 rounded-lg" style={{ background: '#f1faf7', border: '1px solid #d4e8e0' }}><span className="text-sm font-bold" style={{ color: '#1a2e22' }}>Tento kód byl použit {existing.used_count}× z {existing.max_uses ?? '∞'} povolených.</span></div>}
      {err && <p className="mt-3 text-sm" style={{ color: '#dc2626' }}>{err}</p>}
      <div className="flex justify-end gap-3 mt-5"><Button onClick={onClose}>Zrušit</Button><Button green onClick={handleSave} disabled={saving || !form.code || !form.discount_value}>{saving ? 'Ukládám…' : isEdit ? 'Uložit změny' : 'Vytvořit'}</Button></div>
    </Modal>
  )
}
...
export function PromoDetailModal({ code, onClose, onEdit }) {
  ...
  useEffect(() => {
    supabase.from('promo_code_usage').select('*, profiles:customer_id(full_name, email)').eq('promo_code_id', code.id).order('used_at', { ascending: false }).limit(20)
      .then(({ data }) => { setUsage(data || []); setLoadingUsage(false) }).catch(() => { setUsage([]); setLoadingUsage(false) })
  }, [code.id])
  ...
  return (
    <Modal open title={`Detail: ${code.code}`} onClose={onClose}>
      ...
      <div className="flex justify-end gap-3 mt-5"><Button onClick={onClose}>Zavrit</Button><Button onClick={onEdit} style={{ background: '#2563eb', color: '#fff' }}>Upravit</Button></div>
    </Modal>
  )
}
