...
export default function FinancePrehledTab({
  filters, setFilters, categories, summary, chartData, recentInvoices,
  shopPayments, invoiceSums, transactions, detailTx, setDetailTx,
  handleExport, error, loading, debugMode, fmt,
}) {
  ...
  return <>
    <div className="flex flex-wrap items-center gap-3 mb-5">
      ...
      <CheckboxFilterGroup label="Typ" values={filters.types || []}
        ...
        options={[{ value: 'revenue', label: 'Prijmy' }, { value: 'expense', label: 'Vydaje' }]} />
      {categories.length > 0 && (
      ...
      <select value={filters.sort} onChange={e => setFilters(f => ({ ...f, sort: e.target.value }))}
        className="rounded-btn text-sm font-extrabold uppercase tracking-wide cursor-pointer outline-none"
        style={{ padding: '8px 14px', background: '#f1faf7', border: '1px solid #d4e8e0', color: '#1a2e22' }}>
        <option value="date_desc">Datum ↓ nejnovejsi</option>
        <option value="date_asc">Datum ↑ nejstarsi</option>
        <option value="amount_desc">Castka ↓ nejvyssi</option>
        <option value="amount_asc">Castka ↑ nejnizsi</option>
      </select>
    ...
    {loading ? (
      ...
      <>
        ...
        <div className="mb-3 p-3 rounded-card" style={{ background: '#fffbeb', border: '1px solid #fbbf24', fontSize: 13, fontFamily: 'monospace', color: '#78350f' }}>
          ...
          <div>invoices: {recentInvoices.length} zaznamu {recentInvoices.length > 0 && `[${recentInvoices.slice(0,5).map(i => `${i.type}/${i.number}/${i.total}Kc`).join(', ')}${recentInvoices.length > 5 ? '...' : ''}]`}</div>
          <div>accounting_entries: {transactions.length} zaznamu</div>
          <div>shop_orders (paid): {shopPayments.length} zaznamu</div>
        </div>
        )}

        <InvoiceSumsOverview invoiceSums={invoiceSums} fmt={fmt} />
        <InvoicesTable recentInvoices={recentInvoices} fmt={fmt} />
        <ShopPaymentsTable shopPayments={shopPayments} fmt={fmt} />
        <AccountingEntriesTable transactions={transactions} setDetailTx={setDetailTx} fmt={fmt} />
      </>
    )}
    ...
    {detailTx && (
      <Modal open title="Detail transakce" onClose={() => setDetailTx(null)}>
        ...
        <div className="flex justify-end mt-5">
          <Button onClick={() => setDetailTx(null)}>Zavrit</Button>
        </div>
      </Modal>
    )}
  </>
...
function InvoiceSumsOverview({ invoiceSums, fmt }) {
  return (
    <Card className="mb-5">
