          ...
          <div className="space-y-2 max-h-48 overflow-auto">
            {usage.map(u => (
              <div key={u.id} className="flex items-center gap-3 p-2 rounded-lg text-sm" style={{ background: '#f1faf7' }}>
                <span className="font-bold">{u.profiles?.full_name || u.profiles?.email || 'Neznámý'}</span>
                <span style={{ color: '#1a2e22' }}>{u.used_at ? new Date(u.used_at).toLocaleString('cs-CZ') : ''}</span>
                <span className="ml-auto font-bold" style={{ color: '#1a8a18' }}>-{u.discount_applied?.toLocaleString('cs-CZ')} Kč</span>
              </div>
            ))}
          </div>
        )}
