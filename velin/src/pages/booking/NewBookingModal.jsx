...
export default function NewBookingModal({ onClose, onSaved }) {
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <button onClick={() => setCalMonth(p => p.m === 0 ? { m: 11, y: p.y - 1 } : { m: p.m - 1, y: p.y })}
            className="cursor-pointer text-sm font-bold" style={{ background: '#f1faf7', border: '1px solid #d4e8e0', borderRadius: 8, padding: '4px 10px', color: '#4a6357' }}>←</button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3 }}>
          {DAY_NAMES.map(n => <div key={n} className="text-[10px] font-bold text-center" style={{ color: '#8aab99', padding: 4 }}>{n}</div>)}
          {cells.map((date, i) => {
        ...
        <div className="flex items-center gap-4 mt-3 text-[10px]" style={{ color: '#8aab99' }}>
  ...
  return (
    <Modal open title="Nová rezervace" onClose={onClose} wide>
        ...
        {STEP_LABELS.map((label, i) => (
          <div key={i} className="flex items-center gap-2">
            <div className="flex items-center gap-1 cursor-pointer" onClick={() => {
              ...
              <div className="text-xs font-extrabold rounded-full flex items-center justify-center"
                style={{
                  width: 22, height: 22,
              ...
              <span className="text-[10px] font-bold uppercase tracking-wide"
                style={{ color: step === i + 1 ? '#0f1a14' : '#8aab99' }}>{label}</span>
            </div>
      ...
      {step === 1 && (
        <div>
          <p className="text-xs mb-3" style={{ color: '#4a6357' }}>
          ...
          {startDate && (
            <div className="mt-4 p-3 rounded-lg" style={{ background: '#f1faf7', border: '1px solid #d4e8e0' }}>
              <div className="flex gap-6">
                <div>
                  <span className="text-[10px] font-bold uppercase" style={{ color: '#8aab99' }}>Vyzvednutí</span>
                  <div className="text-sm font-bold" style={{ color: '#0f1a14' }}>{fmtDate(startDate)}</div>
                </div>
                <div>
                  <span className="text-[10px] font-bold uppercase" style={{ color: '#8aab99' }}>Vrácení</span>
                  <div className="text-sm font-bold" style={{ color: endDate ? '#0f1a14' : '#d1d5db' }}>{endDate ? fmtDate(endDate) : 'Vyberte…'}</div>
                </div>
                {days > 0 && <div>
                  <span className="text-[10px] font-bold uppercase" style={{ color: '#8aab99' }}>Dní</span>
      ...
      {step === 2 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-bold" style={{ color: '#4a6357' }}>
              Volné motorky pro {fmtDate(startDate)} – {fmtDate(endDate)} ({days} dní)
            </p>
            <span className="text-[10px] font-bold" style={{ color: '#1a8a18' }}>{availableMotos.length} volných</span>
          </div>
          ...
          {loadingMotos ? (
            ...
            <div style={{ maxHeight: 380, overflowY: 'auto' }}>
              {availableMotos.length === 0 && (
                <p className="text-sm py-6 text-center" style={{ color: '#8aab99' }}>Žádná motorka není volná v tomto termínu</p>
              )}
                ...
                return (
                  <div key={m.id}
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[10px]" style={{ color: '#8aab99' }}>{days} dní</div>
                    </div>
              ...
              {unavailableMotos.length > 0 && (
                <>
                  <div className="text-[10px] font-bold uppercase tracking-wide mt-4 mb-2" style={{ color: '#dc2626' }}>
                  ...
                  {unavailableMotos.map(m => (
                    <div key={m.id} className="flex items-center gap-4 p-3 rounded-lg mb-2" style={{ background: '#fafafa', border: '1px solid #f3f4f6', opacity: 0.5 }}>
                      ...
                      <div className="flex-1">
                        <div className="text-sm font-bold" style={{ color: '#6b7280' }}>{m.model}</div>
                        <div className="text-[10px]" style={{ color: '#9ca3af' }}>{m.spz || '—'} · Obsazená</div>
                      </div>
                    </div>
                  ))}
                </>
      ...
      {step === 3 && (
        <div>
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              {selectedCustomer ? (
                <div className="p-3 rounded-lg" style={{ background: '#eafbe9', border: '1px solid #74FB71' }}>
                  ...
                  <div className="text-[10px]" style={{ color: '#4a6357' }}>{selectedCustomer.email}</div>
                  {selectedCustomer.phone && <div className="text-[10px]" style={{ color: '#8aab99' }}>{selectedCustomer.phone}</div>}
                  <button onClick={() => { setSelectedCustomer(null); setCustomerSearch('') }}
                    className="text-[10px] font-bold cursor-pointer mt-1" style={{ color: '#dc2626', background: 'none', border: 'none' }}>Změnit</button>
                </div>
              ) : (
                <div>
                  ...
                  {filteredCustomers.length > 0 && (
                    <div className="mt-1 rounded-lg" style={{ border: '1px solid #d4e8e0', maxHeight: 180, overflowY: 'auto' }}>
                      {filteredCustomers.map(c => (
                        <div key={c.id} className="p-2 cursor-pointer text-xs hover:bg-green-50"
                          style={{ borderBottom: '1px solid #f1faf7' }}
                          onClick={() => { setSelectedCustomer(c); setCustomerSearch('') }}>
                          <span className="font-bold">{c.full_name}</span>
                          <span className="ml-2" style={{ color: '#8aab99' }}>{c.email}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {customerSearch.length > 1 && filteredCustomers.length === 0 && (
                    <p className="text-[10px] mt-1" style={{ color: '#8aab99' }}>Žádný zákazník nenalezen</p>
                  )}
                </div>
              )}

              {/* Pickup time */}
              <label className="block text-[10px] font-extrabold uppercase tracking-wide mb-1 mt-4" style={{ color: '#8aab99' }}>Čas vyzvednutí</label>
              <input type="time" value={pickupTime} onChange={e => setPickupTime(e.target.value)}
                className="w-full rounded-btn text-sm outline-none"
                style={{ padding: '8px 12px', background: '#f1faf7', border: '1px solid #d4e8e0' }} />

              {/* Notes */}
              <label className="block text-[10px] font-extrabold uppercase tracking-wide mb-1 mt-4" style={{ color: '#8aab99' }}>Poznámka</label>
            ...
            {/* Summary */}
            <div>
              <label className="block text-[10px] font-extrabold uppercase tracking-wide mb-1" style={{ color: '#8aab99' }}>Shrnutí rezervace</label>
              <div className="rounded-lg p-4" style={{ background: '#f8faf9', border: '1px solid #e5e7eb' }}>
                ...
                <div className="text-[10px] mb-3" style={{ color: '#8aab99' }}>{selectedMoto?.spz || '—'}</div>

                <div className="flex justify-between text-xs mb-1">
                  <span style={{ color: '#4a6357' }}>Vyzvednutí</span>
                  <span className="font-bold" style={{ color: '#0f1a14' }}>{fmtDate(startDate)}</span>
                </div>
                <div className="flex justify-between text-xs mb-1">
                  <span style={{ color: '#4a6357' }}>Vrácení</span>
                  <span className="font-bold" style={{ color: '#0f1a14' }}>{fmtDate(endDate)}</span>
                </div>
                <div className="flex justify-between text-xs mb-3">
                  <span style={{ color: '#4a6357' }}>Počet dní</span>
              ...
              <label className="flex items-center gap-2 mt-4 cursor-pointer p-3 rounded-lg" style={{ background: '#fffbeb', border: '1px solid #fbbf24' }}>
                ...
                <div>
                  <div className="text-xs font-bold" style={{ color: '#0f1a14' }}>Bez platby</div>
                  <div className="text-[10px]" style={{ color: '#8aab99' }}>
                    {noPayment
