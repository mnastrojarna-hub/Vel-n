...
export default function BookingPriceSection({
  newBreakdown, selectedMoto, booking, origPaidPrice, origCalcPrice,
  origDays, days, newCalcPrice, newDeliveryFee, newTotalPrice, priceDiff,
  chargeCustomer, setChargeCustomer,
}) {
  return (
    <div className="mb-5">
      ...
      <div className="p-4 rounded-lg" style={{ background: '#f1faf7', border: '1px solid #d4e8e0' }}>
        <div className="space-y-2">
          ...
          {origCalcPrice > 0 && origCalcPrice !== origPaidPrice && <div className="flex justify-between text-xs"><span style={{ color: '#9ca3af' }}>Dle ceníku (původní motorka x {origDays}d)</span><span style={{ color: '#9ca3af' }}>{fmtCZK(origCalcPrice)} Kč</span></div>}
          ...
          <div style={{ borderTop: '2px solid #d4e8e0', paddingTop: 8, marginTop: 4 }}><div className="flex justify-between text-sm font-extrabold"><span style={{ color: '#1a2e22' }}>Nová celkova cena</span><span style={{ color: '#0f1a14' }}>{fmtCZK(newTotalPrice)} Kč</span></div></div>
          {priceDiff !== 0 && (
