...
export default function FinanceOverviewTab({
  debugMode, filters, setFilters, defaultFilters, categories,
  error, loading, summary, chartData, invoiceSums,
  recentInvoices, shopPayments, transactions,
  detailTx, setDetailTx, handleExport,
}) {
                  ...
                  {recentInvoices.map(inv => {
                    ...
                    return (
                      <TRow key={inv.id}>
                        ...
                        <TD><span className="inline-block rounded-btn text-sm font-extrabold tracking-wide uppercase" style={{ padding: '4px 10px', background: typeBgs[inv.type] || '#f3f4f6', color: typeColors[inv.type] || '#6b7280' }}>{typeLabels[inv.type] || inv.type}</span></TD>
