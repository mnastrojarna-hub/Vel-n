...
// nesmí ho sdílet dvě zóny ani zóna + venek); v režimu selector zůstává relé {dev, coil} beze změny
...
function DoorHwEditor({ doors, devices, audio, outdoor, busy, onSaveDoor }) {
  ...
  async function save(d) {
    ...
    if (dupZones.has(zoneNo)) {
      const text = zoneNo === outdoorZone ? `Zónu ${zoneNo} má venek (blok Venek) — čísla zón musí být unikátní.` : `Duplicitní čísla zón — zónu ${zoneNo} mají i jiné dveře.`
      setMsg(m => ({ ...m, [d.id]: { text, tone: 'red' } })); return
    }
  ...
  return (
    <div className="space-y-1 max-h-96 overflow-y-auto">
      {doors.map(d => {
        ...
        return (
          <DoorHwRow key={d.id} door={d} draft={draft} devices={devices} audio={audio} devOptions={devOptions} dupes={dupes} dupZones={dupZones} dupOuts={dupOuts}
            outdoorZone={outdoorZone} busy={busy} msg={msg[d.id]} onPatch={fn => patch(d.id, fn)} onSave={() => save(d)} onClear={() => clear(d)} />
        )
      })}
      {dupes.size > 0 && (
      ...
      {dupZones.size > 0 && (
        <div className="text-[12px] font-bold p-2 rounded-lg" style={{ background: '#fee2e2', color: '#dc2626' }}>
          Duplicitní čísla zón: {[...dupZones].join(', ')} — každé dveře (i venek) musí mít vlastní číslo zóny (jednotka by celou mapu odmítla).
        </div>
      )}
      {dupOuts.size > 0 && (
        <div className="text-[12px] font-bold p-2 rounded-lg" style={{ background: '#fee2e2', color: '#dc2626' }}>
          Sdílené audio výstupy: {[...dupOuts].join(', ')} — každý výstup smí používat jen jedna zóna nebo venek.
        </div>
      )}
    </div>
  )
