...
const CATEGORIES = [
  ...
  { value: 'equipment', label: 'Vybaveni' }, { value: 'intangible', label: 'Nehmotný majetek' },
]
...
export function AddAssetModal({ onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      await supabase.from('accounting_entries').insert({ type: 'expense', amount: price, description: `Pořízení DM: ${form.name}`, category: 'dlouhodoby_majetek', date: form.acquired_date })
      onSaved()
  ...
  return (
    <Modal open title="Novy dlouhodobý majetek" onClose={onClose}>
      <div className="space-y-3">
        ...
        <div><Label>Odpisová skupina</Label><select value={form.depreciation_group} onChange={e => set('depreciation_group', e.target.value)} className="w-full rounded-btn text-sm outline-none" style={inputStyle}>{DEPRECIATION_GROUPS.map(g => <option key={g.value} value={g.value}>{g.label}</option>)}</select></div>
        <div><Label>Metoda odpisů</Label><select value={form.depreciation_method} onChange={e => set('depreciation_method', e.target.value)} className="w-full rounded-btn text-sm outline-none" style={inputStyle}>{DEPRECIATION_METHODS.map(m => <option key={m.value} value={m.value}>{m.label}</option>)}</select></div>
        {selectedGroup && form.purchase_price && <div className="p-3 rounded-lg" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', fontSize: 12 }}><strong>Nahled odpisů:</strong> {selectedGroup.years} let, 1. rok: {fmt(Math.round(Number(form.purchase_price) * selectedGroup.firstYearRate / 100))}, další roky: {fmt(Math.round(Number(form.purchase_price) * selectedGroup.nextYearRate / 100))}</div>}
      ...
      <div className="flex justify-end gap-3 mt-5"><Button onClick={onClose}>Zrušit</Button><Button green onClick={handleSave} disabled={saving || !form.name || !form.purchase_price}>{saving ? 'Ukladam...' : 'Uložit'}</Button></div>
    </Modal>
  )
}
  ...
  return (
    <Modal open title={`Majetek: ${asset.name}`} onClose={onClose}>
      <div className="grid grid-cols-2 gap-4 mb-4">
        <DetailRow label="Název" value={asset.name} />
        <DetailRow label="Kategorie" value={CATEGORIES.find(c => c.value === asset.category)?.label || asset.category} />
        <DetailRow label="Pořizovací cena" value={fmt(asset.purchase_price)} />
        <DetailRow label="Zustat. hodnota" value={fmt(asset.current_value)} />
        <DetailRow label="Odepsáno" value={`${fmt(asset.total_depreciated)} (${pct}%)`} />
        <DetailRow label="Odpisová skupina" value={group ? group.label : `Sk. ${asset.depreciation_group}`} />
        <DetailRow label="Metoda" value={asset.depreciation_method === 'linear' ? 'Rovnomerne' : 'Zrychlene'} />
        <DetailRow label="Datum pořízení" value={asset.acquired_date ? new Date(asset.acquired_date).toLocaleDateString('cs-CZ') : '—'} />
      </div>
      <div className="mb-4">
        ...
        <div className="text-sm mt-1" style={{ color: '#6b7280' }}>{pct}% odepsáno</div>
      </div>
