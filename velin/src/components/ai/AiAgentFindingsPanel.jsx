...
export default function AiAgentFindingsPanel() {
  ...
  for (const a of AGENTS) {
    ...
    summary[a.id] = { problems: findings.filter(f => f.severity === 'fail').length, warnings: findings.filter(f => f.severity === 'warn').length, total: metrics[a.id]?.total || 0, confidence: metrics[a.id]?.confidence || 0 }
  }
  ...
  function checkAllProblems() { const next = { ...checked }; filtered.forEach((f, i) => { if (f.severity === 'fail' || f.severity === 'warn') next[i] = true }); setChecked(next) }
