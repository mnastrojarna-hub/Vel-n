...
export default function AiTestRunner() {
  ...
  async function runStep(step) {
    ...
    try {
      ...
      // Extract test user/promo from response
      ...
      // Record learning outcome for tester agent
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-2">
          <Button green onClick={runAll} disabled={!!running} style={{ fontSize: 12, padding: '6px 14px' }}>
            {running ? 'Běží...' : 'Spustit vše'}
          </Button>
        </div>
      </div>

      {/* Test user + promo info */}
      {(testUser || testPromo) && (
      ...
      <div style={{ marginBottom: 8, padding: '8px 12px', borderRadius: 8, background: '#f8fcfa', border: '1px solid #d4e8e0' }}>
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            { label: 'Katalog motorek', url: `${APP_URL}/#catalog` },
            { label: 'Rezervace', url: `${APP_URL}/#booking` },
            { label: 'E-shop', url: `${APP_URL}/#shop` },
            { label: 'Profil', url: `${APP_URL}/#profile` },
            { label: 'Platební metody', url: `${APP_URL}/#payment-methods` },
          ].map(l => (
            <a key={l.label} href={l.url} target="_blank" rel="noopener noreferrer" style={{
              fontSize: 10, padding: '3px 8px', borderRadius: 4, background: '#fff', border: '1px solid #d4e8e0',
          ...
          return (
            <div key={step.id} style={{
              ...
              <div className="flex items-center gap-2">
                ...
                {result?.status === 'done' && <span style={{ fontSize: 12, color: '#22c55e' }}>✓</span>}
                {result?.status === 'error' && <span style={{ fontSize: 12, color: '#ef4444' }}>✗</span>}
              ...
              {result?.error && (
                <div style={{ marginTop: 4, fontSize: 11, color: '#dc2626' }}>Chyba: {result.error}</div>
              )}
            </div>
          )
        })}
